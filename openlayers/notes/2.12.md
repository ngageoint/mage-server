...
***REMOVED***s. This works by requesting tiles at higher resolutions and stretching
...
The `OpenLayers.Layer.MultiMap` cl***REMOVED*** has been removed entirely, as the MultiMap ***REMOVED*** was discontinued.
...
Previously a few functions in the library displayed error messages and returned `undefined`, `null` or `false` if the parameters p***REMOVED***ed in were bad. In 2.12 these functions now just throw an error/exception. People relying on return values to know if a function call is successful may need to change their code. Here are the modified functions:
...
We are deprecating the Rico cl***REMOVED***es/objects in OpenLayers. This has the following implications:
...
The base `OpenLayers.Geometry` cl***REMOVED*** no longer depends on `OpenLayers.Format.WKT` or `OpenLayers.Feature.Vector`.  If you want to make use of the `OpenLayers.Geometry.fromWKT` method, you must explicitly include the OpenLayers/Format/WKT.js file in your build.  
...
See the cl***REMOVED*** description in the API docs for `OpenLayers.Layer.Google.v3` for more details.

Good ideas on how to improve this u***REMOVED***tisfactory situation welcome!
...
The internal `OpenLayers.Layer.getURLasync` function now take a bound, a callback and a scope. The function no longer needs update the p***REMOVED***ed property but simply to return to url.
...
To replace a tile that couldn't be loaded with a static image, create a css selector for the `.olImageLoadError` cl***REMOVED*** (e.g. a `background-image`).
 ...
 * OpenLayers.Cl***REMOVED***.isPrototype
 * OpenLayers.Cl***REMOVED***.create
 * OpenLayers.Cl***REMOVED***.inherit
