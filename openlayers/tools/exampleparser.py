...
def getRelatedCl***REMOVED***es(html):
    """
    parses the html, and returns a list of all OpenLayers Cl***REMOVED***es 
    used within (ie what parts of OL the javascript uses).  
    """
    rawstr = r'''(?P<cl***REMOVED***>OpenLayers\..*?)\('''
...
def parseHtml(html,ids):
    ...
    #cl***REMOVED***es should eventually be parsed from docs - not automatically created.
    cl***REMOVED***es = getRelatedCl***REMOVED***es(html)
    d['cl***REMOVED***es'] = cl***REMOVED***es
    return d
...
def getGitInfo(exampleDir, exampleName):
    ...
    parts = log.split("|")
    d["author"] = parts[0]
    # compe***REMOVED***te for spaces in git log time
    td = parts[1].split(" ")
    td.insert(1, "T")
    d["date"] = "".join(td)
    return d
