 ...
 *         fields:['name', 'seniority', '***REMOVED***'],
 *         groupField: '***REMOVED***',
 *         data: {'employees':[
 *             { "name": "Michael Scott",  "seniority": 7, "***REMOVED***": "Management" },
 *             { "name": "Dwight Schrute", "seniority": 2, "***REMOVED***": "Sales" },
 *             { "name": "Jim Halpert",    "seniority": 3, "***REMOVED***": "Sales" },
 *             { "name": "Kevin Malone",   "seniority": 4, "***REMOVED***": "Accounting" },
 *             { "name": "Angela Martin",  "seniority": 5, "***REMOVED***": "Accounting" }
 *         ]},
 *         proxy: {
 *             type: 'memory',
 *             reader: {
...
Ext.define('Ext.grid.feature.Grouping', {
     ...
     * @cfg {String}           groupHeaderTpl.columnName         The column header ***REMOVED***ociated with the field being grouped by *if there is a column for the field*, falls back to the groupField name.
     ...
     * 
     * This is set to `false` when the ***REMOVED***ociated {@link Ext.data.Store store} is 
    ...
    getFeatureTpl: function(values, parent, x, xcount) {
        return [
            '<tpl if="typeof rows !== \'undefined\'">',
                // group row tpl
                '<tr id="{groupHeaderId}" cl***REMOVED***="' + Ext.baseCSSPrefix + 'grid-group-hd {hdCollapsedCls} {collapsibleCl***REMOVED***}"><td cl***REMOVED***="' + Ext.baseCSSPrefix + 'grid-cell" colspan="' + parent.columns.length + '" {[this.indentByDepth(values)]}><div cl***REMOVED***="' + Ext.baseCSSPrefix + 'grid-cell-inner"><div cl***REMOVED***="' + Ext.baseCSSPrefix + 'grid-group-title">{collapsed}{[this.renderGroupHeaderTpl(values, parent)]}</div></div></td></tr>',
                // this is the rowbody
                '<tr id="{groupBodyId}" cl***REMOVED***="' + Ext.baseCSSPrefix + 'grid-group-body {collapsedCls}"><td colspan="' + parent.columns.length + '">{[this.recurse(values)]}</td></tr>',
            '</tpl>'
        ].join('');
    },
     ...
     * @param {Boolean} focus P***REMOVED*** `true` to focus the group after expand.
     */
    ...
    getGroupRows: function(group, records, preppedRecords, fullWidth) {
        ...
        if (me.collapsible) {
            group.collapsibleCl***REMOVED*** = me.hdCollapsibleCls;
        } else {
            group.collapsibleCl***REMOVED*** = '';
        }

        return group;
    },

    // Create an ***REMOVED***ociated DOM id for the group's header element given the group name
    getGroupHeaderId: function(groupName) {
        return this.view.id + '-hd-' + groupName;
    },

    // Create an ***REMOVED***ociated DOM id for the group's body element given the group name
    getGroupBodyId: function(groupName) {
        return this.view.id + '-bd-' + groupName;
    },

    // Get the group name from an ***REMOVED***ociated element whether it's within a header or a body
