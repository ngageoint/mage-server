...
Ext.define('Ext.grid.column.Action', {
    ...
    alternateCl***REMOVED***Name: 'Ext.grid.ActionColumn',
    ...
    /**
     * @cfg {String} iconCls
     * A CSS cl***REMOVED*** to apply to the icon image. To determine the cl***REMOVED*** dynamically, configure the Column with
     * a `{@link #getCl***REMOVED***}` function.
     */
    /**
    ...
    /**
     * @cfg {Object} scope
     * The scope (**this** reference) in which the `{@link #handler}` and `{@link #getCl***REMOVED***}` fuctions are executed.
     ...
     */
    /**
     * @cfg {Function} getCl***REMOVED***
     * A function which returns the CSS cl***REMOVED*** to apply to the icon image.
     *
     * @cfg {Object} getCl***REMOVED***.v The value of the column's configured field (if any).
     *
     * @cfg {Object} getCl***REMOVED***.metadata An object in which you may set the following attributes:
     * @cfg {String} getCl***REMOVED***.metadata.css A CSS cl***REMOVED*** name to add to the cell's TD element.
     * @cfg {String} getCl***REMOVED***.metadata.attr An HTML attribute definition string to apply to the data container
     * element *within* the table cell (e.g. 'style="color:red;"').
     *
     * @cfg {Ext.data.Model} getCl***REMOVED***.r The Record providing the data.
     *
     * @cfg {Number} getCl***REMOVED***.rowIndex The row index..
     *
     * @cfg {Number} getCl***REMOVED***.colIndex The column index.
     *
     * @cfg {Ext.data.Store} getCl***REMOVED***.store The Store which is providing the data Model.
     */
    /**
     * @cfg {Object[]} items
     ...
     *
     * @cfg {String} items.iconCls A CSS cl***REMOVED*** to apply to the icon image. To determine the cl***REMOVED*** dynamically,
     * configure the item with a `getCl***REMOVED***` function.
     *
     * @cfg {Function} items.getCl***REMOVED*** A function which returns the CSS cl***REMOVED*** to apply to the icon image.
     * @cfg {Object} items.getCl***REMOVED***.v The value of the column's configured field (if any).
     * @cfg {Object} items.getCl***REMOVED***.metadata An object in which you may set the following attributes:
     * @cfg {String} items.getCl***REMOVED***.metadata.css A CSS cl***REMOVED*** name to add to the cell's TD element.
     * @cfg {String} items.getCl***REMOVED***.metadata.attr An HTML attribute definition string to apply to the data
     * container element _within_ the table cell (e.g. 'style="color:red;"').
     * @cfg {Ext.data.Model} items.getCl***REMOVED***.r The Record providing the data.
     * @cfg {Number} items.getCl***REMOVED***.rowIndex The row index..
     * @cfg {Number} items.getCl***REMOVED***.colIndex The column index.
     * @cfg {Ext.data.Store} items.getCl***REMOVED***.store The Store which is providing the data Model.
     *
     * @cfg {Function} items.handler A function called when the icon is clicked.
     *
     * @cfg {Object} items.scope The scope (`this` reference) in which the `handler` and `getCl***REMOVED***` functions
     ...
     * an `enable` and `disable` method added which will enable and disable the ***REMOVED***ociated action, and
    ...
    constructor: function(config) {
        ...
        for (i = 0, len = items.length; i < len; ++i) {
            if (items[i].getCl***REMOVED***) {
                hasGetCl***REMOVED*** = true;
                break;
            }
        }
        
        // Also need to check for getCl***REMOVED***, since it changes how the cell renders
        if (me.origRenderer || hasGetCl***REMOVED***) {
            me.hasCustomRenderer = true;
        }
    },
    
    // Renderer closure iterates through items creating an <img> element for each and tagging with an identifying
    // cl***REMOVED*** name x-action-col-{n}
    defaultRenderer: function(v, meta){
        ...
        for (; i < len; i++) {
            ...
            v += '<img alt="' + (item.altText || me.altText) + '" src="' + (item.icon || Ext.BLANK_IMAGE_URL) +
                '" cl***REMOVED***="' + prefix + 'action-col-icon ' + prefix + 'action-col-' + String(i) + ' ' + (item.disabled ? prefix + 'item-disabled' : ' ') +
                ' ' + (Ext.isFunction(item.getCl***REMOVED***) ? item.getCl***REMOVED***.apply(item.scope || scope, arguments) : (item.iconCls || me.iconCls || '')) + '"' +
    ...
    processEvent : function(type, view, cell, recordIndex, cellIndex, e, record, row){
        ...
        // NOTE: The statement below tests the truthiness of an ***REMOVED***ignment.
        if (target && (match = target.cl***REMOVED***Name.match(me.actionIdRe))) {
