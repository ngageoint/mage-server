/**
 * This cl***REMOVED*** specifies the definition for a column inside a {@link Ext.grid.Panel}. It encomp***REMOVED***es
 ...
 * contain other columns inside. In general, this cl***REMOVED*** will not be created directly, rather
 * an array of column configurations will be p***REMOVED***ed to the grid:
 *
 *     @example
 ...
 *     });
 *
 * # Convenience Subcl***REMOVED***es
 *
 * There are several column subcl***REMOVED***es that provide default rendering for various data types
 *
...
Ext.define('Ext.grid.column.Column', {
    ...
    alternateCl***REMOVED***Name: 'Ext.grid.Column',
    ...
    renderTpl:
        '<div id="{id}-titleEl" {tipMarkup}cl***REMOVED***="' + Ext.baseCSSPrefix + 'column-header-inner">' +
            '<span id="{id}-textEl" cl***REMOVED***="' + Ext.baseCSSPrefix + 'column-header-text">' +
                '{text}' +
            '</span>' +
            '<tpl if="!menuDisabled">'+
                '<div id="{id}-triggerEl" cl***REMOVED***="' + Ext.baseCSSPrefix + 'column-header-trigger"></div>'+
            '</tpl>' +
        '</div>' +
     ...
     *     }
     *
     * Additionally a string naming an {@link Ext.util.Format} method can be p***REMOVED***ed:
     *
     *     {
     *         renderer: 'uppercase'
     *     }
     ...
     * This is meant to be defined in subcl***REMOVED***es that wish to supply their own renderer.
     * @protected
     * @template
     */
    ...
    // Header does not use the typical ComponentDraggable cl***REMOVED*** and therefore we
    // override this with an emptyFn. It is controlled at the HeaderDragZone.
    initDraggable: Ext.emptyFn,

    /**
     * @cfg {String} tdCls
     * A CSS cl***REMOVED*** names to apply to the table cells for this column.
     */
     ...
     * Set in this cl***REMOVED*** to identify, at runtime, instances which are not instances of the
     * HeaderContainer base cl***REMOVED***, but are in fact, the subcl***REMOVED***: Header.
     */
    isHeader: true,
    ...
    // Total available header height must be p***REMOVED***ed to enable padding for inner elements to be calculated.
     ...
     * @param {Number} recordIndex Index of the ***REMOVED***ociated Store Model (-1 if none)
    ...
    /**
     * @method getEditor
     * Retrieves the editing field for editing ***REMOVED***ociated with this header. Returns false if there is no field
     * ***REMOVED***ociated with the Header the method will return false. If the field has not been instantiated it will be
