 ...
 *     });
 *
 * This does several things. First it creates a global variable called 'MyApp' - all of your Application's cl***REMOVED***es (such
 ...
 * files whenever we add a new cl***REMOVED***, but more importantly it enables us to create a minimized build of our entire
...
Ext.define('Ext.app.Application', {
     ...
     * The path to the directory which contains all application's cl***REMOVED***es.
    ...
    constructor: function(config) {
        ...
        if (me.autoCreateViewport) {
            requires.push(me.getModuleCl***REMOVED***Name('Viewport', 'view'));
        }

        for (i = 0; i < ln; i++) {
            requires.push(me.getModuleCl***REMOVED***Name(controllers[i], 'controller'));
        }
    ...
    getModuleCl***REMOVED***Name: function(name, module) {
        ...
        if (name.indexOf('.') !== -1 && (Ext.Cl***REMOVED***Manager.isCreated(name) || Ext.Loader.isACl***REMOVED***NameWithAKnownPrefix(name))) {
            return name;
        } else {
    ...
    getController: function(name) {
        ...
        if (!controller) {
            controller = Ext.create(this.getModuleCl***REMOVED***Name(name, 'controller'), {
                application: this,
                id: name
            });
    ...
    getStore: function(name) {
        ...
        if (!store) {
            store = Ext.create(this.getModuleCl***REMOVED***Name(name, 'store'), {
                storeId: name
            });
        }
    ...
    getModel: function(model) {
        model = this.getModuleCl***REMOVED***Name(model, 'model');

        return Ext.ModelManager.getModel(model);
    },

    getView: function(view) {
        view = this.getModuleCl***REMOVED***Name(view, 'view');

        return Ext.Cl***REMOVED***Manager.get(view);
    }
});
