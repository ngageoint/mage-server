 ...
 * makes it easy to listen to events on your view cl***REMOVED***es and take some action with a handler function. Let's update
 ...
 * it allows us to p***REMOVED*** a CSS-like selector that will find every matching component on the page.
 *
 ...
 *     });
 *
 * This example ***REMOVED***umes the existence of a {@link Ext.grid.Panel Grid} on the page, which contains a single button to
 ...
 * ***REMOVED***igns it to the reference 'list'.
 *
...
Ext.define('Ext.app.Controller', {
    ...
    onCl***REMOVED***Extended: function(cls, data, hooks) {
        var cl***REMOVED***Name = Ext.getCl***REMOVED***Name(cls),
            match = cl***REMOVED***Name.match(/^(.*)\.controller\./),
            namespace,
        ...
        if (match !== null) {
            namespace = Ext.Loader.getPrefix(cl***REMOVED***Name) || match[1];
            ...
            hooks.onBeforeCreated = function(cls, data) {
                ...
                for (i = 0,ln = modules.length; i < ln; i++) {
                    ...
                    for (j = 0,subLn = items.length; j < subLn; j++) {
                        item = items[j];
                        // Deciding if a cl***REMOVED*** name must be qualified:
                        // 1 - if the name doesn't contains at least one dot, we must definitely qualify it
                        // 2 - the name may be a qualified name of a known cl***REMOVED***, but:
                        // 2.1 - in runtime, the loader may not know the cl***REMOVED*** - specially in production - so we must check the cl***REMOVED*** manager
                        // 2.2 - in build time, the cl***REMOVED*** manager may not know the cl***REMOVED***, but the loader does, so we check the second one
                        //       (the loader check ***REMOVED***ures it's really a cl***REMOVED***, and not a namespace, so we can have 'Books.controller.Books',
                        //       and requesting a controller called Books will not be underqualified)
                        if (item.indexOf('.') !== -1 && (Ext.Cl***REMOVED***Manager.isCreated(item) || Ext.Loader.isACl***REMOVED***NameWithAKnownPrefix(item))) {
                            requires.push(item);
                        } else {
                ...
                Ext.require(requires, Ext.Function.p***REMOVED***(onBeforeCl***REMOVED***Created, arguments, this));
            };
        }
    },
    ...
    createGetters: function(type, refs) {
        ...
        for (; i < length; i++) {
            ...
            // Handle namespaced cl***REMOVED*** names. E.g. feed.Add becomes getFeedAddView etc.
            ...
            if (!this[fn]) {
                this[fn] = Ext.Function.p***REMOVED***(this['get' + type], [ref], this);
            }
    ...
    ref: function(refs) {
        ...
        for (; i < length; i++) {
            ...
            if (!me[fn]) {
                me[fn] = Ext.Function.p***REMOVED***(me.getRef, [ref, info], me);
            }
     ...
     * listeners, otherwise an object of selectors -> listeners is ***REMOVED***umed
    ...
    /**
     * Returns a {@link Ext.data.Model Model} cl***REMOVED*** with the given name.
    ...
    /**
     * Returns a View cl***REMOVED*** with the given name.  To create an instance of the view,
     * you can use it like it's used by Application to create the Viewport:
     *
     *     this.getView('Viewport').create();
     *
     * @param {String} name
     * @return {Ext.Base} a view cl***REMOVED***.
     */
