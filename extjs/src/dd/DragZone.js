/**
 * This cl***REMOVED*** provides a container DD instance that allows dragging of multiple child source nodes.
 *
 * This cl***REMOVED*** does not move the drag target nodes, but a proxy element which may contain any DOM structure you wish. The
 ...
 * object (For example nodes in a {@link Ext.view.View DataView}) then use of this cl***REMOVED*** is the most efficient way to
 * "activate" those nodes.
 *
 * By default, this cl***REMOVED*** requires that draggable child nodes are registered with {@link Ext.dd.Registry}. However a
 * simpler way to allow a DragZone to manage any number of draggable elements is to configure the DragZone with an
 * implementation of the {@link #getDragData} method which interrogates the p***REMOVED***ed mouse event to see if it has taken
 * place within an element, or cl***REMOVED*** of elements. This is easily done by using the event's {@link
...
Ext.define('Ext.dd.DragZone', {
     ...
     * based on the mouse down. Override this method to provide your own lookup logic (e.g. finding a child by cl***REMOVED***
