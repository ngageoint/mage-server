/**
 * This cl***REMOVED*** encapsulates the user interface for a tabular data set.
 ...
 *
 * This cl***REMOVED*** does not provide ways to manipulate the underlying data of the configured
 * {@link Ext.data.Store}.
 *
 * This is the base cl***REMOVED*** for both {@link Ext.grid.View} and {@link Ext.tree.View} and is not
 * to be used directly.
 */
Ext.define('Ext.view.Table', {
     ...
     * A CSS cl***REMOVED*** to add to the *first* cell in every row to enable special styling for the first column.
     ...
     * A CSS cl***REMOVED*** to add to the *last* cell in every row to enable special styling for the last column.
    ...
    /**
     * Override this function to apply custom CSS cl***REMOVED***es to rows during rendering. This function should return the
     * CSS cl***REMOVED*** name (or empty string '' for none) that will be added to the row's wrapping div. To apply multiple
     * cl***REMOVED*** names, simply return them space-delimited within the string (e.g. 'my-cl***REMOVED*** another-cl***REMOVED***').
     * Example usage:
     *
     *     viewConfig: {
     *         getRowCl***REMOVED***: function(record, rowIndex, rowParams, store){
     ...
     * @return {String} a CSS cl***REMOVED*** name to add to the row.
     * @method
     */
    getRowCl***REMOVED***: null,

    /**
     * @cfg {Boolean} stripeRows
     * True to stripe the rows.
     *
     * This causes the CSS cl***REMOVED*** **`x-grid-row-alt`** to be added to alternate rows of
    ...
    moveColumn: function(fromIdx, toIdx, colsToMove) {
        var me = this,
            ...
            lastIdx = colCount - 1,
            doFirstLastCl***REMOVED***es = (me.firstCls || me.lastCls) && (toIdx === 0 || toIdx == colCount || fromIdx === 0 || fromIdx == lastIdx),
            i,
            j,
        ...
        if (me.rendered) {
            ...
            for (i = 0, len = rows.length; i < len; i++) {
                ...
                // Keep first cell cl***REMOVED*** and last cell cl***REMOVED*** correct *only if needed*
                if (doFirstLastCl***REMOVED***es) {
    ...
    // Private template method implemented starting at the AbstractView cl***REMOVED***.
    ...
    collectData: function(records, startIndex) {
        ...
        // process row cl***REMOVED***es, rowParams has been deprecated and has been moved
        // to the individual features that implement the behavior.
        if (me.getRowCl***REMOVED***) {
            for (; j < jln; j++) {
                rowParams = {};
                rec = preppedRecords[j];
                cls = rec.rowCls || '';
                rec.rowCls = this.getRowCl***REMOVED***(records[j], j, rowParams, me.store) + ' ' + cls;
                //<debug>
                if (rowParams.alt) {
                    Ext.Error.raise("The getRowCl***REMOVED*** alt property is no longer supported.");
                }
                if (rowParams.tstyle) {
                    Ext.Error.raise("The getRowCl***REMOVED*** tstyle property is no longer supported.");
                }
                if (rowParams.cells) {
                    Ext.Error.raise("The getRowCl***REMOVED*** cells property is no longer supported.");
                }
                if (rowParams.body) {
                    Ext.Error.raise("The getRowCl***REMOVED*** body property is no longer supported. Use the getAdditionalData method of the rowbody feature.");
                }
                if (rowParams.bodyStyle) {
                    Ext.Error.raise("The getRowCl***REMOVED*** bodyStyle property is no longer supported.");
                }
                if (rowParams.cols) {
                    Ext.Error.raise("The getRowCl***REMOVED*** cols property is no longer supported.");
                }
                //</debug>
            }
        }
    ...
    refreshSize: function() {
        ...
        if (!me.hasLoadingHeight) {
            ...
            // Suspend layouts in case the supercl***REMOVED*** requests a layout. We might too, so they
            // must be coalescsed.
            Ext.suspendLayouts();
    ...
    /**
     * Adds a CSS Cl***REMOVED*** to a specific row.
    ...
    /**
     * Removes a CSS Cl***REMOVED*** from a specific row.
     * @param {HTMLElement/String/Number/Ext.data.Model} rowInfo An HTMLElement, index or instance of a model
     * representing this row
     * @param {String} cls
     */
     ...
     * record ***REMOVED***ociated with the node.
     */
    ...
    getHeaderByCell: function(cell) {
        if (cell) {
            var m = cell.cl***REMOVED***Name.match(this.cellRe);
            if (m && m[1]) {
    ...
    // TODO: have this use the new Ext.grid.CellContext cl***REMOVED***
    ...
    doStripeRows: function(startRow, endRow) {
        ...
        if (me.rendered && me.stripeRows) {
            ...
            for (i = 0, rowsLn = rows.length; i < rowsLn; i++) {
                row = rows[i];
                // Remove prior applied row cl***REMOVED***es.
                row.cl***REMOVED***Name = row.cl***REMOVED***Name.replace(me.rowClsRe, ' ');
                startRow++;
                // Every odd row will get an additional cls
                if (startRow % 2 === 0) {
                    row.cl***REMOVED***Name += (' ' + me.altRowCls);
                }
            }
        }
    }
