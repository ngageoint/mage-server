/**
 * @cl***REMOVED*** Ext.view.AbstractView
 * This is an abstract supercl***REMOVED*** and should not be used directly. Please see {@link Ext.view.View}.
 * @private
 */
Ext.define('Ext.view.AbstractView', {
     ...
     * <b>This is a required setting</b>. A simple CSS selector (e.g. <tt>div.some-cl***REMOVED***</tt> or
    ...
    /**
     * @cfg {String} itemCls
     * Specifies the cl***REMOVED*** to be ***REMOVED***igned to each element in the view when used in conjunction with the
     ...
     * A CSS cl***REMOVED*** to apply to each item in the view on mouseover.
     ...
     * The CSS cl***REMOVED*** to apply to the loading message element. Defaults to Ext.LoadMask.prototype.msgCls "x-mask-loading".
     */
     ...
     * A CSS cl***REMOVED*** to apply to each selected item in the view.
     */
    ...
    initComponent : function(){
        ...
        if (itemTpl) {
            ...
            itemTpl = Ext.String.format('<tpl for="."><div cl***REMOVED***="{0}">{1}</div></tpl>', me.itemCls, itemTpl);
        ...
        if (!isDef(me.tpl) || !isDef(me.itemSelector)) {
            Ext.Error.raise({
                sourceCl***REMOVED***: 'Ext.view.View',
                tpl: me.tpl,
        ...
        //<debug>
        // backwards compat alias for overCl***REMOVED***/selectedCl***REMOVED***
        // TODO: Consider support for overCls generation Ext.Component config
        if (isDef(me.overCls) || isDef(me.overCl***REMOVED***)) {
            if (Ext.isDefined(Ext.global.console)) {
                Ext.global.console.warn('Ext.view.View: Using the deprecated overCls or overCl***REMOVED*** configuration. Use overItemCls instead.');
            }
            me.overItemCls = me.overCls || me.overCl***REMOVED***;
            delete me.overCls;
            delete me.overCl***REMOVED***;
        }
        ...
        if (isDef(me.selectedCls) || isDef(me.selectedCl***REMOVED***)) {
            if (Ext.isDefined(Ext.global.console)) {
                Ext.global.console.warn('Ext.view.View: Using the deprecated selectedCls or selectedCl***REMOVED*** configuration. Use selectedItemCls instead.');
            }
            me.selectedItemCls = me.selectedCls || me.selectedCl***REMOVED***;
            delete me.selectedCls;
            delete me.selectedCl***REMOVED***;
        }
        //</debug>
        ...
        me.addEvents(
            ...
            /**
             * @event itemupdate
             * Fires when the node ***REMOVED***ociated with an individual record is updated
            ...
            /**
             * @event itemadd
             * Fires when the nodes ***REMOVED***ociated with an recordset have been added to the underlying store
            ...
            /**
             * @event itemremove
             * Fires when the node ***REMOVED***ociated with an individual record is removed
             * @param {Ext.data.Model} record The model instance
             * @param {Number} index The index of the record/node
             */
            'itemremove'
    ...
    // Private template method to be overridden in subcl***REMOVED***es.
    ...
    prepareData: function(data, index, record) {
        var ***REMOVED***ociatedData, attr;
        if (record) {
            ***REMOVED***ociatedData = record.getAssociatedData();
            for (attr in ***REMOVED***ociatedData) {
                if (***REMOVED***ociatedData.hasOwnProperty(attr)) {
                    data[attr] = ***REMOVED***ociatedData[attr];
                }
            }
        }
        return data;
    ...
    /**
     * <p>Function which can be overridden which returns the data object p***REMOVED***ed to this
    ...
    /**
     * Returns the store ***REMOVED***ociated with this DataView.
    ...
    /**
     * Returns the template node the p***REMOVED***ed child belongs to, or null if it doesn't belong to one.
    ...
    /**
     * Returns true if the p***REMOVED***ed node is selected, else false.
     ...
     * the id of a template node or the record ***REMOVED***ociated with the node.
    ...
    /**
     * Finds the index of the p***REMOVED***ed node.
     ...
     * or a record ***REMOVED***ociated with a node.
