/**
 * An abstract base cl***REMOVED*** which provides shared methods for Containers across the Sencha product line.
 *
...
Ext.define('Ext.container.AbstractContainer', {
     ...
     *     each layout type, see the layout cl***REMOVED*** corresponding to the `type`
     *     specified.
     *
     * # Specify as a String
     *
     ...
     *     layout cl***REMOVED*** corresponding to the `layout` specified.
     *
     * # Configuring the default layout type
     *
     *     If a certain Container cl***REMOVED*** has a default layout (For example a {@link Ext.toolbar.Toolbar Toolbar}
     ...
     *
     * Every Component cl***REMOVED*** has its own {@link Ext.Component#xtype xtype}.
     *
     ...
     * using this Container as the scope (`this` reference) and p***REMOVED***ing the added item as
     * the first parameter. Any resulting object from that call is then applied to the item
     * as default properties.
     *
     * For example, to automatically apply padding to the body of each of a set of
     * contained {@link Ext.panel.Panel} items, you could p***REMOVED***:
     ...
     * and not p***REMOVED***ing them as multiple arguments or an array.
     */
    suspendLayout : false,
     ...
     * `true` in this cl***REMOVED*** to identify an object as an instantiated Container, or subcl***REMOVED*** thereof.
     */
    isContainer : true,
    ...
    initComponent : function(){
        ...
        me.addEvents(
            /**
             * @event afterlayout
             * Fires when the components in this container are arranged by the ***REMOVED***ociated layout manager.
    ...
    /**
     * @private
     * Returns the focus holder element ***REMOVED***ociated with this Container. By default, this is the Container's target
     * element. Subcl***REMOVED***es which use embedded focusable elements (such as Window and Button) should override this for use
    ...
    /**
     * Returns the {@link Ext.layout.container.Container layout} instance currently ***REMOVED***ociated with this Container.
    ...
    getLayout : function() {
        ...
        if (!me.layout || !me.layout.isLayout) {
            // P***REMOVED*** any configured in layout property, defaulting to the prototype's layout property, falling back to Auto.
    ...
    prepareItems : function(items, applyDefaults) {
        // Create an Array which does not refer to the p***REMOVED***ed array.
        // The p***REMOVED***ed array is a reference to a user's config object and MUST NOT be mutated.
        if (Ext.isArray(items)) {
            items = items.slice();
        } else {
            items = [items];
     ...
     *  
     * If adding multiple new child Components, p***REMOVED*** them as an array to the `add` method,
    ...
    add : function() {
        ...
        // We need to update our layout after adding all p***REMOVED***ed items
        me.updateLayout();
        if (me.rendered) {
     ...
     * is p***REMOVED***ed the Component which has been added. This method may
     ...
     * removed. It is p***REMOVED***ed the Component which has been
     ...
     * it become necessary.
     *
     * A Component config object may be p***REMOVED***ed in order to avoid the overhead of
     ...
     * is p***REMOVED***ed the new Component, and may be used to modify the
    ...
    /**
     * Cascades down the component/container heirarchy from this component (p***REMOVED***ed in
     ...
     * always p***REMOVED***ed as the last argument.
    ...
    /**
     * Determines whether **this Container** is an ancestor of the p***REMOVED***ed Component.
     * This will return `true` if the p***REMOVED***ed Component is anywhere within the subtree
    ...
    /**
     * Retrieves all descendant components which match the p***REMOVED***ed selector.
    ...
    /**
     * Retrieves all descendant components which match the p***REMOVED***ed function.
     ...
     * @return {Ext.Component[]} Components matched by the p***REMOVED***ed function
     */
    ...
    /**
     * Retrieves the first direct child of this container which matches the p***REMOVED***ed selector.
     * The p***REMOVED***ed in selector must comply with an Ext.ComponentQuery selector.
    ...
    /**
     * Retrieves the first descendant of this container which matches the p***REMOVED***ed selector.
     * The p***REMOVED***ed in selector must comply with an Ext.ComponentQuery selector.
     * @param {String} [selector] An Ext.ComponentQuery selector. If no selector is
     * specified, the first child will be returned.
     * @return Ext.Component
