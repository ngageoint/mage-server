/**
 * Simple header cl***REMOVED*** which is used for on {@link Ext.panel.Panel} and {@link Ext.window.Window}.
 */
Ext.define('Ext.panel.Header', {
     ...
     * `true` in this cl***REMOVED*** to identify an objact as an instantiated Header, or subcl***REMOVED*** thereof.
     */
    isHeader       : true,
    defaultType    : 'tool',
    indicateDrag   : false,
    ...
    renderTpl: [
        '<div id="{id}-body" cl***REMOVED***="{baseCls}-body {bodyCls}',
    ...
    headingTpl: '<span id="{id}-textEl" cl***REMOVED***="{cls}-text {cls}-text-{ui}">{title}</span>',
            ...
            me.titleCmp = new Ext.draw.Component({
                ...
                xhooks: {
                    setSize: function (width) {
                        // don't p***REMOVED*** 2nd arg (height) on to setSize or we break 'flex:1'
    ...
    addUIClsToElement: function(cls) {
        var me = this,
            ...
            cl***REMOVED***es = [me.baseCls + '-body-' + cls, me.baseCls + '-body-' + me.ui + '-' + cls],
            array, i;

        if (me.bodyCls) {
            array = me.bodyCls.split(' ');

            for (i = 0; i < cl***REMOVED***es.length; i++) {
                if (!Ext.Array.contains(array, cl***REMOVED***es[i])) {
                    array.push(cl***REMOVED***es[i]);
                }
            }

            me.bodyCls = array.join(' ');
        } else {
            me.bodyCls = cl***REMOVED***es.join(' ');
        }
    ...
    removeUIClsFromElement: function(cls) {
        ...
        if (me.bodyCls) {
            ...
            for (i = 0; i < cl***REMOVED***es.length; i++) {
                Ext.Array.remove(array, cl***REMOVED***es[i]);
            }
    ...
    /**
     * Sets the CSS cl***REMOVED*** that provides the icon image for this header.  This method will replace any existing
     * icon cl***REMOVED*** if one has already been set.
     * @param {String} cls The new CSS cl***REMOVED*** name
     */
