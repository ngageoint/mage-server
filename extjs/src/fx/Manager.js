...
Ext.define('Ext.fx.Manager', {
    ...
    addAnim: function(anim) {
        ...
        // can target a single DOM node concurrently, the target id cannot be ***REMOVED***umned to be unique.
     ...
     * @return {Object} The animation target wrapper object containing the p***REMOVED***ed animation along with the
    ...
    collectTargetData: function(anim, elapsedTime, useCSS3, isLastFrame) {
        ...
        if (!target) {
            ...
            // target element and its ***REMOVED***ociated animations. This is important later when applying
        ...
        // ***REMOVED***ociated with it, which is why the anim is added to the target's `anims` hash by id.
    ...
    applyPendingAttrs: function() {
        ...
        for (targetId in targetArr) {
            if (targetArr.hasOwnProperty(targetId)) {
                ...
                // Each target could have multiple ***REMOVED***ociated animations, so iterate those
