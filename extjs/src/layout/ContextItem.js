/**
 * This cl***REMOVED*** manages state information for a component or element during a layout.
 * 
 * # Blocks
 *
 * A "block" is a required value that is preventing further calculation. When a layout has
 * encountered a situation where it cannot possibly calculate results, it can ***REMOVED***ociate
...
Ext.define('Ext.layout.ContextItem', {
    ...
    requires: ['Ext.layout.Cl***REMOVED***List'],
     ...
     * @return {Mixed} A value to p***REMOVED*** as the first argument to {@link #initContinue}.
     * @private
     */
    init: function (full, options) {
        ...
        if (full) {
            // We must null these out or getSizeModel will ***REMOVED***ume they are the correct,
    ...
    /**
     * Queue the addition of a cl***REMOVED*** name (or array of cl***REMOVED*** names) to this ContextItem's target when next flushed.
     */
    addCls: function(newCls) {
        this.getCl***REMOVED***List().addMany(newCls);
    },

    /**
     * Queue the removal of a cl***REMOVED*** name (or array of cl***REMOVED*** names) from this ContextItem's target when next flushed.
     */
    removeCls: function(removeCls) {
        this.getCl***REMOVED***List().removeMany(removeCls);
    },
    ...
    /**
     * Reschedules any layouts ***REMOVED***ociated with a given trigger.
     * 
    ...
    flush: function () {
        ...
        // Flush added/removed cl***REMOVED***es
        if (me.cl***REMOVED***List && me.cl***REMOVED***List.dirty) {
            me.cl***REMOVED***List.flush();
        }
    ...
    flushAnimations: function() {
        ...
        if (animateFrom) {
            ...
            if (changeCount) {
                // It'a Panel being collapsed. rollback, and then fix the cl***REMOVED*** name string
    ...
    /**
     * Returns a Cl***REMOVED***List-like object to buffer access to this item's element's cl***REMOVED***es.
     */
    getCl***REMOVED***List: function () {
        return this.cl***REMOVED***List || (this.cl***REMOVED***List = new Ext.layout.Cl***REMOVED***List(this));
    },
    ...
    /**
     * Invalidates the component ***REMOVED***ociated with this item. The layouts for this component
     ...
     * @param {Object} options.before.options The options object p***REMOVED***ed to {@link #invalidate}.
     ...
     * @param {Object} options.after.options The options object p***REMOVED***ed to {@link #invalidate}.
     ...
     * calculation is not done. To indicate that such as value was p***REMOVED***ed, 0 is returned.
     ...
     * 
     * Calculated values can sometimes be NaN or undefined, which generally means that the
     * calculation is not done. To indicate that either p***REMOVED***ed value was such a value, false
    ...
    writeProps: function(dirtyProps, flushing) {
        ...
        for (propName in dirtyProps) {
            ...
            if (info && info.dom) {
                // Numeric dirty values should have their ***REMOVED***ociated suffix added
        ...
        if (dirtyX || dirtyY) {
            if (target.isComponent) {
                // Ensure we always p***REMOVED*** the current coordinate in if one coordinate has not been dirtied by a calculation cycle.
        ...
        if (!isBorderBox && (width > 0 || height > 0)) { // no need to subtract from 0
            // The width and height values ***REMOVED***ume the border-box box model,
    ...
    // decoding values read by getStyle and preparing values to p***REMOVED*** to setStyle.
    //
