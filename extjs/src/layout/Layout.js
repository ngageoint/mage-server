/**
 * Base Layout cl***REMOVED*** - extended by ComponentLayout and ContainerLayout
 */
Ext.define('Ext.layout.Layout', {
     ...
     * `true` in this cl***REMOVED*** to identify an object as an instantiated Layout, or subcl***REMOVED*** thereof.
     */
    isLayout: true,
    initialized: false,
    running: false,
    ...
    statics: {
        ...
        create: function(layout, defaultType) {
            var Cl***REMOVED***Manager = Ext.Cl***REMOVED***Manager,
                layoutsByType = this.layoutsByType,
                alias, cl***REMOVED***Name, config, layoutCl***REMOVED***, type, load;
            ...
            if (!(layoutCl***REMOVED*** = layoutsByType[type])) {
                alias = 'layout.' + type;
                cl***REMOVED***Name = Cl***REMOVED***Manager.getNameByAlias(alias);
                ...
                layoutCl***REMOVED*** = Cl***REMOVED***Manager.get(cl***REMOVED***Name);
                if (load || !layoutCl***REMOVED***) {
                    return Cl***REMOVED***Manager.instantiateByAlias(alias, config || {});
                }
                layoutsByType[type] = layoutCl***REMOVED***;
            }

            return new layoutCl***REMOVED***(config);
        }
    },
     ...
     * 
     * This is a read phase and DOM writes should be strictly avoided in derived cl***REMOVED***es.
     ...
     * be flushed at the next opportunity.
     * 
     * This method need not be implemented by derived cl***REMOVED***es and, in fact, should only be
    ...
    /**
     * Iterates over all p***REMOVED***ed items, ensuring they are rendered.  If the items are already rendered,
    ...
    isValidParent : function(item, target, position) {
        ...
        if (itemDom.parentNode && itemDom.parentNode.cl***REMOVED***Name.indexOf(Ext.baseCSSPrefix + 'resizable-wrap') !== -1) {
    ...
    // Placeholder empty functions for subcl***REMOVED***es to extend
    ...
    afterRemove : function(item) {
        ...
        if (item.rendered) {
            removeCl***REMOVED***es = [].concat(me.itemCls || []);
            if (owner.itemCls) {
                removeCl***REMOVED***es = Ext.Array.push(removeCl***REMOVED***es, owner.itemCls);
            }
            if (removeCl***REMOVED***es.length) {
                el.removeCls(removeCl***REMOVED***es);
            }
        }
     ...
     * 
     * A derived cl***REMOVED*** can override either this method or `onDestroy` but in all
     * cases must call the base cl***REMOVED*** versions of these methods to allow the base cl***REMOVED*** to
     * perform its cleanup.
     * 
     * This method (or `onDestroy`) are overridden by subcl***REMOVED***es most often to purge
    ...
    /**
    * This cl***REMOVED*** describes a size determination strategy or algorithm used by the layout
    * system. There are special instances of this cl***REMOVED*** stored as static properties to
    ...
    *
    * @cl***REMOVED*** Ext.layout.SizeModel
    * @protected
    */
    ...
    var defaults = {
        ...
        * True if the size is determined by CSS and not by content. Such sizes are ***REMOVED***umed to
