 ...
 *
 *  - ContainerA's initComponent calls #render p***REMOVED***ing the `renderTo` property as the
 *    container argument.
 ...
 *  - The result of `getRenderTree` is p***REMOVED***ed to {@link Ext.DomHelper#append}.
 ...
 *    versions.
 *  - During the p***REMOVED***, the component's `el` is set. Likewise, the `renderSelectors` and
 ...
 *    capture its elements. Both of these cl***REMOVED***es use {@link Ext.util.ElementContainer} to
 *    handle `childEls` processing.
 *  - Once this is complete, a similar p***REMOVED*** is made by calling #finishAfterRender.
...
Ext.define('Ext.util.Renderable', {
    ...
    frameTpl: [
        ...
        '<tpl if="top">',
            '<tpl if="left"><div id="{fgid}TL" cl***REMOVED***="{frameCls}-tl {baseCls}-tl {baseCls}-{ui}-tl<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-tl</tpl>" style="background-position: {tl}; padding-left: {frameWidth}px" role="presentation"></tpl>',
                '<tpl if="right"><div id="{fgid}TR" cl***REMOVED***="{frameCls}-tr {baseCls}-tr {baseCls}-{ui}-tr<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-tr</tpl>" style="background-position: {tr}; padding-right: {frameWidth}px" role="presentation"></tpl>',
                    '<div id="{fgid}TC" cl***REMOVED***="{frameCls}-tc {baseCls}-tc {baseCls}-{ui}-tc<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-tc</tpl>" style="background-position: {tc}; height: {frameWidth}px" role="presentation"></div>',
                '<tpl if="right"></div></tpl>',
            '<tpl if="left"></div></tpl>',
        '</tpl>',
        '<tpl if="left"><div id="{fgid}ML" cl***REMOVED***="{frameCls}-ml {baseCls}-ml {baseCls}-{ui}-ml<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-ml</tpl>" style="background-position: {ml}; padding-left: {frameWidth}px" role="presentation"></tpl>',
            '<tpl if="right"><div id="{fgid}MR" cl***REMOVED***="{frameCls}-mr {baseCls}-mr {baseCls}-{ui}-mr<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-mr</tpl>" style="background-position: {mr}; padding-right: {frameWidth}px" role="presentation"></tpl>',
                '<div id="{fgid}MC" cl***REMOVED***="{frameCls}-mc {baseCls}-mc {baseCls}-{ui}-mc<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-mc</tpl>" role="presentation">',
                    '{%this.applyRenderTpl(out, values)%}',
                '</div>',
            '<tpl if="right"></div></tpl>',
        '<tpl if="left"></div></tpl>',
        '<tpl if="bottom">',
            '<tpl if="left"><div id="{fgid}BL" cl***REMOVED***="{frameCls}-bl {baseCls}-bl {baseCls}-{ui}-bl<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-bl</tpl>" style="background-position: {bl}; padding-left: {frameWidth}px" role="presentation"></tpl>',
                '<tpl if="right"><div id="{fgid}BR" cl***REMOVED***="{frameCls}-br {baseCls}-br {baseCls}-{ui}-br<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-br</tpl>" style="background-position: {br}; padding-right: {frameWidth}px" role="presentation"></tpl>',
                    '<div id="{fgid}BC" cl***REMOVED***="{frameCls}-bc {baseCls}-bc {baseCls}-{ui}-bc<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-bc</tpl>" style="background-position: {bc}; height: {frameWidth}px" role="presentation"></div>',
                '<tpl if="right"></div></tpl>',
            '<tpl if="left"></div></tpl>',
        '</tpl>',
    ...
    frameTableTpl: [
        ...
        '<table><tbody>',
            '<tpl if="top">',
                '<tr>',
                    '<tpl if="left"><td id="{fgid}TL" cl***REMOVED***="{frameCls}-tl {baseCls}-tl {baseCls}-{ui}-tl<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-tl</tpl>" style="background-position: {tl}; padding-left:{frameWidth}px" role="presentation"></td></tpl>',
                    '<td id="{fgid}TC" cl***REMOVED***="{frameCls}-tc {baseCls}-tc {baseCls}-{ui}-tc<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-tc</tpl>" style="background-position: {tc}; height: {frameWidth}px" role="presentation"></td>',
                    '<tpl if="right"><td id="{fgid}TR" cl***REMOVED***="{frameCls}-tr {baseCls}-tr {baseCls}-{ui}-tr<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-tr</tpl>" style="background-position: {tr}; padding-left: {frameWidth}px" role="presentation"></td></tpl>',
                '</tr>',
            '</tpl>',
            '<tr>',
                '<tpl if="left"><td id="{fgid}ML" cl***REMOVED***="{frameCls}-ml {baseCls}-ml {baseCls}-{ui}-ml<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-ml</tpl>" style="background-position: {ml}; padding-left: {frameWidth}px" role="presentation"></td></tpl>',
                '<td id="{fgid}MC" cl***REMOVED***="{frameCls}-mc {baseCls}-mc {baseCls}-{ui}-mc<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-mc</tpl>" style="background-position: 0 0;" role="presentation">',
                    '{%this.applyRenderTpl(out, values)%}',
                '</td>',
                '<tpl if="right"><td id="{fgid}MR" cl***REMOVED***="{frameCls}-mr {baseCls}-mr {baseCls}-{ui}-mr<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-mr</tpl>" style="background-position: {mr}; padding-left: {frameWidth}px" role="presentation"></td></tpl>',
            '</tr>',
            '<tpl if="bottom">',
                '<tr>',
                    '<tpl if="left"><td id="{fgid}BL" cl***REMOVED***="{frameCls}-bl {baseCls}-bl {baseCls}-{ui}-bl<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-bl</tpl>" style="background-position: {bl}; padding-left: {frameWidth}px" role="presentation"></td></tpl>',
                    '<td id="{fgid}BC" cl***REMOVED***="{frameCls}-bc {baseCls}-bc {baseCls}-{ui}-bc<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-bc</tpl>" style="background-position: {bc}; height: {frameWidth}px" role="presentation"></td>',
                    '<tpl if="right"><td id="{fgid}BR" cl***REMOVED***="{frameCls}-br {baseCls}-br {baseCls}-{ui}-br<tpl for="uiCls"> {parent.baseCls}-{parent.ui}-{.}-br</tpl>" style="background-position: {br}; padding-left: {frameWidth}px" role="presentation"></td></tpl>',
                '</tr>',
            '</tpl>',
        '</tbody></table>',
        '{%this.renderDockedItems(out,values,1);%}'
     ...
     * will have been styled according to the configuration, will have had any configured CSS cl***REMOVED***
    ...
    beforeRender: function () {
        ...
        if (me.disabled) {
            // p***REMOVED*** silent so the event doesn't fire the first time.
            me.disable(true);
        }
    },
    ...
    finishRender: function(containerIdx) {
        ...
        if (!me.el || me.$pid) {
            ...
            if (!me.el) {
                ...
                // TODO - what about cl***REMOVED***/style?
            }
    ...
    getElConfig : function() {
        ...
        // It's important to ***REMOVED***ign the id here as an autoEl.id could have been (wrongly) applied and this would get things out of sync
        config.id = me.id;
    ...
    /**
     * This function takes the position argument p***REMOVED***ed to onRender and returns a
     ...
     *
     * Subcl***REMOVED***es which override this to gain access to the structure at render time should
     * call the parent cl***REMOVED***'s method before attempting to access any child elements of the Component.
     *
    ...
    onRender: function(parentNode, containerIdx) {
        ...
        if (Ext.scopeResetCSS && !me.ownerCt) {
            // If this component's el is the body element, we add the reset cl***REMOVED*** to the html tag
            if (el.dom === body) {
                el.parent().addCls(Ext.resetCls);
            }
            // Otherwise, we ensure that there is a wrapper which has the reset cl***REMOVED***
            else {
                ...
                // Else we wrap this element in an element that adds the reset cl***REMOVED***.
                else {
        ...
        // If we're using the content box model, we also cannot ***REMOVED***ign initial sizes since we do not know the border widths to subtract
    ...
    /**
     * Renders the Component into the p***REMOVED***ed HTML element.
     * 
    ...
    updateFrame: function() {
        ...
        if (oldFrameMC) {
            if (me.frame) {
                ...
                // Store the cl***REMOVED*** names set on the new MC
                newMCCl***REMOVED***Name = this.frameMC.dom.cl***REMOVED***Name;
                ...
                // Apply the new mc cl***REMOVED***es to the old mc element
                oldFrameMC.dom.cl***REMOVED***Name = newMCCl***REMOVED***Name;
     ...
     * This information is memoized based upon the CSS cl***REMOVED*** name of this Component's element.
     * Because child Components are rendered as textual HTML as part of the topmost Container, a dummy div is inserted
     * into the document to receive the document element's CSS cl***REMOVED*** name, and therefore style attributes.
     */
    getFrameInfo: function() {
        ...
        var me = this,
            ...
            el = me.el || me.protoEl,
            cls = el.dom ? el.dom.cl***REMOVED***Name : el.cl***REMOVED***List.join(' '),
            frameInfo = frameInfoCache[cls],
            styleEl, left, top, info;

        if (frameInfo == null) {
            // Get the singleton frame style proxy with our el cl***REMOVED*** name stamped into it.
            ...
            //<debug error>
            // This happens when you set frame: true explicitly without using the x-frame mixin in s***REMOVED***.
    ...
    calculateFrame: function(left, top){
        // We actually p***REMOVED*** a string in the form of '[type][tl][tr]px [direction][br][bl]px' as
    ...
    /**
     * @private
     * Returns an offscreen div with the same cl***REMOVED*** name as the element this is being rendered.
