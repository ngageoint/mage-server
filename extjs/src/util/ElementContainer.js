/**
 * This mixin enables cl***REMOVED***es to declare relationships to child elements and provides the
 * mechanics for acquiring the {@link Ext.Element elements} and storing them on an object
 * instance as properties.
 *
 * This cl***REMOVED*** is used by {@link Ext.Component components} and {@link Ext.layout.container.Container container layouts} to
 ...
 *      - `select`: A selector that will be p***REMOVED***ed to {@link Ext#select}.
 *      - `selectNode`: A selector that will be p***REMOVED***ed to {@link Ext.DomQuery#selectNode}.
 * 
 ...
 * up the cl***REMOVED*** hierarchy so that they are effectively inherited. In other words, if a
 * cl***REMOVED*** where to derive from `Ext.ux.SomeComponent` in the example above, it could also
 ...
 * properties on all the super cl***REMOVED***es and mixins.
 * 
 * @private
 */
Ext.define('Ext.util.ElementContainer', {
    ...
    childEls: [
        ...
        //  1. It ensures that all cl***REMOVED***es have a childEls (avoid null ptr)
    ...
    constructor: function () {
        ...
        // cl***REMOVED***, its bases and the set of mixins...
    ...
    /**
     * Adds each argument p***REMOVED***ed to this method to the {@link Ext.AbstractComponent#cfg-childEls childEls} array.
     */
    ...
    getChildEls: function () {
        ...
        // Typically, however, the childEls is a cl***REMOVED***-level concept, so check to see if
        // we have cached the complete set on the cl***REMOVED***:
        self = me.self;
        return self.$childEls || me.getCl***REMOVED***ChildEls(self);
    },

    getCl***REMOVED***ChildEls: function (cls) {
        var me = this,
            result = cls.$childEls,
            childEls, i, length, forked, mixin, mixins, name, parts, proto, supr, superMixins;

        if (!result) {
            // We put the various childEls arrays into parts in the order of supercl***REMOVED***,
            // new mixins and finally from cls. These parts can be null or undefined and
            // we will skip them later.

            supr = cls.supercl***REMOVED***;
            if (supr) {
                supr = supr.self;
                parts = [supr.$childEls || me.getCl***REMOVED***ChildEls(supr)]; // super+mixins
            ...
            for (name in mixins) {
                if (mixins.hasOwnProperty(name) && !superMixins.hasOwnProperty(name)) {
                    ...
                    parts.push(mixin.$childEls || me.getCl***REMOVED***ChildEls(mixin));
                }
            }
