/**
 * Base cl***REMOVED*** that provides a common interface for publishing events. Subcl***REMOVED***es are expected to to have a property
...
Ext.define('Ext.util.Observable', {
    ...
    statics: {
        ...
        /**
         * Starts capture on the specified Observable. All events will be p***REMOVED***ed to the supplied function with the event
        ...
        /**
         * Sets observability on the p***REMOVED***ed cl***REMOVED*** constructor.
         *
         * This makes any event fired on any instance of the p***REMOVED***ed cl***REMOVED*** also fire a single event through
         * the **cl***REMOVED***** allowing for central handling of events on many instances at once.
         *
         * Usage:
         *
         ...
         *     });
         *
         * @param {Function} c The cl***REMOVED*** constructor to make observable.
        ...
        /**
         * Prepares a given cl***REMOVED*** for observable instances. This method is called when a
         * cl***REMOVED*** derives from this cl***REMOVED*** or uses this cl***REMOVED*** as a mixin.
         * @param {Function} T The cl***REMOVED*** constructor to prepare.
         * @private
         */
        prepareCl***REMOVED***: function (T, mixin) {
            // T.hasListeners is the object to track listeners on cl***REMOVED*** T. This object's
            // prototype (__proto__) is the "hasListeners" of T.supercl***REMOVED***.

            // Instances of T will create "hasListeners" that have T.hasListeners as their
            // immediate prototype (__proto__).

            if (!T.HasListeners) {
                // We create a HasListeners "cl***REMOVED***" for this cl***REMOVED***. The "prototype" of the
                // HasListeners cl***REMOVED*** is an instance of the HasListeners cl***REMOVED*** ***REMOVED***ociated
                // with this cl***REMOVED***'s super cl***REMOVED*** (or with Observable).
                var Observable = Ext.util.Observable,
                    HasListeners = function () {},
                    SuperHL = T.supercl***REMOVED***.HasListeners || (mixin && mixin.HasListeners) ||
                              Observable.HasListeners;

                // Make the HasListener cl***REMOVED*** available on the cl***REMOVED*** and its prototype:
                T.prototype.HasListeners = T.HasListeners = HasListeners;

                // And connect its "prototype" to the new HasListeners of our super cl***REMOVED***
                // (which is also the cl***REMOVED***-level "hasListeners" instance).
     ...
     *
     * While _some_ Ext JS Component cl***REMOVED***es export selected DOM events (e.g. "click", "mouseover" etc), this is usually
     * only done when extra value can be added. For example the {@link Ext.view.View DataView}'s **`{@link
     * Ext.view.View#itemclick itemclick}`** event p***REMOVED***ing the node clicked on. To access DOM events directly from a
     ...
     * `true` in this cl***REMOVED*** to identify an object as an instantiated Observable, or subcl***REMOVED*** thereof.
     */
    isObservable: true,
     ...
     * directly on the instance, or on its cl***REMOVED*** or a super cl***REMOVED*** (via {@link #observe}) or
    ...
    constructor: function(config) {
        ...
        // The subcl***REMOVED*** may have already initialized it.
        if (!me.hasListeners) {
    ...
    onCl***REMOVED***Extended: function (T) {
        if (!T.HasListeners) {
            // Some cl***REMOVED***es derive from us and some others derive from those cl***REMOVED***es. All
            // of these are p***REMOVED***ed to this method.
            Ext.util.Observable.prepareCl***REMOVED***(T);
        }
    },
    ...
    /**
     * Fires the specified event with the p***REMOVED***ed parameters (minus the event name, plus the `options` object p***REMOVED***ed
     * to {@link #addListener}).
     *
     ...
     * @param {Object...} args Variable number of parameters are p***REMOVED***ed to handlers.
     ...
     *
     * The method also allows for a single argument to be p***REMOVED***ed which is a config object
     ...
     *
     * **Note:** Unlike in ExtJS 3.x, the options object will also be p***REMOVED***ed as the last
     ...
     *
     * @param {String} eventName The type of event the handler was ***REMOVED***ociated with.
     * @param {Function} fn The handler to remove. **This must be a reference to the function p***REMOVED***ed into the
        ...
        if (typeof ename !== 'string') {
            ...
            if (event && event.isEvent) {
                if (event.removeListener(fn, scope) && !--me.hasListeners[ename]) {
                    ...
                    // cl***REMOVED*** observers to "poke" through and expose their presence.
     ...
     *
     * @param {Boolean} queueSuspended P***REMOVED*** as true to queue up suspended events to be fired
     ...
     *
     * The grid instance will then have an observable 'load' event which will be p***REMOVED***ed the
    ...
    /**
     * @private
     * Creates an event handling function which refires the event from this object as the p***REMOVED***ed event name.
     * @param newName
     ...
     * present. There is no implementation in the Observable base cl***REMOVED***.
     *
    ...
    var Observable = this,
        ...
        prepareMixin = function (T) {
            if (!T.HasListeners) {
                ...
                // Cl***REMOVED***es that use us as a mixin (best practice) need to be prepared.
                Observable.prepareCl***REMOVED***(T, this);

                // Now that we are mixed in to cl***REMOVED*** T, we need to watch T for derivations
                // and prepare them also.
                T.onExtended(function (U) {
                    Observable.prepareCl***REMOVED***(U);
                });

                // Also, if a cl***REMOVED*** uses us as a mixin and that cl***REMOVED*** is then used as
                // a mixin, we need to be notified of that as well.
                if (proto.onCl***REMOVED***MixedIn) {
                    // play nice with other potential overrides...
                    Ext.override(T, {
                        onCl***REMOVED***MixedIn: function (U) {
                    ...
                    proto.onCl***REMOVED***MixedIn = function (U) {
                        prepareMixin.call(this, U);
                    };
                }
            }
    ...
    Observable.observeCl***REMOVED*** = Observable.observe;
    ...
    Ext.apply(proto, {
        onCl***REMOVED***MixedIn: prepareMixin,
