 ...
 * {@link Ext.data.proxy.Proxy Proxy} that we want a JSON Reader. The Store automatically p***REMOVED***es the configured model to the
 * Store, so it is as if we p***REMOVED***ed this instead:
 *
 *     reader: {
 *         type : 'json',
 *         model: 'User'
 ...
 * p***REMOVED*** JsonReader a couple of configuration options to make it parse your format. For example, we can use the
 ...
 *     }
 *
 * To parse this we just p***REMOVED*** in a {@link #root} configuration that matches the 'users' above:
 *
 *     reader: {
 *         type: 'json',
 *         root: 'users'
 ...
 * of the data set itself or are used to reconfigure the Reader. To p***REMOVED*** metadata in the response you simply
 ...
 *     }
 *
 * If you were to p***REMOVED*** a response object containing attributes different from those initially defined above, you could
 ...
 * but will be accessible via the Reader's {@link #metaData} property (which is also p***REMOVED***ed to listeners via the Proxy's
 * {@link Ext.data.proxy.Proxy#metachange metachange} event (also relayed by the {@link Ext.data.AbstractStore#metachange
 * store}). Application code can then process the p***REMOVED***ed metadata in any way it chooses.
 * 
 * A simple example for how this can be used would be customizing the fields for a Model that is bound to a grid. By p***REMOVED***ing
 ...
 * could simply p***REMOVED*** a standard grid {@link Ext.panel.Table#columns column} config object as part of the `metaData` attribute
 * and then p***REMOVED*** that along to the grid. Here's a very simple example for how that could be accomplished:
 *
 *     // response format:
 *     {
 *         ...
...
Ext.define('Ext.data.reader.Json', {
    ...
    alternateCl***REMOVED***Name: 'Ext.data.JsonReader',
    alias : 'reader.json',
    ...
    readRecords: function(data) {
        //this has to be before the call to super because we use the meta data in the supercl***REMOVED*** readRecords
        if (data.metaData) {
    ...
    /**
     * @private
     * We're just preparing the data for the supercl***REMOVED*** by pulling out the record objects we want. If a {@link #record}
     ...
     * @private
     * @method
     * Returns an accessor expression for the p***REMOVED***ed Field. Gives support for properties such as the following:
     * 
     * - 'someProperty'
     * - 'some.property'
     * - 'some["property"]'
