 ...
 * # Loading Nested Data
 *
 * Readers have the ability to automatically load deeply-nested data objects based on the {@link Ext.data.***REMOVED***ociation.Association
 * ***REMOVED***ociations} configured on each Model. Below is an example demonstrating the flexibility of these ***REMOVED***ociations in a
 ...
 *         ],
 *
 *         hasMany  : {model: 'OrderItem', name: 'orderItems', ***REMOVED***ociationKey: 'order_items'},
 *         belongsTo: 'User'
 *     });
 *
 ...
 *         ],
 *
 *         belongsTo: ['Order', {model: 'Product', ***REMOVED***ociationKey: 'product'}]
 *     });
 *
 *     Ext.define("Product", {
 *         extend: 'Ext.data.Model',
 ...
 * and finally the Product ***REMOVED***ociated with each OrderItem. Now we can read the data and use it as follows:
 *
 ...
 *                     //usually, we would use the asynchronous version (see {@link Ext.data.***REMOVED***ociation.BelongsTo})
...
Ext.define('Ext.data.reader.Reader', {
    ...
    alternateCl***REMOVED***Name: ['Ext.data.Reader', 'Ext.data.DataReader'],
     ...
     * the whole dataset is not p***REMOVED***ed in one go, but is being paged from the remote server.
     */
    totalProperty: 'total',
     ...
     * since the data is ***REMOVED***umed to be a single-level array of arrays.
     * 
     ...
     * if needed. However, meta data is also often used to p***REMOVED*** other custom data to be processed
     * by application code. For example, it is common when reconfiguring the data model of a grid to
     * also p***REMOVED*** a corresponding column model config to be applied to the grid. Any such data will
     * not get applied to the Reader directly (it just gets p***REMOVED***ed through and is ignored by Ext).
     * This metaData property gives you access to all meta data that was p***REMOVED***ed, including any such
     ...
     * p***REMOVED***ed to the reader. Note that typically you would handle proxy's
     * {@link Ext.data.proxy.Proxy#metachange metachange} event which p***REMOVED***es this exact same meta
     ...
     * `true` in this cl***REMOVED*** to identify an object as an instantiated Reader, or subcl***REMOVED*** thereof.
     */
    isReader: true,
    ...
    /**
     * Reads the given response object. This method normalizes the different types of response object that may be p***REMOVED***ed to it.
     * If it's an XMLHttpRequest object, hand off to the subcl***REMOVED***' {@link #getResponseData} method.
    ...
    /**
     * Abstracts common functionality used by all Reader subcl***REMOVED***es. Each subcl***REMOVED*** is expected to call this function
    ...
    readRecords: function(data) {
         ...
         * The raw data object that was last p***REMOVED***ed to {@link #readRecords}. Stored for further processing if needed.
         */
        me.rawData = data;
        ...
        if (me.readRecordsOnFailure || success) {
            // If we p***REMOVED*** an array as the data, we dont use getRoot on the data.
    ...
    /**
     * @private
     * Loads a record's ***REMOVED***ociations from the data object. This prepopulates hasMany and belongsTo ***REMOVED***ociations
     * on the record provided.
     * @param {Ext.data.Model} record The record to load ***REMOVED***ociations for
     * @param {Object} data The data object
     * @return {String} Return value description
     */
    readAssociated: function(record, data) {
        var ***REMOVED***ociations = record.***REMOVED***ociations.items,
            i            = 0,
            length       = ***REMOVED***ociations.length,
            ***REMOVED***ociation, ***REMOVED***ociationData, proxy, reader;
        
        for (; i < length; i++) {
            ***REMOVED***ociation     = ***REMOVED***ociations[i];
            ***REMOVED***ociationData = this.getAssociatedDataRoot(data, ***REMOVED***ociation.***REMOVED***ociationKey || ***REMOVED***ociation.name);
            
            if (***REMOVED***ociationData) {
                reader = ***REMOVED***ociation.getReader();
                if (!reader) {
                    proxy = ***REMOVED***ociation.***REMOVED***ociatedModel.proxy;
                    // if the ***REMOVED***ociated model has a Reader already, use that, otherwise attempt to create a sensible one
                    if (proxy) {
                        reader = proxy.getReader();
                    } else {
                        reader = new this.constructor({
                            model: ***REMOVED***ociation.***REMOVED***ociatedName
                        });
                    }
                }
                ***REMOVED***ociation.read(record, reader, ***REMOVED***ociationData);
            }  
        }
    },
     ...
     * record, this should return the relevant part of that data for the given ***REMOVED***ociation name. This is only really
     * needed to support the XML Reader, which has to do a query to get the ***REMOVED***ociated data object
     * @param {Object} data The raw data object
     * @param {String} ***REMOVED***ociationName The name of the ***REMOVED***ociation to get data for (uses ***REMOVED***ociationKey if present)
     * @return {Object} The root
     */
    getAssociatedDataRoot: function(data, ***REMOVED***ociationName) {
        return data[***REMOVED***ociationName];
    },
    ...
    /**
     * @private
     * By default this function just returns what is p***REMOVED***ed to it. It can be overridden in a subcl***REMOVED***
    ...
    /**
     * @private
     * This will usually need to be implemented in a subcl***REMOVED***. Given a generic data object (the type depends on the type
    ...
    /**
     * Takes a raw response object (as p***REMOVED***ed to the {@link #read} method) and returns the useful data
     * segment from it. This must be implemented by each subcl***REMOVED***.
     * @param {Object} response The response object
     * @return {Ext.data.ResultSet} A ResultSet object
     */
    getResponseData: function(response) {
        //<debug>
        Ext.Error.raise("getResponseData must be implemented in the Ext.data.reader.Reader subcl***REMOVED***");
        //</debug>
    },
     ...
     * Subcl***REMOVED***es may need to implement their own getRoot function.
    ...
    recordDataExtractorTemplate : [
        ...
        '<tpl for="fields">',
        // createFieldAccessExpression must be implemented in subcl***REMOVED***es to extract data from the source object in the correct way
        ...
        // so the server is p***REMOVED***ing back a client id that can be used to pair the server side record up with the client record
        '<tpl if="clientIdProp">',
     ...
     *
     * The returned function must be p***REMOVED***ed the following parameters:
     *
    ...
    buildRecordDataExtractor: function() {
        ...
        // which, when p***REMOVED***ed a record data object, a raw data row in the format this Reader is configured to read,
