/**
 * @author Ed Spencer
 * @cl***REMOVED*** Ext.data.***REMOVED***ociation.HasMany
 * 
...
Ext.define('User', {
    ...
    // we can use the hasMany shortcut on the model to create a hasMany ***REMOVED***ociation
 ...
 * we specified in the ***REMOVED***ociation configuration above.</p>
 * 
...
//the user.products function was created automatically by the ***REMOVED***ociation and returns a {@link Ext.data.Store Store}
 ...
 * records where the ***REMOVED***ociated model's foreign key matches the owner model's primary key. For example, if a User
 ...
 * 
 * <p>The tweets ***REMOVED***ociation above is filtered by the query property by setting the {@link #filterProperty}, and is
 ...
 */
Ext.define('Ext.data.***REMOVED***ociation.HasMany', {
    extend: 'Ext.data.***REMOVED***ociation.Association',
    alternateCl***REMOVED***Name: 'Ext.data.HasManyAssociation',
    requires: ['Ext.util.Inflector'],

    alias: '***REMOVED***ociation.hasmany',

    /**
     * @cfg {String} foreignKey The name of the foreign key on the ***REMOVED***ociated model that links it to the owner
     * model. Defaults to the lowercased name of the owner model plus "_id", e.g. an ***REMOVED***ociation with a where a
...
console.log(group.getUserList());
    ...
    /**
     * @cfg {Object} storeConfig Optional configuration object that will be p***REMOVED***ed to the generated Store. Defaults to 
     * undefined.
     */
    
    /**
     * @cfg {String} filterProperty Optionally overrides the default filter that is set up on the ***REMOVED***ociated Store. If
     * this is not set, a filter is automatically created which filters the ***REMOVED***ociation based on the configured 
     ...
     * Use 'hasMany' to create a HasMany ***REMOVED***ociation
     * <pre><code>
***REMOVED***ociations: [{
    type: 'hasMany',
    model: 'User'
    ...
    constructor: function(config) {
        ...
        me.name = me.name || Ext.util.Inflector.pluralize(me.***REMOVED***ociatedName.toLowerCase());
     ...
     * by the owner model's primary key - e.g. in a hasMany ***REMOVED***ociation where Group hasMany Users, this function
    ...
    createStore: function() {
        var that            = this,
            ***REMOVED***ociatedModel = that.***REMOVED***ociatedModel,
    ...
    read: function(record, reader, ***REMOVED***ociationData){
        ...
        store.add(reader.read(***REMOVED***ociationData).records);
    
        //now that we've added the related records to the hasMany ***REMOVED***ociation, set the inverse belongsTo
        //***REMOVED***ociation on each of them if it exists
        inverse = this.***REMOVED***ociatedModel.prototype.***REMOVED***ociations.findBy(function(***REMOVED***oc){
