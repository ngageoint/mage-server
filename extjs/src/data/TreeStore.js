 ...
 * the hierarchical tree structure combined with a store. This cl***REMOVED*** is generally used
 * in conjunction with {@link Ext.tree.Panel}. This cl***REMOVED*** also relays many events from
...
Ext.define('Ext.data.TreeStore', {
    ...
    // Keep a copy of the default so we know if it's been changed in a subcl***REMOVED***/config
    ...
    /**
     * Removes all records that used to be descendants of the p***REMOVED***ed node from the removed array
     * @private
    ...
    clearRemoved: function(node) {
        ...
        if(node === me.getRootNode()) {
            // if the p***REMOVED***ed node is the root node, just reset the removed array
            me.removed = [];
            return;
        }
        ...
        for(i = removedLength; i--;) {
            ...
            while(targetNode && targetNode.getId() !== id) {
                // walk up the parent hierarchy until we find the p***REMOVED***ed node or until we get to the root node
                targetId = targetNode.get('parentId');
                targetNode = targetNode.parentNode || me.getNodeById(targetId) || removedHash[targetId];
            }
            if(targetNode) {
                // removed node was previously a descendant of the p***REMOVED***ed node - add it to the records to clear from "removed" later
