/**
 * The Store cl***REMOVED*** encapsulates a client side cache of {@link Ext.data.Model Model} objects. Stores load data via a
 ...
 * ## Inline data
 *
 * Stores can also load data inline. Internally, Store converts each of the objects we p***REMOVED*** in as {@link #cfg-data} into
 * Model instances:
 *
 ...
 *     });
 *
 * Here a bunch of arbitrary parameters is p***REMOVED***ed along with the load request and a callback function is set
 * up to do something after the loading is over.
 *
 * ## Loading Nested Data
 *
 * Applications often need to load sets of ***REMOVED***ociated data - for example a CRM system might load a User and her Orders.
 * Instead of issuing an AJAX request for the User and a series of additional AJAX requests for each Order, we can load
 * a nested dataset and allow the Reader to automatically populate the ***REMOVED***ociated models. Below is a brief example, see
 ...
 * ## Further Reading
 *
 * Stores are backed up by an ecosystem of cl***REMOVED***es that enables their operation. To gain a full understanding of these
 * pieces and how they fit together, see:
 *
 *   - {@link Ext.data.proxy.Proxy Proxy} - overview of what Proxies are and how they are used
 *   - {@link Ext.data.Model Model} - the core cl***REMOVED*** in the data package
 *   - {@link Ext.data.reader.Reader Reader} - used by any subcl***REMOVED*** of {@link Ext.data.proxy.Server ServerProxy} to read a response
 *
 * @author Ed Spencer
 */
Ext.define('Ext.data.Store', {
    ...
    // Required cl***REMOVED***es must be loaded before the definition callback runs
    // The cl***REMOVED*** definition callback creates a dummy Store which requires that
    // all the cl***REMOVED***es below have been loaded.
    requires: [
    ...
    onCl***REMOVED***Extended: function(cls, data, hooks) {
        var model = data.model,
            onBeforeCl***REMOVED***Created;

        if (typeof model == 'string') {
            onBeforeCl***REMOVED***Created = hooks.onBeforeCreated;
            ...
            hooks.onBeforeCreated = function() {
                ...
                Ext.require(model, function() {
                    onBeforeCl***REMOVED***Created.apply(me, args);
                });
            };
        }
    },
    ...
    constructor: function(config) {
         ...
         * @param {Ext.data.Operation} operation The ***REMOVED***ociated operation
         */
        /**
         * @event groupchange
    ...
    decodeGroupers: function(groupers) {
        ...
        for (i = 0; i < length; i++) {
            ...
            if (!(config instanceof Grouper)) {
                ...
                //support a function to be p***REMOVED***ed as a sorter definition
                ...
                // return resulting Groupers in a separate array so as not to mutate p***REMOVED***ed in data objects.
     ...
     *
     * @param {String} [groupName] P***REMOVED*** in an optional groupName argument to access a specific
     ...
     *
     * @param {Number} index The start index at which to insert the p***REMOVED***ed Records.
    ...
    insert: function(index, records) {
        ...
        for (i = 0,len = records.length; i < len; i++) {
            ...
            // re***REMOVED***ign the model in the array in case it wasn't created yet
            records[i] = record;
    ...
    add: function(records) {
        ...
        // If this Store is sorted, and they only p***REMOVED***ed one Record (99% or use cases)
    ...
    /**
     * (Local sort only) Inserts the p***REMOVED***ed Record into the Store at the index where it
     ...
     *
     * @param {Function} fn The function to call. The {@link Ext.data.Model Record} is p***REMOVED***ed as the first parameter.
    ...
    remove: function(records, /* private */ isMove) {
        ...
        /*
         * P***REMOVED*** the isMove parameter if we know we're going to be re-inserting this record
         */
        isMove = isMove === true;
        var me = this,
            sync = false,
     ...
     *     });
     *
     * If the callback scope does not need to be set, a function can simply be p***REMOVED***ed:
     *
     *     store.load(function(records, operation, success) {
     *         console.log('loaded records');
     *     });
     *
     * @param {Object/Function} [options] config object, p***REMOVED***ed into the Ext.data.Operation object before loading.
    ...
    reload: function(options) {
        ...
        if (me.buffered) {
            ...
            // Calculate a page range which encomp***REMOVED***es the Store's loaded range plus both buffer zones
    ...
    onProxyLoad: function(operation) {
        ...
        //this is a callback that would have been p***REMOVED***ed to the 'read' function and is optional
     ...
     *
     * By default, the p***REMOVED***ed filter(s) are *added* to the collection of filters being used to filter this Store.
     *
     ...
     * Alternatively, p***REMOVED*** in a property string
     ...
     * {@link #getById}.
     *
     * @param {Function} fn The function to be called. It will be p***REMOVED***ed the following parameters:
     *  @param {Ext.data.Model} fn.record The record to test for filtering. Access field values
     *  using {@link Ext.data.Model#get}.
     *  @param {Object} fn.id The ID of the Record p***REMOVED***ed.
     * @param {Object} [scope] The scope (this reference) in which the function is executed.
     ...
     * @param {Boolean} [options.addRecords=false] P***REMOVED*** `true` to add these records to the existing records, `false` to remove the Store's existing records first.
     ...
     * load operation, p***REMOVED***ing in calculated 'start' and 'limit' params
    ...
    loadPage: function(page, options) {
        ...
        // Copy options into a new object so as not to mutate p***REMOVED***ed in objects
        options = Ext.apply({
            page: page,
    ...
    loadToPrefetch: function(options) {
        var me = this,
            ...
            // Calculate a page range which encomp***REMOVED***es the requested range plus both buffer zones
     ...
     * @param {Object} options (Optional) config object, p***REMOVED***ed into the Ext.data.Operation object before loading.
     ...
     * @param {Ext.data.Operation} The ***REMOVED***ociated operation
     */
    ...
    /**
     * Determines if the p***REMOVED***ed range is available in the page cache.
     * @private
    ...
    /**
     * Determines if the p***REMOVED***ed page is available in the page cache.
     * @private
    ...
    guaranteeRange: function(start, end, cb, scope) {
        ...
        if (me.rangeCached(start, end)) {
            ...
            // Attempt to keep the page cache primed with pages which encomp***REMOVED*** the live data range
        ...
        // At least some of the requested range needs loading from server
            ...
            // Private event used by the LoadMask cl***REMOVED*** to perform masking when the range required for rendering is not found in the cache
            ...
            pageAddHandler = function(page, records) {
                if (me.rangeCached(options.prefetchStart, options.prefetchEnd)) {
                    // Private event used by the LoadMask cl***REMOVED*** to unmask when the range required for rendering has been loaded into the cache
     ...
     * @param {Boolean} [allowNull] P***REMOVED*** true to allow null, undefined or empty string values
     * @param {Boolean} [byp***REMOVED***Filter] P***REMOVED*** true to collect from all records, even ones which are filtered.
     * @return {Object[]} An array of the unique values
     */
    collect: function(dataIndex, allowNull, byp***REMOVED***Filter) {
        var me = this,
            data = (byp***REMOVED***Filter === true && me.snapshot) ? me.snapshot : me.data;
     ...
     * @return {Ext.data.Model} The Record at the p***REMOVED***ed index. Returns undefined if not found.
     */
     ...
     * @return {Ext.data.Model} The Record with the p***REMOVED***ed id. Returns null if not found.
     */
    getById: function(id) {
     ...
     * @return {Number} The index of the p***REMOVED***ed Record. Returns -1 if not found.
     */
    ...
    /**
     * Get the index within the store of the Record with the p***REMOVED***ed id.
     *
     * Like #indexOf, this method is effected by filtering.
     *
    ...
    /**
     * @cl***REMOVED*** Ext.data.Store.PageMap
     * @extends Ext.util.LruCache
     * Private cl***REMOVED*** for use by only Store when configured `buffered: true`.
     * @private
     */
    this.prototype.PageMap = new Ext.Cl***REMOVED***({
