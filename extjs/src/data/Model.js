 ...
 * # Associations
 *
 * Models can have ***REMOVED***ociations with other Models via {@link Ext.data.***REMOVED***ociation.HasOne},
 * {@link Ext.data.***REMOVED***ociation.BelongsTo belongsTo} and {@link Ext.data.***REMOVED***ociation.HasMany hasMany} ***REMOVED***ociations.
 ...
 *     });
 *
 * See the docs for {@link Ext.data.***REMOVED***ociation.HasOne}, {@link Ext.data.***REMOVED***ociation.BelongsTo} and
 * {@link Ext.data.***REMOVED***ociation.HasMany} for details on the usage and configuration of ***REMOVED***ociations.
 * Note that ***REMOVED***ociations can also be specified like this:
 *
 *     Ext.define('User', {
 *         extend: 'Ext.data.Model',
 *         fields: ['id'],
 ...
 *
 *     //get a reference to the User model cl***REMOVED***
...
Ext.define('Ext.data.Model', {
    alternateCl***REMOVED***Name: 'Ext.data.Record',
    ...
    onCl***REMOVED***Extended: function(cls, data, hooks) {
        var onBeforeCl***REMOVED***Created = hooks.onBeforeCreated;

        hooks.onBeforeCreated = function(cls, data) {
            var me = this,
                name = Ext.getCl***REMOVED***Name(cls),
                prototype = cls.prototype,
                superCls = cls.prototype.supercl***REMOVED***,

                validations = data.validations || [],
                fields = data.fields || [],
                field,
                ***REMOVED***ociationsConfigs = data.***REMOVED***ociations || [],
                addAssociations = function(items, type) {
                    ...
                    if (items) {
                        ...
                        for (len = items.length; i < len; ++i) {
                            ...
                            item.type = type;
                            ***REMOVED***ociationsConfigs.push(item);
                        }
                    }
                },
                idgen = data.idgen,
                ...
                ***REMOVED***ociationsMixedCollection = new Ext.util.MixedCollection(false, prototype.itemNameFn),

                superValidations = superCls.validations,
                superFields = superCls.fields,
                superAssociations = superCls.***REMOVED***ociations,

                ***REMOVED***ociationConfig, i, ln,
                dependencies = [],
                ...
                // Use the proxy from the cl***REMOVED*** definition object if present, otherwise fall back to the inherited one, or the default    
            ...
            // Save modelName on cl***REMOVED*** and its prototype
            cls.modelName = name;
            prototype.modelName = name;

            // Merge the validations of the supercl***REMOVED*** and the new subcl***REMOVED***
            if (superValidations) {
            ...
            // Merge the fields of the supercl***REMOVED*** and the new subcl***REMOVED***
            if (superFields) {
            ...
            //***REMOVED***ociations can be specified in the more convenient format (e.g. not inside an '***REMOVED***ociations' array).
            ...
            if (superAssociations) {
                ***REMOVED***ociationsConfigs = superAssociations.items.concat(***REMOVED***ociationsConfigs);
            }

            for (i = 0, ln = ***REMOVED***ociationsConfigs.length; i < ln; ++i) {
                dependencies.push('***REMOVED***ociation.' + ***REMOVED***ociationsConfigs[i].type.toLowerCase());
            }
            ...
            Ext.require(dependencies, function() {
                ...
                for (i = 0, ln = ***REMOVED***ociationsConfigs.length; i < ln; ++i) {
                    ***REMOVED***ociationConfig = ***REMOVED***ociationsConfigs[i];
                    if (***REMOVED***ociationConfig.isAssociation) {
                        ***REMOVED***ociationConfig = Ext.applyIf({
                            ownerModel: name,
                            ***REMOVED***ociatedModel: ***REMOVED***ociationConfig.model
                        }, ***REMOVED***ociationConfig.initialConfig);
                    } else {
                        Ext.apply(***REMOVED***ociationConfig, {
                            ownerModel: name,
                            ***REMOVED***ociatedModel: ***REMOVED***ociationConfig.model
                        });
                    }

                    if (Ext.ModelManager.getModel(***REMOVED***ociationConfig.model) === undefined) {
                        Ext.ModelManager.registerDeferredAssociation(***REMOVED***ociationConfig);
                    } else {
                        ***REMOVED***ociationsMixedCollection.add(Ext.data.***REMOVED***ociation.Association.create(***REMOVED***ociationConfig));
                    }
                }

                data.***REMOVED***ociations = ***REMOVED***ociationsMixedCollection;

                // onBeforeCreated may get called *asynchronously* if any of those required cl***REMOVED***es caused
                // an asynchronous script load. This would mean that the cl***REMOVED*** definition object
                ...
                onBeforeCl***REMOVED***Created.call(me, cls, data, hooks);
    ...
    inheritableStatics: {
         ...
         * fields, including fields inherited from supercl***REMOVED***es. Mainly for reconfiguring the
         ...
         *
         * Fields are sorted upon Model cl***REMOVED*** definition. Fields with custom {@link Ext.data.Field#convert convert} functions
    ...
    statics: {
         ...
         * create}d and {@link #constructor no id has been specified}. The id will automatically be ***REMOVED***igned to the
     ...
     * `true` in this cl***REMOVED*** to identify an object as an instantiated Model, or subcl***REMOVED*** thereof.
     */
    isModel: true,
     ...
     * A {@link Ext.util.MixedCollection Collection} of the fields defined for this Model (including fields defined in supercl***REMOVED***es)
     *
    ...
    /**
     * @cfg {Object[]} ***REMOVED***ociations
     * An array of {@link Ext.data.Association ***REMOVED***ociations} for this model.
     */
    /**
     * @cfg {String/Object/String[]/Object[]} hasMany
     * One or more {@link Ext.data.HasManyAssociation HasMany ***REMOVED***ociations} for this model.
     */
    /**
     * @cfg {String/Object/String[]/Object[]} belongsTo
     * One or more {@link Ext.data.BelongsToAssociation BelongsTo ***REMOVED***ociations} for this model.
     */
    /**
     ...
     * @param {Object} data An object containing keys corresponding to this model's fields, and their ***REMOVED***ociated values
     */
        ...
        if (!convertedData) {
            ...
            if (Ext.isArray(data)) {
                for (; i < length; i++) {
                    ...
                    // Have to map array data so the values get ***REMOVED***igned to the named fields
    ...
    /**
     * @private
     * Copies data from the p***REMOVED***ed record into this record. If the p***REMOVED***ed record is undefined, does nothing.
     *
    ...
    /**
     * Returns true if the p***REMOVED***ed field name has been `{@link #modified}` since the load or last commit.
    ...
    copy : function(newId) {
        ...
        // P***REMOVED*** a copy iof our converted data in to be used as the new record's convertedData
     ...
     * @param {Object} options Options to p***REMOVED*** to the proxy. Config object for {@link Ext.data.Operation}.
     ...
     * a Ext.data.NodeStore, then this method calls the given method on the NodeStore and the ***REMOVED***ociated Ext.data.TreeStore
     ...
     * @param {Boolean} includeAssociated True to also include ***REMOVED***ociated data. Defaults to false.
    ...
    /**
     * Gets all of the data from this Models *loaded* ***REMOVED***ociations. It does this recursively - for example if we have a
     ...
     *     }
     *
     * @return {Object} The nested data set for the Model's loaded ***REMOVED***ociations
     */
     ...
     * all of that Model's *loaded* ***REMOVED***ociations. See {@link #getAssociatedData}
     * @param {Object} seenKeys A hash of all the ***REMOVED***ociations we've already seen
    ...
    prepareAssociatedData: function(seenKeys, depth) {
         ...
         * issues when figuring out which ***REMOVED***ociations we've already processed
         * & at what depths.
         */
        var me               = this,
            ***REMOVED***ociations     = me.***REMOVED***ociations.items,
            ***REMOVED***ociationCount = ***REMOVED***ociations.length,
            ***REMOVED***ociationData  = {},
            ...
            toReadKey        = [],
            toReadIndex      = [],
            ***REMOVED***ociatedStore, ***REMOVED***ociatedRecords, ***REMOVED***ociatedRecord, o, index, result, seenDepth,
            ***REMOVED***ociationId, ***REMOVED***ociatedRecordCount, ***REMOVED***ociation, i, j, type, name;

        for (i = 0; i < ***REMOVED***ociationCount; i++) {
            ***REMOVED***ociation = ***REMOVED***ociations[i];
            ***REMOVED***ociationId = ***REMOVED***ociation.***REMOVED***ociationId;
            
            seenDepth = seenKeys[***REMOVED***ociationId];
            if (seenDepth && seenDepth !== depth) {
                continue;
            }
            seenKeys[***REMOVED***ociationId] = depth;

            type = ***REMOVED***ociation.type;
            name = ***REMOVED***ociation.name;
            if (type == 'hasMany') {
                //this is the hasMany store filled with the ***REMOVED***ociated data
                ***REMOVED***ociatedStore = me[***REMOVED***ociation.storeName];

                //we will use this to contain each ***REMOVED***ociated record's data
                ***REMOVED***ociationData[name] = [];

                //if it's loaded, put it into the ***REMOVED***ociation data
                if (***REMOVED***ociatedStore && ***REMOVED***ociatedStore.getCount() > 0) {
                    ***REMOVED***ociatedRecords = ***REMOVED***ociatedStore.data.items;
                    ***REMOVED***ociatedRecordCount = ***REMOVED***ociatedRecords.length;

                    //now we're finally iterating over the records in the ***REMOVED***ociation. Get
                    // all the records so we can process them
                    for (j = 0; j < ***REMOVED***ociatedRecordCount; j++) {
                        ***REMOVED***ociatedRecord = ***REMOVED***ociatedRecords[j];
                        ***REMOVED***ociationData[name][j] = ***REMOVED***ociatedRecord.getData();
                        toRead.push(***REMOVED***ociatedRecord);
                        toReadKey.push(name);
                        toReadIndex.push(j);
                    }
                }
            } else if (type == 'belongsTo' || type == 'hasOne') {
                ***REMOVED***ociatedRecord = me[***REMOVED***ociation.instanceName];
                // If we have a record, put it onto our list
                if (***REMOVED***ociatedRecord !== undefined) {
                    ***REMOVED***ociationData[name] = ***REMOVED***ociatedRecord.getData();
                    toRead.push(***REMOVED***ociatedRecord);
                    toReadKey.push(name);
                    toReadIndex.push(-1);
                }
        ...
        for (i = 0, ***REMOVED***ociatedRecordCount = toRead.length; i < ***REMOVED***ociatedRecordCount; ++i) {
            ***REMOVED***ociatedRecord = toRead[i];
            o = ***REMOVED***ociationData[toReadKey[i]];
            index = toReadIndex[i];
            result = ***REMOVED***ociatedRecord.prepareAssociatedData(seenKeys, depth + 1);
            if (index === -1) {
                Ext.apply(o, result);
            } else {
