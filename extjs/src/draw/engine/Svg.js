...
Ext.define('Ext.draw.engine.Svg', {
    ...
    applyAttrs: function (sprite) {
        ...
        for (key in attrs) {
            if (attrs.hasOwnProperty(key) && attrs[key] != null) {
                ...
                //attribute is a reference to a gradient then ***REMOVED***ign a plain color to that value instead of the gradient.
     ...
     */
    hasCls: function(sprite, cl***REMOVED***Name) {
        return cl***REMOVED***Name && (' ' + (sprite.el.dom.getAttribute('cl***REMOVED***') || '') + ' ').indexOf(' ' + cl***REMOVED***Name + ' ') != -1;
    },

    addCls: function(sprite, cl***REMOVED***Name) {
        var el = sprite.el,
            i,
            len,
            v,
            cls = [],
            curCls =  el.getAttribute('cl***REMOVED***') || '';
        // Separate case is for speed
        if (!Ext.isArray(cl***REMOVED***Name)) {
            if (typeof cl***REMOVED***Name == 'string' && !this.hasCls(sprite, cl***REMOVED***Name)) {
                el.set({ 'cl***REMOVED***': curCls + ' ' + cl***REMOVED***Name });
            }
        }
        else {
            for (i = 0, len = cl***REMOVED***Name.length; i < len; i++) {
                v = cl***REMOVED***Name[i];
            ...
            if (cls.length) {
                el.set({ 'cl***REMOVED***': ' ' + cls.join(' ') });
            }
        }
    },

    removeCls: function(sprite, cl***REMOVED***Name) {
        var me = this,
            el = sprite.el,
            curCls =  el.getAttribute('cl***REMOVED***') || '',
            i, idx, len, cls, elCl***REMOVED***es;
        if (!Ext.isArray(cl***REMOVED***Name)){
            cl***REMOVED***Name = [cl***REMOVED***Name];
        }
        if (curCls) {
            elCl***REMOVED***es = curCls.replace(me.trimRe, ' ').split(me.spacesRe);
            for (i = 0, len = cl***REMOVED***Name.length; i < len; i++) {
                cls = cl***REMOVED***Name[i];
                if (typeof cls == 'string') {
                    cls = cls.replace(me.trimRe, '');
                    idx = Ext.Array.indexOf(elCl***REMOVED***es, cls);
                    if (idx != -1) {
                        Ext.Array.erase(elCl***REMOVED***es, idx, 1);
                    }
                }
            }
            el.set({ 'cl***REMOVED***': elCl***REMOVED***es.join(' ') });
        }
    },
