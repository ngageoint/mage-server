...
Ext.define('Ext.ProgressBar', {
    ...
    * The base CSS cl***REMOVED*** to apply to the progress bar's wrapper element.
    */
    ...
    renderTpl: [
        '<tpl if="internalText">',
            '<div cl***REMOVED***="{baseCls}-text {baseCls}-text-back">{text}</div>',
        '</tpl>',
        '<div id="{id}-bar" cl***REMOVED***="{baseCls}-bar" style="width:{percentage}%">',
            '<tpl if="internalText">',
                '<div cl***REMOVED***="{baseCls}-text">',
                    '<div>{text}</div>',
                '</div>',
            '</tpl>',
        '</div>'
     ...
     * value will be unchanged. To blank out existing text, p***REMOVED*** ''. Note that even if the progress bar value exceeds 1,
     ...
     * specified, the default for the cl***REMOVED*** is used
     ...
     * duration is p***REMOVED***ed in, then the progress bar will run indefinitely and must be manually cleared by calling
     * {@link #reset}.
     *
     * Example usage:
     *
     ...
     * value is not specified, the default for the cl***REMOVED*** is used.
     ...
     * @param {Object} config.scope The scope that is p***REMOVED***ed to the callback function (only applies when
     * duration and fn are both p***REMOVED***ed).
     * @return {Ext.ProgressBar} this
     */
    wait: function(o) {
        var me = this, scope;
