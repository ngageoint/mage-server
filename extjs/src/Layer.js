...
Ext.define('Ext.Layer', {
    ...
    /**
     * @cfg {String} cls
     * CSS cl***REMOVED*** to add to the element
     */
     ...
     * The CSS cl***REMOVED*** name to add in order to hide this Layer if this layer
     * is configured with <code>{@link #hideMode}: 'ascl***REMOVED***'</code>
     */
    ...
    constructor: function(config, existingEl) {
        ...
        // Allow Components to p***REMOVED*** their hide mode down to the Layer if they are floating.
        // Otherwise, allow useDisplay to override the default hiding method which is visibility.
        // TODO: Have ExtJS's Element implement visibilityMode by using cl***REMOVED***es as in Mobile.
        if (hm) {
    ...
    /**
     * @private
     * Synchronize this Layer's ***REMOVED***ociated elements, the shadow, and possibly the shim.
     *
     ...
     *
     * @param {Boolean} doShow P***REMOVED*** true to ensure that the shadow is shown.
     */
    ...
    constrainXY: function() {
        if (this.constrain) {
            ...
            if (moved) {
                Ext.Layer.supercl***REMOVED***.setXY.call(this, [x, y]);
                this.sync();
            }
        }
        return this;
    ...
    setXY: function(xy, animate, duration, callback, easing) {
        ...
        // Callback will restore shadow state and call the p***REMOVED***ed callback
    ...
    setBounds: function(x, y, width, height, animate, duration, callback, easing) {
        ...
        me.beforeAction();
        if (!animate) {
            Ext.Layer.supercl***REMOVED***.setXY.call(me, [x, y]);
            Ext.Layer.supercl***REMOVED***.setSize.call(me, width, height);
            callback();
        } else {
     ...
     * shadow in so that it always shows above those two ***REMOVED***ociated elements.
     *
     * Any shim, will be ***REMOVED***igned the p***REMOVED***ed z-index. A shadow will be ***REMOVED***igned the next
