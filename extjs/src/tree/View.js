...
Ext.define('Ext.tree.View', {
    ...
    // Cl***REMOVED*** to add to the node wrap element used to hold nodes when a parent is being
    // collapsed or expanded. During the animation, UI interaction is forbidden by testing
    // for an ancestor node with this cl***REMOVED***.
    ...
    processUIEvent: function(e) {
        ...
        // This is because during a collapse animation, the ***REMOVED***ociated Records
        ...
        tmpEl = nodeEl.insertSibling({
            ...
            html: [
                '<td colspan="' + headerCt.getColumnCount() + '">',
                    '<div cl***REMOVED***="' + this.nodeAnimWrapCls + '">',
                        '<table cl***REMOVED***="' + Ext.baseCSSPrefix + 'grid-table" style="width: ' + headerCt.getFullWidth() + 'px;"><tbody>',
                            thHtml,
                        '</tbody></table>',
                    '</div>',
                '</td>'
     ...
     * 
     * @param [bubble=true] If the p***REMOVED***ed parent node does not already have a wrap element created, by default
     ...
     * always p***REMOVED*** `bubble: false` to be on the safe side.
     * 
     * If the p***REMOVED***ed parent has no wrap (or there is no valid ancestor wrap after bubbling), this function 
