/**
 * An abstract base cl***REMOVED*** which provides shared methods for Components across the Sencha product line.
 *
 * Please refer to sub cl***REMOVED***'s documentation
 * @private
 */
Ext.define('Ext.AbstractComponent', {
    ...
    // The "uses" property specifies cl***REMOVED*** which are used in an instantiated AbstractComponent.
    // They do *not* have to be loaded before this cl***REMOVED*** may be defined - that is what "requires" is for.
    uses: [
        'Ext.PluginManager',
        'Ext.Element',
        'Ext.DomHelper',
     ...
     * `true` in this cl***REMOVED*** to identify an object as an instantiated Component, or subcl***REMOVED*** thereof.
     */
    isComponent: true,
     ...
     *
     * Instead of using ***REMOVED***igned ids, use the {@link #itemId} config, and {@link Ext.ComponentQuery ComponentQuery}
     * which provides selector-based searching for Sencha Components analogous to DOM querying. The {@link
     * Ext.container.Container Container} cl***REMOVED*** contains {@link Ext.container.Container#down shortcut methods} to query
     ...
     *
     * Defaults to an {@link #getId auto-***REMOVED***igned id}.
     */
     ...
     *
     * You do not normally need to specify this. For the base cl***REMOVED***es {@link Ext.Component} and
     * {@link Ext.container.Container}, this defaults to **'div'**. The more complex Sencha cl***REMOVED***es use a more
     ...
     * cl***REMOVED***es which use a more complex DOM structure, provide their own template definitions.
     *
     ...
     * cl***REMOVED*** which simply sizes the Component's encapsulating element to the height and width specified in the
     * {@link #setSize} method.
     */
     ...
     * cl***REMOVED*** name. Using `xtype` is the most common way to define component instances,
     ...
     * The base CSS cl***REMOVED*** to apply to this components's element. This will also be prepended to elements within this
     * component like Panel's body will get a cl***REMOVED*** x-panel-body. This means that if you create a subcl***REMOVED*** of Panel, and
     ...
     * CSS Cl***REMOVED*** to be added to a components root level element to give distinction to it via styling.
     */

    /**
     * @cfg {String} [cls='']
     * An optional extra CSS cl***REMOVED*** that will be added to this component's Element. This can be useful
     * for adding customized styles to the component or any of its children using standard CSS rules.
     */

    /**
     * @cfg {String} [overCls='']
     * An optional extra CSS cl***REMOVED*** that will be added to this component's Element when the mouse moves over the Element,
     ...
     * CSS cl***REMOVED*** to add when the Component is disabled. Defaults to 'x-item-disabled'.
     */
    ...
    /**
     * @cfg {String[]} uiCls
     * An array of of cl***REMOVED***Names which are currently applied to this component
     * @private
     */
    uiCls: [],
     ...
     * **`{@link Ext.container.Container#cfg-items items}`**.
     *
     * Add either the `x-hidden` or the `x-hide-display` CSS cl***REMOVED*** to prevent a brief flicker of the content before it
     ...
     * The cl***REMOVED*** that is added to the content target when you set styleHtmlContent to true.
     */
     ...
     * is created, if any plugins are available, the component will call the init method on each plugin, p***REMOVED***ing a
    ...
    constructor : function(config) {
        ...
        // so we must apply styles and cl***REMOVED***es here too.
        if (me.cls) {
        ...
        // Dont p***REMOVED*** the config so that it is not applied to 'this' again
        ...
        //<debug>
        if (Ext.isDefined(me.disabledCl***REMOVED***)) {
            if (Ext.isDefined(Ext.global.console)) {
                Ext.global.console.warn('Ext.Component: disabledCl***REMOVED*** has been deprecated. Please use disabledCls.');
            }
            me.disabledCls = me.disabledCl***REMOVED***;
            delete me.disabledCl***REMOVED***;
        }
        //</debug>
    },

    initComponent: function () {
        // This is called again here to allow derived cl***REMOVED***es to add plugin configs to the
    ...
    /**
     * The supplied default state gathering method for the AbstractComponent cl***REMOVED***.
     *
     * This method returns dimension settings such as `flex`, `anchor`, `width` and `height` along with `collapsed`
     * state.
     *
     * Subcl***REMOVED***es which implement more complex state should call the supercl***REMOVED***'s implementation, and apply their state
    ...
    setupProtoEl: function() {
        ...
        me.protoEl = new Ext.util.ProtoElement({
            cls: cls.join(' ') // in case any of the parts have multiple cl***REMOVED***es
        });
    },
    ...
    setUI: function(ui) {
        ...
        for (i = 0; i < oldUICls.length; i++) {
            ...
            cl***REMOVED***es = cl***REMOVED***es.concat(me.removeClsWithUI(cls, true));
            newUICls.push(cls);
        }

        if (cl***REMOVED***es.length) {
            me.removeCls(cl***REMOVED***es);
        }
        ...
        for (i = 0; i < newUICls.length; i++) {
            cls = newUICls[i];
            cl***REMOVED***es = cl***REMOVED***es.concat(me.addClsWithUI(cls, true));
        }

        if (cl***REMOVED***es.length) {
            me.addCls(cl***REMOVED***es);
        }
     ...
     * component.
     * @param {String/String[]} cl***REMOVED***es A string or an array of strings to add to the uiCls
     * @param {Object} skip (Boolean) skip True to skip adding it to the cl***REMOVED*** and do it later (via the return)
     */
    addClsWithUI: function(cl***REMOVED***es, skip) {
        var me = this,
            clsArray = [],
            length,
            i = 0,
        ...
        if (typeof cl***REMOVED***es === "string") {
            cl***REMOVED***es = (cl***REMOVED***es.indexOf(' ') < 0) ? [cl***REMOVED***es] : Ext.String.splitWords(cl***REMOVED***es);
        }

        length = cl***REMOVED***es.length;
     ...
     */
    removeClsWithUI: function(cl***REMOVED***es, skip) {
        var me = this,
            clsArray = [],
            i = 0,
            length, cls;
    ...
    initStyles: function(targetEl) {
        ...
        if (!me.getFrameInfo()) {
            ...
            // If we're using the content box model, we also cannot ***REMOVED***ign numeric initial sizes since we do not know the border widths to subtract
            if (width !== undefined) {
    ...
    /**
     * @private
     * <p>Returns the focus holder element ***REMOVED***ociated with this Component. At the Component base cl***REMOVED*** level, this function returns <code>undefined</code>.</p>
     * <p>Subcl***REMOVED***es which use embedded focusable elements (such as Window, Field and Button) should override this for use by the {@link #focus} method.</p>
    ...
    /**
     * Walks up the `ownerCt` axis looking for an ancestor Container which matches the p***REMOVED***ed simple selector.
     *
     * Example:
     *
     ...
     *
     * Optionally selects the next sibling which matches the p***REMOVED***ed {@link Ext.ComponentQuery ComponentQuery} selector.
     *
     ...
     *
     * Optionally selects the previous sibling which matches the p***REMOVED***ed {@link Ext.ComponentQuery ComponentQuery}
     * selector.
     *
    ...
    /**
     * Returns the value of {@link #itemId} ***REMOVED***igned to this component, or when that
     ...
     *
     * **If using your own subcl***REMOVED***es, be aware that a Component must register its own xtype to participate in
     ...
     *     var isBoxSubcl***REMOVED*** = t.isXType('field');       // true, descended from Ext.form.field.Base
    ...
    getXTypes: function() {
        ...
        if (!self.xtypes) {
            ...
            while (parentPrototype) {
                ...
                parentPrototype = parentPrototype.supercl***REMOVED***;
            }
     ...
     *
     * @param {Boolean} [deep=false] P***REMOVED*** `true` to interrogate the visibility status of all parent Containers to
     ...
     * @param {Boolean} [silent=false] P***REMOVED***ing true will supress the 'enable' event from being fired.
     */
     ...
     * @param {Boolean} [silent=false] P***REMOVED***ing true will supress the 'disable' event from being fired.
     */
     ...
     * After calling the supercl***REMOVED***’s onEnable, the Component will be enabled.
     *
     * @template
     * @protected
     */
     ...
     * After calling the supercl***REMOVED***’s onDisable, the Component will be disabled.
     *
     * @template
     * @protected
     */
    ...
    mask: function() {
        ...
        // P***REMOVED*** it the height of our element if we know it.
        if (box) {
            args[2] = box.height;
        }
    ...
    /**
     * Adds a CSS cl***REMOVED*** to the top level element representing this component.
     * @param {String/String[]} cls The CSS cl***REMOVED*** name to add
    ...
    /**
     * Removes a CSS cl***REMOVED*** from the top level element representing this component.
     * @param {String/String[]} cls The CSS cl***REMOVED*** name to remove
    ...
    //<debug>
    removeCl***REMOVED*** : function() {
        if (Ext.isDefined(Ext.global.console)) {
            Ext.global.console.warn('Ext.Component: removeCl***REMOVED*** has been deprecated. Please use removeCls.');
        }
    ...
    addListener : function(element, listeners, scope, options) {
        ...
        if (Ext.isString(element) && (Ext.isObject(listeners) || options && options.element)) {
            ...
            // and a listeners object that can be p***REMOVED***ed to on
     ...
     * supercl***REMOVED***'s onAdded, the ownerCt reference will be present,
     ...
     * p***REMOVED***ed a truthy second parameter). After calling the
     * supercl***REMOVED***'s onRemoved, the ownerCt and the refOwner will not
     * be present.
     * @param {Boolean} destroying Will be p***REMOVED***ed as true if the Container performing the remove operation will delete this
     ...
     * either width and height as separate arguments, or you can p***REMOVED*** a size object like `{width:10, height:20}`.
     *
     ...
     *
     * @param {Number/String} height The new height to set (not required if a size object is p***REMOVED***ed as the first arg).
     * This may be one of:
     *
     ...
     * this component can run its layout without ***REMOVED***istance from or impact on its owner.
     ...
     * @param {Boolean/Object} [animate] If true, the Component is _animated_ into its new position. You may also p***REMOVED*** an
     ...
     * subcl***REMOVED*** that needs to perform custom logic after a move occurs.
     *
     * @param {Number} x The new x position
     * @param {Number} y The new y position
     *
    ...
    /**
     *
     * @param {String/Number} border The border, see {@link #border}. If a falsey value is p***REMOVED***ed
        ...
        if (!me.isDestroyed) {
            if (!me.hasListeners.beforedestroy || me.fireEvent('beforedestroy', me) !== false) {
                ...
                if (me.floating) {
                    ...
                    // If it has no zIndexManager at render time, it is ***REMOVED***igned to the global Ext.WindowManager instance.
                    if (me.zIndexManager) {
