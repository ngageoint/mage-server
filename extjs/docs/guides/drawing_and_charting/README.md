...
- Section II: "Chart" A high level presentation of the charting package and how cl***REMOVED***es are
organized in it.
...
The Draw package contains a Surface cl***REMOVED*** that abstracts the underlying graphics implementation
...
The underlying/concrete implementations for the Surface cl***REMOVED*** are SVG (for SVG capable browsers) and
...
most interesting about this cl***REMOVED*** is that we aren't tied to a specific shape or structure; also all elements
...
package consists of a hierarchy of cl***REMOVED***es that define a chart container (something like a surface but more specific for
...
In this section we will cover how these cl***REMOVED***es are tied together and what bits of functionality go into each of these
cl***REMOVED***es. We won't cover each particular series, since that is done in the next section.

### Chart

The Chart cl***REMOVED*** is the main drawing surface for series. It manages the rendering of each series and also how axes are
drawn and defined. Chart also delegates mouse events over to different areas of the Chart like Series, Axes, etc.
The Chart cl***REMOVED*** extends Draw Component.
...
to set the position of the legend. These options are p***REMOVED***ed into the constructor of the chart. For example:
...
The `axis` package contains an `Abstract` axis cl***REMOVED*** that is extended by `Axis` and `Radial` axes. `Axis` represents
...
A `Series` is an abstract cl***REMOVED*** extended by concrete visualizations like
`Line` or `Scatter`. The `Series` cl***REMOVED*** contains code that is common to all of these series, like event handling, animation
handling, shadows, gradients, common offsets, etc. The `Series` cl***REMOVED*** is enhanced with a set of *mixins* that provide functionality
...
positioning of each element, its ***REMOVED***ociated `sprite` and a `storeItem`. The series also share the `drawSeries` method that updates
