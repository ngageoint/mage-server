...
So there should no longer be a `directFn`, it's been replaced by a more robust API. Writing this tells it that it's looking for a cl***REMOVED*** called QueryDatabase and the method name (e.g. createRecord).
...
QueryDatabase.php
	...
	...
	public function updateRecords(stdCl***REMOVED*** $params)
	{
...
QueryDatabase.php
	...
	...
	public function createRecord(stdCl***REMOVED*** $params)
	{
		$_db = $this->__construct();
...
Our function name corresponds to the name we set when declaring our API in grid.js, i.e. createRecord, we then say that 'data that comes from the cl***REMOVED*** stdCl***REMOVED*** (from router.php) will be ***REMOVED***igned to a variable called params, this is for added security so attacks can't be spoofed from another file. The data in question looks like this:

{@img creating-record.png Firebug showing the JSON that contains the data to create a record}

This clearly shows us which cl***REMOVED*** and method is being invoked and includes a JSON data object that we access to get the individual fields for our database. The `tid` is the number of POST requests in this session, so this is the second (the first being when it loaded the data).
...
Noticed that the buttons are looking a bit bare? To add an icon we're going to write some old school CSS. The cl***REMOVED***es aren't arbitrary, when coding our buttons we added an `iconCls` to each one, this adds the cl***REMOVED*** name that we're now using in the CSS. I've also positioned the grid so that it's no longer tucked up in the top left corner.
