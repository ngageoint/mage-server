Mapping a Grid to a MySQL table using Direct and PHP
==================================================================
...
Because we're using the HTML5 document type we're allowed to omit the type in a script tag, it ***REMOVED***umes that all `<script>` tags will be JavaScript which helps cut down our bytes. However, you've probably also noticed the peculiar api.php file, surely that can't be JavaScript? All will be explained in time.
...
The asterisk ('`*`') in this context loads all of the cl***REMOVED***es within those areas of Ext JS, we could optimize it at the end by only requiring the cl***REMOVED***es that we use. We then want to make a pretty grid to look at, but first, a slight digression.
...
In the root directory of our app, create a folder called 'php' followed by another one inside it called 'cl***REMOVED***es'. Within cl***REMOVED***es create a file called 'QueryDatabase.php'.
...
First, we'll make a new cl***REMOVED*** and declare some variables:

QueryDatabase.php

	<?php
	cl***REMOVED*** QueryDatabase
	{
		private $_db;
		protected $_result;
		public $results;
...
_Within_ this cl***REMOVED***, we want to make a function that will connect to the database, (note that you don't write the ..., it's to denote that this block of code continues on from the last one).
...
QueryDatabase.php
	...
	{
		$_db = new mysqli('host', 'username' ,'p***REMOVED***word', 'database');
...
Notice in the parenthesis we've put `$_db`? This means that this function is going to expect a parameter p***REMOVED***ed to it, i.e. it's expecting `$_db` otherwise it'll have nothing to close!
...
QueryDatabase.php
	...
	public function getResults($params)
	{
		...
		while ($row = $_result->fetch_***REMOVED***oc()) {
...
To make sure the correct methods are called, we need a router. The router is where the calls from Ext Direct get routed to the correct cl***REMOVED*** using a Remote Procedure Call (RPC).
...
Here, we've declared a cl***REMOVED*** and required our config file that contains which methods we expose in our API.
...
router.php
	...
	function doRpc($cdata){
		...
		try {
			...
			require_once("cl***REMOVED***es/$action.php");
			$o = new $action();
	        if (isset($mdef['len'])) {
