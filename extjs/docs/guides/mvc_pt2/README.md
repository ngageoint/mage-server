...
Models in Ext JS 4 are very similar to Records which we had in Ext JS 3. One key difference is that you can now specify a proxy on your model, as well as validations and ***REMOVED***ociations. The Song model for our application in Ext JS 4 would look like this.
...
We simply get the single selected item provided by the selectionchange event and p***REMOVED*** it as the single argument when firing the stationstart event.
