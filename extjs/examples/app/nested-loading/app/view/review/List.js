    ...
    initComponent: function() {
        this.dataview = Ext.create('Ext.view.View', {
            ...
            tpl: new Ext.XTemplate(
                '<tpl for=".">',
                    '<div cl***REMOVED***="review {[xindex === 1 ? "first-review" : ""]}">',
                        '<div cl***REMOVED***="title">{title} {[this.stars(values)]}</div>',
                        '<div cl***REMOVED***="author">By <span>{author}</span> - {date}</div>',
                        '<div cl***REMOVED***="comment">{comment}</div>',
                    '</div>',
                '</tpl>',
                {
