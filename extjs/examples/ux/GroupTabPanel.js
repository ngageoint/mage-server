 ...
 * @cl***REMOVED*** Ext.ux.GroupTabPanel
...
Ext.define('Ext.ux.GroupTabPanel', {
    ...
    initComponent: function(config) {
        ...
        me.items = [{
            ...
            viewConfig: {
                overItemCls: '',
                getRowCl***REMOVED***: me.getRowCl***REMOVED***,
    ...
    getRowCl***REMOVED***: function(node, rowIndex, rowParams, store) {
        var cls = '';
...
Ext.define('Ext.ux.GroupTreeChunker', {
    ...
    metaTableTpl: [
        ...
        '<table cl***REMOVED***="' + Ext.baseCSSPrefix + 'grid-table-resizer" border="0" cellspacing="0" cellpadding="0" {[this.embedFullWidth(values)]}><tr><td>',
            '{[this.openRows()]}',
                '{row}',
            '{[this.closeRows()]}',
        '</td></tr><table>',
    ...
    metaRowTpl: [
        '<div cl***REMOVED***="' + Ext.baseCSSPrefix + 'grouptab-row {[this.embedRowCls()]}" {[this.embedRowAttr()]}>',
            '<tpl for="columns">',
                '<div cl***REMOVED***="{cls} ' + Ext.baseCSSPrefix + 'grouptab-cell ' + Ext.baseCSSPrefix + 'grid-cell-{columnId} {{id}-modified} {{id}-tdCls} {[this.firstOrLastCls(xindex, xcount)]}" {{id}-tdAttr}>',
                    '<div {unselectableAttr} cl***REMOVED***="' + Ext.baseCSSPrefix + 'grid-cell-inner {unselectableCls}" style="text-align: {align}; {{id}-style};">{{id}}</div>',
                    '<div cl***REMOVED***="x-grouptabs-corner x-grouptabs-corner-top-left" id="ext-gen25"></div>',
                    '<div cl***REMOVED***="x-grouptabs-corner x-grouptabs-corner-bottom-left" id="ext-gen26"></div>',
                '</div>',
            '</tpl>',
        '</div>'
    ],
