Ext.define('Ext.calendar.template.BoxLayout', {
    ...
    constructor: function(config){
        ...
        var weekLinkTpl = this.showWeekLinks ? '<div id="{weekLinkId}" cl***REMOVED***="ext-cal-week-link">{weekNum}</div>' : '';
        
        this.callParent([
            '<tpl for="weeks">',
                '<div id="{[this.id]}-wk-{[xindex-1]}" cl***REMOVED***="ext-cal-wk-ct" style="top:{[this.getRowTop(xindex, xcount)]}%; height:{[this.getRowHeight(xcount)]}%;">',
                    weekLinkTpl,
                    '<table cl***REMOVED***="ext-cal-bg-tbl" cellpadding="0" cellspacing="0">',
                        '<tbody>',
                            '<tr>',
                                '<tpl for=".">',
                                     '<td id="{[this.id]}-day-{date:date("Ymd")}" cl***REMOVED***="{cellCls}">&#160;</td>',
                                '</tpl>',
                            '</tr>',
                        '</tbody>',
                    '</table>',
                    '<table cl***REMOVED***="ext-cal-evt-tbl" cellpadding="0" cellspacing="0">',
                        '<tbody>',
                            '<tr>',
                                '<tpl for=".">',
                                    '<td id="{[this.id]}-ev-day-{date:date("Ymd")}" cl***REMOVED***="{titleCls}"><div>{title}</div></td>',
                                '</tpl>',
                            '</tr>',
                        '</tbody>',
                    '</table>',
    ...
    getTodayText : function(){
        var dt = Ext.Date.format(new Date(), 'l, F j, Y'),
            ...
            timeText = this.showTime !== false ? ' <span id="'+this.id+'-clock" cl***REMOVED***="ext-cal-dtitle-time">' + 
