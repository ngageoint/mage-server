/**
 * @cl***REMOVED*** Ext.calendar.view.Month
 * @extends Ext.calendar.CalendarView
 * <p>Displays a calendar view by month. This cl***REMOVED*** does not usually need ot be used directly as you can
...
Ext.define('Ext.calendar.view.Month', {
    ...
    /**
     * @cfg {String} weekLinkOverCl***REMOVED***
     * The CSS cl***REMOVED*** name applied when the mouse moves over a week link element (only applies when {@link #showWeekLinks} is true,
     * defaults to 'ext-week-link-over').
     */
    weekLinkOverCl***REMOVED***: 'ext-week-link-over',
    ...
    // private
    forceSize: function() {
        // Compe***REMOVED***te for the week link gutter width if visible
    ...
    getEventTemplate: function() {
        if (!this.eventTpl) {
            ...
            new Ext.XTemplate(
                '<div id="{_elId}" cl***REMOVED***="{_selectorCls} {_colorCls} {spanCls} ext-cal-evt ext-cal-evr">',
                    body,
                '</div>'
            )
            : new Ext.XTemplate(
                '<tpl if="_renderAsAllDay">',
                    '<div id="{_elId}" cl***REMOVED***="{_selectorCls} {spanCls} {_colorCls} ext-cal-evt ext-cal-evo">',
                        '<div cl***REMOVED***="ext-cal-evm">',
                            '<div cl***REMOVED***="ext-cal-evi">',
                '</tpl>',
                '<tpl if="!_renderAsAllDay">',
                    '<div id="{_elId}" cl***REMOVED***="{_selectorCls} {_colorCls} ext-cal-evt ext-cal-evr">',
                '</tpl>',
                    body,
                '<tpl if="_renderAsAllDay">',
                            '</div>',
    ...
    onInitDrag: function() {
        ...
        Ext.select(this.daySelector).removeCls(this.dayOverCl***REMOVED***);
        if (this.detailPanel) {
            this.detailPanel.hide();
        }
    },
    ...
    onClick: function(e, t) {
        ...
        if (Ext.calendar.view.Month.supercl***REMOVED***.onClick.apply(this, arguments)) {
    ...
    handleDayMouseEvent: function(e, t, type) {
        ...
        if (el) {
            el[type == 'over' ? 'addCls': 'removeCls'](this.weekLinkOverCl***REMOVED***);
            return;
        }
        this.callParent(arguments);
    }
