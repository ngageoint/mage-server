/**S
 * @cl***REMOVED*** Ext.calendar.view.DayBody
 ...
 * Normally you should not need to use this cl***REMOVED*** directly -- instead you should use {@link Ext.calendar.DayView DayView}
 * which aggregates this cl***REMOVED*** and the {@link Ext.calendar.DayHeaderView DayHeaderView} into the single unified view
...
Ext.define('Ext.calendar.view.DayBody', {
     ...
     * <p>This method will automatically defer itself for IE and Opera (even if you p***REMOVED*** false) otherwise
     * the scroll position will not update in those browsers. You can optionally p***REMOVED*** true, however, to
    ...
    getEventBodyMarkup: function() {
        if (!this.eventBodyMarkup) {
            ...
            '<tpl if="_isReminder">',
            '<i cl***REMOVED***="ext-cal-ic ext-cal-ic-rem">&#160;</i>',
            '</tpl>',
            '<tpl if="_isRecurring">',
            '<i cl***REMOVED***="ext-cal-ic ext-cal-ic-rcr">&#160;</i>',
            '</tpl>'
            //                '<tpl if="spanLeft">',
            //                    '<i cl***REMOVED***="ext-cal-spl">&#160;</i>',
            //                '</tpl>',
            //                '<tpl if="spanRight">',
            //                    '<i cl***REMOVED***="ext-cal-spr">&#160;</i>',
            //                '</tpl>'
            ].join('');
        }
    ...
    getEventTemplate: function() {
        if (!this.eventTpl) {
            ...
            new Ext.XTemplate(
                '<div id="{_elId}" cl***REMOVED***="{_selectorCls} {_colorCls} ext-cal-evt ext-cal-evr" style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
                '<div cl***REMOVED***="ext-evt-bd">', this.getEventBodyMarkup(), '</div>',
                '<div cl***REMOVED***="ext-evt-rsz"><div cl***REMOVED***="ext-evt-rsz-h">&#160;</div></div>',
                '</div>'
            )
            : new Ext.XTemplate(
                '<div id="{_elId}" cl***REMOVED***="ext-cal-evt {_selectorCls} {_colorCls}-x" style="left: {_left}%; width: {_width}%; top: {_top}px;">',
                '<div cl***REMOVED***="ext-cal-evb">&#160;</div>',
                '<dl style="height: {_height}px;" cl***REMOVED***="ext-cal-evdm">',
                '<dd cl***REMOVED***="ext-evt-bd">',
    ...
    getEventAllDayTemplate: function() {
        if (!this.eventAllDayTpl) {
            ...
            new Ext.XTemplate(
                '<div id="{_elId}" cl***REMOVED***="{_selectorCls} {_colorCls} {spanCls} ext-cal-evt ext-cal-evr" style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
                body,
                '</div>'
            )
            : new Ext.XTemplate(
                '<div id="{_elId}" cl***REMOVED***="ext-cal-evt" style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
                '<div cl***REMOVED***="{_selectorCls} {spanCls} {_colorCls} ext-cal-evo">',
                '<div cl***REMOVED***="ext-cal-evm">',
                '<div cl***REMOVED***="ext-cal-evi">',
                body,
                '</div>',
                '</div>',
                '</div></div>'
    ...
    onClick: function(e, t) {
        if (this.dragPending || Ext.calendar.view.DayBody.supercl***REMOVED***.onClick.apply(this, arguments)) {
            // The supercl***REMOVED*** handled the click already so exit
            return;
        }
