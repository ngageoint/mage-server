<?php
	 ...
	 * ***REMOVED***ociated with that type, used by the generateData function.
	 */
	...
	/**
	 * Returns a hard-coded data value matching the type p***REMOVED***ed in.
	 */
    ...
    function generateData() {
        ...
        for ($i=0; $i<$row_count; $i++) {
			...
			for ($j=0; $j<$col_count; $j++) {
				// first p***REMOVED*** through columns only, define fields and columns
				if ($defineFields) {
				...
				// every row/col p***REMOVED***, load up some data
				$row['field-'.($j+1)] = $j == 0 ? ($i+1) : getDataValue($types[$j]);
			}
			
			// flip this flag after the first column p***REMOVED*** since the fields are defined
			$defineFields = false;
		...
		// ***REMOVED***emble the top-level data object being returned.
