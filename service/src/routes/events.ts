...
function determineReadAccess(req: express.Request, res: express.Response, next: express.NextFunction) {
  if (!access.userHasPermission(req.user, MageEventPermission.READ_EVENT_ALL)) {
    req.access = { user: req.user, permission: EventPermission.Read };
  }
  next();
}
...
function EventRoutes(app: express.Application, security: { authentication: authentication.AuthLayer }) {
  ...
  app.get(
    '/api/events/:eventId',
    passport.authenticate('bearer'),
    middlewareAuthorizeAccess(MageEventPermission.READ_EVENT_ALL, EventPermission.Read),
    determineReadAccess,
    parseEventQueryParams,
    function (req, res, next) {
  ...
  app.put(
    '/api/events/:eventId',
    passport.authenticate('bearer'),
    middlewareAuthorizeAccess(MageEventPermission.UPDATE_EVENT, EventPermission.Update),
    function(req, res, next) {
  ...
  app.delete(
    '/api/events/:eventId',
    passport.authenticate('bearer'),
    middlewareAuthorizeAccess(MageEventPermission.DELETE_EVENT, EventPermission.Delete),
    function(req, res, next) {
