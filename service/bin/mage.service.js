...
const options = [
  ...
  new Option('--log-level <string>', 'The root logging level for MAGE logs, e.g., debug, info, warn').env('MAGE_LOG_LEVEL'),
  new Option('--log-format <string>', `Log output format: 'json' emits one JSON object per line for log collectors; any other value emits the human-readable console format`).env('MAGE_LOG_FORMAT'),
  new Option('--http-debug [boolean]', 'Log verbose HTTP request and response debugging detail').env('MAGE_HTTP_DEBUG'),
  new Option('--http-request-log-methods <string>', 'Comma-separated list of HTTP methods to include in the audit request log, e.g., POST,PUT,DELETE').env('MAGE_HTTP_REQUEST_LOG_METHODS'),
