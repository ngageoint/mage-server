...
describe('EventDetailsComponent', () => {
  ...
  describe('Basic Component Tests', () => {
    ...
    it('should initialize with default values', () => {
      ...
      expect(component.showArchivedForms).toBe(false);
      expect(component.loadingMembers).toBe(true);
      expect(component.loadingTeams).toBe(true);
      expect(component.loadingLayers).toBe(true);
    });
  ...
  describe('User Role Management', () => {
    ...
    it('should update user role via teams service', () => {
      ...
      component.updateUserRole(USER_RANMA, 'MANAGER');

      expect(teamsService.updateUserRole).toHaveBeenCalledWith(
        'team-1',
        '1',
        'MANAGER'
      );
      expect((component.eventTeam as any).acl['1'].role).toBe('MANAGER');
    ...
    it('should not update role without event team', () => {
      component.eventTeam = null;
      component.updateUserRole(USER_RANMA, 'MANAGER');
      expect(teamsService.updateUserRole).not.toHaveBeenCalled();
    });

    it('should not update role without a new role', () => {
      component.updateUserRole(USER_RANMA, '');
      expect(teamsService.updateUserRole).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Event Details Editing', () => {
    ...
    it('should update event and breadcrumbs when dialog returns an updated event', () => {
      ...
      const dialogRef = jasmine.createSpyObj('MatDialogRef', ['afterClosed']);
      dialogRef.afterClosed.and.returnValue(of(updatedEvent));
      dialog.open.and.returnValue(dialogRef);
    ...
    it('should not open dialog without an event', () => {
      component.event = null;
      component.editEventDetails();
      expect(dialog.open).not.toHaveBeenCalled();
    });
  ...
  describe('Form Operations', () => {
    ...
    it('should open upload form dialog and navigate on close when form has id', fakeAsync(() => {
      const dialogRef = jasmine.createSpyObj('MatDialogRef', ['afterClosed']);
    ...
    it('should not navigate when upload form dialog closes without id', fakeAsync(() => {
      const dialogRef = jasmine.createSpyObj('MatDialogRef', ['afterClosed']);
      dialogRef.afterClosed.and.returnValue(of(null));
      dialog.open.and.returnValue(dialogRef);
    ...
    it('should not open upload form dialog without an event', () => {
      component.event = null;
      component.uploadForm();
      expect(dialog.open).not.toHaveBeenCalled();
    });
    ...
    it('should not reorder forms when event has no forms', () => {
      ...
      component.onFormsReordered([] as any);
