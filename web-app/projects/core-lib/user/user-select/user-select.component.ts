  ...
  implements OnInit, OnChanges, AfterViewInit, ControlValueAccessor
{
  ...
  ngOnInit() {
    this.searchTerm$
      .pipe(distinctUntilChanged(), debounceTime(this.searchTermDebounceTime))
      .subscribe((x) => {
  ...
  onScroll({ end }: { start: number; end: number }) {
    if (
      end < this.users.length - this.nextPageScrollThreshold ||
  ...
  fetchNextPage() {
    ...
    const nextPage = pageForItemIndex(
      this.users.length,
      this.baseFindParams.pageSize
    );
    ...
    this.currentFetch = this.userService
      .search(findParams)
      .subscribe((page) => {
