import {
  ...
  const TEST_LOGINS = [
    { id: 1, user: TEST_USERS[0], device: TEST_DEVICES[0] },
    { id: 2, user: TEST_USERS[1], device: TEST_DEVICES[1] },
    { id: 3, user: TEST_USERS[2], device: TEST_DEVICES[2] },
  ];
  ...
  describe("AdminDashboardComponent", () => {
    ...
    // fixture.detectChanges() runs ngOnInit
      ...
      expect(component.users.length).toBe(TEST_USERS.length);
      expect(component.devices.length).toBe(TEST_DEVICES.length);
    }));
    ...
    it("should filter logins by user, device, and dates", fakeAsync(() => {
      component.user = TEST_USERS[0];
      component.device = [TEST_DEVICES[0]];
      component.login.startDate = new Date();
      component.login.endDate = new Date();
      component.filterLogins();
      tick();
      expect(mockLoginService.query).toHaveBeenCalled();
      expect(component.loginPage.logins.length).toBeGreaterThan(0);
    }));
  
    it("should toggle filter visibility", () => {
      component.toggleFilters = false;
      component.toggleFilters = !component.toggleFilters;
      expect(component.toggleFilters).toBeTrue();
    });
    ...
    it("should display user and device properly", () => {
      expect(component.displayUser(TEST_USERS[0])).toBe("Ranma Saotome");
      expect(component.displayDevice(TEST_DEVICES[0])).toBe("Ranma's Phone");
    });
