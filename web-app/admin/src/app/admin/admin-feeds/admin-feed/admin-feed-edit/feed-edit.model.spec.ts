...
import {
  ...
  feedPostFromEditState
} from './feed-edit.model';

export type FeedMetaDataBooleanKeys = {
  [K in keyof FeedMetaData]: FeedMetaData[K] extends boolean ? K : never;
}[keyof FeedMetaData];
...
describe('feedMetaDataLean', () => {
  ...
  it('creates feed meta-data from populated feed', () => {
    ...
    const metaData = feedMetaDataLean(feed);
  ...
  it('strips empty string keys', () => {
    type FeedMetaDataStringKeys = {
      [K in keyof FeedMetaData]: FeedMetaData[K] extends string ? K : never;
    }[keyof FeedMetaData];
...
describe('feedPostFromEditState', () => {
  it('maps all properties from complete edit state', () => {
    ...
    const expectedFeed = {
      id: (state.originalFeed as any).id,
      service: (state.selectedService as any).id,
      topic: (state.selectedTopic as any).id,
