...
describe('ChooseServiceTopicComponent', () => {
  ...
  async function clickMdcSelectTrigger(selector: string): Promise<void> {
    ...
    expect(trigger)
      .withContext(`Missing select trigger for ${selector}`)
      .toBeTruthy();
  ...
  async function clickMdcOptionByIndex(index: number): Promise<void> {
    fixture.detectChanges();
    await fixture.whenStable();
    ...
    expect(options.length)
      .withContext('No mat-mdc-option elements found')
  ...
  it('sets the service form value from the input selected service', async () => {
    ...
    expect(element.querySelector('mat-panel-description')?.innerHTML).toEqual(
      'Hello 2 : '
    );
  ...
  it('emits service selected when form value changes', async () => {
    ...
    await clickMdcSelectTrigger('.service-select .mat-mdc-select-trigger');
    await clickMdcOptionByIndex(1);
  ...
  it('emits topic selected when form value changes', async () => {
    ...
    await clickMdcSelectTrigger('.topic-select .mat-mdc-select-trigger');
    await clickMdcOptionByIndex(0);
  ...
  it('changes selected topic to null before changing selected service from form', async () => {
    ...
    expect(serviceSelected).toHaveBeenCalledTimes(1);
    expect(topicSelected).toHaveBeenCalledTimes(1);
    expect(changes[0].type).toBe('topic');
    expect(changes[1].type).toBe('service');
  });
