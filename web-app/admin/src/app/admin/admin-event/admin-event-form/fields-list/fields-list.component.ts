...
import {
  ...
  FieldDialogData
} from '../form-details/field-dialog/field-dialog.component';
import { MatIconModule } from '@angular/material/icon';
import { DraggableListComponent } from '../../../../core/draggable-list/draggable-list.component';
...
@Component({
  ...
  standalone: true,
  imports: [DraggableListComponent, MatIconModule, CommonModule],
...
export class FieldsListComponent {
  ...
  getAttachmentTypesDisplay(field: Field): string {
    ...
    return field.allowedAttachmentTypes
      .map((type) => {
        const attachmentType = this.attachmentAllowedTypes.find(
          (at) => at.name === type
        );
  ...
  isMemberField(field: Field): boolean {
    const isExplicitUserType =
      field.type === 'userDropdown' || field.type === 'multiSelectUserDropdown';
