...
import { FieldsListComponent } from '../admin-event-form/fields-list/fields-list.component';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, FieldsListComponent],
...
export class CreateFormDialogComponent {
  ...
  private createFormWithFile(): void {
    ...
    this.eventsService
      .createForm(String(this.data.event.id), formData)
      .subscribe({
        next: (newForm) => {
          this.saving = false;
        ...
        error: (err) => {
          this.saving = false;
          this.errorMessage =
            err.error || 'Failed to create form. Please try again.';
        }
      });
  }
  ...
  save(): void {
    ...
    this.eventsService
      .createForm(String(this.data.event.id), formPayload)
      .subscribe({
        next: (newForm) => {
          this.saving = false;
          this.dialogRef.close(newForm);
