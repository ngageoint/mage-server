...
app.get('/api/v1/features/:id', function (req, res){
	...
	return FeatureModel.findOne({"attributes.OBJECTID": req.params.id}, {'_id': 0, 'geometry': 1, 'attributes': 1}, function (err, feature) {
...
app.get('/api/v1/esriFeatures/:id', function (req, res){
	...
	return FeatureModel.findOne({"attributes.OBJECTID": req.params.id}, {'_id': 0, 'geometry': 1, 'attributes': 1}, function (err, feature) {
		...
		//return res.send(feature.attributes);
...
app.post('/api/v1/esriFeatures/', function(req, res) {
	...
	FeatureModel.find({}, {'_id': 1, 'geometry': 1, 'attributes': 1}, function (err, features) {
		...
		if (postData[0].geometry.y != undefined) {
			var y = postData[0].geometry.y;
		} else {
			var y = "";
		}
		...
		feature.save(function (err) {
			if (!err) {
				return console.log(JSON.parse("{\"objectId\":" + newID + ", \"globalId\": null, \"success\": true}"));
				
			} else {
				return console.log(err);
			}
		});
		return res.send(JSON.parse("{\"addResults\": [{\"objectId\":" + newID + ", \"globalId\": null, \"success\": true}]}"));

	});
});

// This function will update a feature by the ID
app.put('/api/v1/esriFeatures/:id', function (req, res){
	console.log("SAGE Features (ID) UPDATE REST Service Requested");
	...
	FeatureModel.findOne({"attributes.OBJECTID": postData[0].attributes.OBJECTID}, function (err, feature) {
	...
	if (postData[0].attributes.UNIT != undefined) {
		...
		feature.attributes.OBJECTID = req.params.id;
		feature.attributes.ADDRESS = address;
		feature.attributes.EVENTDATE = eventDate;  
		feature.attributes.TYPE = type;  
		feature.attributes.EVENTLEVEL = eventLevel;
		feature.attributes.TEAM = team;
		feature.attributes.DESCRIPTION = description; 
		feature.attributes.USNG = usng;
		feature.attributes.EVENTCLEAR = eventClear;
		feature.attributes.UNIT = theUnit;

	
		return feature.save(function (err) {
			if (!err) {
				console.log("{\"updateResults\": [{\"objectId\":" + req.params.id + ", \"globalId\": null, \"success\": true}]}");
			} else {
				console.log(err);
			}
			
			return res.send(JSON.parse("{\"updateResults\": [{\"objectId\":" + req.params.id + ", \"globalId\": null, \"success\": true}]}"));
			//return res.send(feature);
		});
	});
...
app.get('/api/v1/esriFeatures/:id/attachments', function(req, res){
	...
	return AttachmentModel.find({OBJECTID: req.params.id}, function (err, attachments) {
		...
		if( err || !attachments.length) {
			console.log("No attachments were found.");
			return res.send("No attachments were found.");	
		}
		else {
			...
			return res.send(JSON.parse(fullString));
		};
	});
...
//						res.send(attachment);	
	...
	return res.send(JSON.parse("{\"addAttachmentResult\": [{\"objectId\":" + newID + ", \"globalId\": null, \"success\": true}]}"));

	});
});

// This function gets all the attachments for a particular ESRI record	
app.get('/api/v1/esriFeatures/:id/attachments/:id', function(req, res){
	console.log("SAGE ESRI Features (ID) Attachments (ID) GET REST Service Requested");
	
	console.log(req.params);
	//return res.send("No bueno.");

	return AttachmentModel.find({id: req.params.id}, function (err, attachment) {
		
		
		attachment.forEach( function(attachmentData) {
			//console.log(attachmentData.contentType);
			//return res.send(attachmentData.contentType);
			if (err) {
				return console.log(err);
			} else {

				fs.readFile('uploads/' + attachmentData.name, function (err,data) {
					if (err) {
						return console.log(err);
					}
...
app.get('/api/v1/getMax', function (req, res){
	console.log("SAGE Get Max REST Service Requested");
...
app.get('/api/v1/epoch', function (req, res){
	console.log("SAGE Epoch GET REST Service Requested");
	...
	//var test_time = getDate(1352095860000);
	
	//var test_date = test_time.get
...
app.get('/api/v1/random', function (req, res){
	console.log("SAGE Random X Y Coordinates REST Service Requested");

	var x=Math.floor((Math.random()*14) + 30);
	var y=Math.floor((Math.random()*42) + 81);
