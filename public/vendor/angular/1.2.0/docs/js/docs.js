...
docsApp.controller.DocsNavigationCtrl = ['$scope', '$location', 'docsSearch', function($scope, $location, docsSearch) {
  ...
  $scope.search = function(q) {
    ...
    if(q.length >= MIN_SEARCH_LENGTH) {
      ...
      if(totalSections > 0) {
        $scope.colCl***REMOVED***Name = 'cols-' + totalSections;
      }
...
docsApp.***REMOVED***Factory.lunrSearch = function() {
...
docsApp.***REMOVED***Factory.docsSearch = ['$rootScope','lunrSearch', 'NG_PAGES',
...
docsApp.directive.sourceEdit = function(getEmbeddedTemplate) {
  return {
    template: '<div cl***REMOVED***="btn-group pull-right">' +
        '<a cl***REMOVED***="btn dropdown-toggle btn-primary" data-toggle="dropdown" href>' +
        '  <i cl***REMOVED***="icon-pencil icon-white"></i> Edit<span cl***REMOVED***="caret"></span>' +
        '</a>' +
...
docsApp.directive.docTutorialNav = function(templateMerge) {
  ...
  return {
    compile: function(element, attrs) {
      ...
      element.addCl***REMOVED***('btn-group');
      element.addCl***REMOVED***('tutorial-nav');
      element.append(templateMerge(
        '<li cl***REMOVED***="btn btn-primary"><a href="tutorial/{{prev}}"><i cl***REMOVED***="icon-step-backward"></i> Previous</a></li>\n' +
        '<li cl***REMOVED***="btn btn-primary"><a href="http://angular.github.com/angular-phonecat/step-{{seq}}/app"><i cl***REMOVED***="icon-play"></i> Live Demo</a></li>\n' +
        '<li cl***REMOVED***="btn btn-primary"><a href="https://github.com/angular/angular-phonecat/compare/step-{{diffLo}}...step-{{diffHi}}"><i cl***REMOVED***="icon-search"></i> Code Diff</a></li>\n' +
        '<li cl***REMOVED***="btn btn-primary"><a href="tutorial/{{next}}">Next <i cl***REMOVED***="icon-step-forward"></i></a></li>', props));
    }
  };
};
...
docsApp.directive.docTutorialReset = function() {
  function tab(name, command, id, step) {
    return '' +
      '  <div cl***REMOVED***=\'tab-pane well\' title="' + name + '" value="' + id + '">\n' +
      '    <ol>\n' +
  ...
  return {
    compile: function(element, attrs) {
      ...
      element.html(
        ...
        '</div>\n' +
        '<div cl***REMOVED***="tabbable" ng-show="show" ng-model="$cookies.platformPreference">\n' +
...
docsApp.***REMOVED***Factory.angularUrls = function($document) {
  var urls = {};
...
docsApp.***REMOVED***Factory.formPostData = function($document) {
...
docsApp.***REMOVED***Factory.prepareDefaultAppModule = function() {
...
docsApp.***REMOVED***Factory.prepareEditorAssetTags = function(angularUrls) {
    ...
    angular.forEach(content.deps, function(file) {
      if (file.name !== 'angular.js') {
        ...
        if(!(isLocal && !includeLocalFiles)) {
          var ***REMOVED***etUrl = angularUrls[file.name] || file.name;
          html += makeScriptTag(***REMOVED***etUrl);
        }
      }
    });
...
docsApp.***REMOVED***Factory.openPlunkr = function(templateMerge, formPostData, prepareEditorAssetTags, prepareDefaultAppModule) {
  return function(content) {
...
docsApp.***REMOVED***Factory.openJsFiddle = function(templateMerge, formPostData, prepareEditorAssetTags, prepareDefaultAppModule) {
...
docsApp.***REMOVED***Factory.sections = ['NG_PAGES', function sections(NG_PAGES) {
  var sections = {
    guide: [],
    api: [],
    tutorial: [],
...
docsApp.controller.DocsController = function($scope, $location, $window, $cookies, sections) {
  ...
  var OFFLINE_COOKIE_NAME = 'ng-offline',
      ...
      URL = {
        ...
        ***REMOVED***: 'guide/dev_guide.***REMOVED***s',
        type: 'guide/types'
      };
  ...
  $scope.navCl***REMOVED*** = function(page1, page2) {
    return {
      last: this.$last,
  ...
  function updateSearch() {
    ...
    angular.forEach(pages, function(page) {
      ...
      if (page.id == 'index') {
        ...
        module(match[1]).***REMOVED***(match[2])[match[3] ? 'provider' : 'instance'] = page;
      ...
      if (!module) {
        module = moduleCache[name] = {
          ...
          ***REMOVED***: function(name) {
            var ***REMOVED*** =  moduleCache[this.name + ':' + name];
            if (!***REMOVED***) {
              ***REMOVED*** = {name: name};
              moduleCache[this.name + ':' + name] = ***REMOVED***;
              this.***REMOVED***s.push(***REMOVED***);
            }
            return ***REMOVED***;
          },
          types: [],
...
angular.module('docsApp', ['ngResource', 'ngRoute', 'ngCookies', 'ngSanitize', 'ngAnimate', 'bootstrap', 'bootstrapPrettify', 'docsData']).
  ...
  factory(docsApp.***REMOVED***Factory).
