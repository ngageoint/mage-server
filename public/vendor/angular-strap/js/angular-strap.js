  ...
  function ($parse, $timeout, $compile) {
    return {
      ...
      link: function postLink(scope, element, attrs) {
        var getter = $parse(attrs.bsAlert), setter = getter.***REMOVED***ign, value = getter(scope);
        ...
        element.on('close', function (ev) {
          ...
          if (parentArray) {
            ...
            $.support.transition && element.hasCl***REMOVED***('fade') ? element.on($.support.transition.end, removeElement) : removeElement();
          } else if (value) {
            ev.preventDefault();
            element.removeCl***REMOVED***('in');
            removeElement = function () {
        ...
        button.toggle = function () {
          ...
          if ($parent.length) {
            element.siblings('[ng-model]').each(function (k, v) {
              $parse($(v).attr('ng-model')).***REMOVED***ign(scope, false);
            });
            scope.$digest();
    ...
    return {
      ...
      compile: function compile(tElement, tAttrs, transclude) {
        ...
        return function postLink(scope, iElement, iAttrs, controller) {
          if (controller) {
            $timeout(function () {
              iElement.find('[value]').button().filter('[value="' + controller.$viewValue + '"]').addCl***REMOVED***('active');
            });
  ...
  function ($parse, $timeout) {
    return {
      ...
      link: function postLink(scope, element, attrs, ctrl) {
        var getter = $parse(attrs.bsButtonSelect), setter = getter.***REMOVED***ign;
        if (ctrl) {
...
angular.module('$strap.directives').directive('bsDropdown', [
  ...
  function ($parse, $compile, $timeout) {
    var buildTemplate = function (items, ul) {
      if (!ul)
        ul = [
          '<ul cl***REMOVED***="dropdown-menu" role="menu" aria-labelledby="drop1">',
          '</ul>'
        ];
      angular.forEach(items, function (item, index) {
        if (item.divider)
          return ul.splice(index + 1, 0, '<li cl***REMOVED***="divider"></li>');
        var li = '<li' + (item.submenu && item.submenu.length ? ' cl***REMOVED***="dropdown-submenu"' : '') + '>' + '<a tabindex="-1" ng-href="' + (item.href || '') + '"' + (item.click ? '" ng-click="' + item.click + '"' : '') + (item.target ? '" target="' + item.target + '"' : '') + (item.method ? '" data-method="' + item.method + '"' : '') + '>' + (item.text || '') + '</a>';
    ...
    return {
      ...
      link: function postLink(scope, iElement, iAttrs) {
        ...
        iElement.addCl***REMOVED***('dropdown-toggle').attr('data-toggle', 'dropdown');
      }
    };
  }
]);
    ...
    var ModalFactory = function ModalFactory(config) {
      function Modal(config) {
        ...
        return $q.when($templateCache.get(templateUrl) || $http.get(templateUrl, { cache: true }).then(function (res) {
          ...
          var $modal = $('<div cl***REMOVED***="modal hide" tabindex="-1"></div>').attr('id', id).addCl***REMOVED***('fade').html(template);
          if (options.modalCl***REMOVED***)
            $modal.addCl***REMOVED***(options.modalCl***REMOVED***);
          $('body').append($modal);
          $timeout(function () {
            $compile($modal)(scope);
          });
        ...
        scope.$watch(function () {
          ...
          $('li[data-match-route]', element).each(function (k, li) {
            ...
            if (regexp.test(newValue)) {
              $li.addCl***REMOVED***('active').find('.collapse.in').collapse('hide');
            } else {
              $li.removeCl***REMOVED***('active');
            }
          });
        });
      }
  ...
  function ($parse, $compile, $http, $timeout, $q, $templateCache) {
    ...
    return {
      ...
      link: function postLink(scope, element, attr, ctrl) {
        var getter = $parse(attr.bsPopover), setter = getter.***REMOVED***ign, value = getter(scope), options = {};
    ...
    return {
      ...
      link: function postLink(scope, element, attrs, controller) {
        ...
        $timeout(function () {
          ...
          element.next().removeCl***REMOVED***('ng-scope');
        });
        if (controller) {
          scope.$watch(attrs.ngModel, function (newValue, oldValue) {
...
angular.module('$strap.directives').directive('bsTabs', [
  ...
  '$timeout',
  function ($parse, $compile, $timeout) {
    var template = '<div cl***REMOVED***="tabs">' + '<ul cl***REMOVED***="nav nav-tabs">' + '<li ng-repeat="pane in panes" ng-cl***REMOVED***="{active:pane.active}">' + '<a data-target="#{{pane.id}}" data-index="{{$index}}" data-toggle="tab">{{pane.title}}</a>' + '</li>' + '</ul>' + '<div cl***REMOVED***="tab-content" ng-transclude>' + '</div>';
    return {
      restrict: 'A',
      require: '?ngModel',
      priority: 0,
      ...
      compile: function compile(tElement, tAttrs, transclude) {
        return function postLink(scope, iElement, iAttrs, controller) {
          var getter = $parse(iAttrs.bsTabs), setter = getter.***REMOVED***ign, value = getter(scope);
          scope.panes = [];
  ...
  function ($timeout, $strapConfig) {
    ...
    return {
      ...
      link: function postLink(scope, element, attrs, controller) {
        ...
        element.parent().addCl***REMOVED***('bootstrap-timepicker');
...
angular.module('$strap.directives').directive('bsTooltip', [
  ...
  function ($parse, $compile) {
    return {
      ...
      scope: true,
      link: function postLink(scope, element, attrs, ctrl) {
        var getter = $parse(attrs.bsTooltip), setter = getter.***REMOVED***ign, value = getter(scope);
...
angular.module('$strap.directives').directive('bsTypeahead', [
  ...
  function ($parse) {
    return {
      ...
      require: '?ngModel',
      link: function postLink(scope, element, attrs, controller) {
        var getter = $parse(attrs.bsTypeahead), setter = getter.***REMOVED***ign, value = getter(scope);
