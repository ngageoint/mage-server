		...
		function mdDateTimePicker(_ref) {
			var type = _ref.type,
			    ...
			    _ref$ok = _ref.ok,
					...
					_ref$showOk = _ref.showOk,
					showOk = _ref$showOk === undefined ? true : _ref$showOk,
			    ...
			    _ref$nextHandle = _ref.nextHandle,
					...
					containerElement = _ref$containerElement === undefined ? document.getElementsByTagName('body').item(0) : _ref$containerElement;
  ...
  */
		...
		_createClass(mdDateTimePicker, [{
			...
			value: function show() {
				...
				if (this._type === 'date') {
					...
					this._initDateDialog(this._init);
					this._initTimeDialog(this._init);
					// this.initDateTimeDialog(this._init);
				}
				this._showDialog();
			}
		}, {
    ...
    */
			...
			value: function _hideDialog() {
				...
				if (this.type === 'time' || this._type === 'datetime') {
			...
			value: function _buildDialog() {
				...
				this._addId(headerWrap, 'header-wrap');
				this._addClass(headerWrap, 'header-wrap');
				...
				this._addClass(pickers, 'pickers');
				this._addClass(pickers, this._orientation.toLowerCase())
				...
				if (this._type === 'date' || this._type === 'datetime') {
					...
					this._addId(dateHeader, 'dateHeader');
					this._addClass(dateHeader, 'dateHeader');
					...
					// add them to header
					dateHeader.appendChild(subtitle);
					dateHeader.appendChild(title);
					header.appendChild(dateHeader);
					// inside body
					this._addId(dateHolder, 'dateHolder');
					this._addClass(dateHolder, 'dateHolder');
					// inside viewHolder
					...
					dateHolder.appendChild(viewHolder);
					dateHolder.appendChild(left);
					dateHolder.appendChild(right);
					dateHolder.appendChild(years);
					// add them to body
					pickers.appendChild(dateHolder);
					// body.appendChild(left);
					// body.appendChild(right);
					// body.appendChild(years);
				}
				if (this._type === 'time' || this._type === 'datetime') {
					...
					// add properties to them
					// inside header
					this._addId(timeHeader, 'timeHeader');
					this._addClass(timeHeader, 'timeHeader');
					...
					// add them to header
					timeHeader.appendChild(_title);
					timeHeader.appendChild(_subtitle);
					...
					// add them to body
					pickers.appendChild(circularHolder);
				}
				...
				if (!this._showCancel) cancel.style.display = "none";
				this._addId(ok, 'ok');
				ok.classList.add('mddtp-button');
				ok.setAttribute('type', 'button');
				if (!this._showOk) ok.style.display = "none";
				// add actions
			...
			value: function _initTimeDialog(m) {
				...
				this._changeM();
				this._dragDial();
				this._switchToView(hour, 'time');
				this._switchToView(minute, 'time');
				this._addClockEvent();
				this._setButtonText();
			}
		}, {
			...
			value: function _initDateDialog(m) {
				...
				this._attachEventHandlers();
				this._changeMonth();
				this._switchToView(subtitle, 'date');
				this._switchToView(title, 'date');
				this._setButtonText();
			}
		}, {
			key: '_initViewHolder',
    ...
    */
			...
			key: '_switchToView',
			value: function _switchToView(el, typeOverride) {
				var me = this;
				// attach the view change button
				if (typeOverride === 'date' || this._type == 'date') {
					el.onclick = function () {
			...
			value: function _switchToDateView(el, me) {
				...
				if (mdDateTimePicker.dialog.view) {
					...
					me._sDialog.right.style.display = null;
					me._sDialog.left.style.display = null;
					setTimeout(function () {
						years.classList.remove('zoomIn', 'zoomOut');
