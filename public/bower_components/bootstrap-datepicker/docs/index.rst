...
Requires bootstrap's dropdown component (``dropdowns.less``) for some styles, and bootstrap's sprites (``sprites.less`` and ***REMOVED***ociated images) for arrows.
...
:doc:`options` are p***REMOVED***ed to the ``datepicker`` function via an options hash at instantiation::
...
.. code-block:: html
    ...
    <input cl***REMOVED***="datepicker" data-date-format="mm/dd/yyyy">
