...
test('adds cw header column', function(){
    ...
    ok(target.hasCl***REMOVED***('cw'), 'First column heading is from cw column');
});
...
test('adds calendar week cells to each day row', function(){
    ...
    target.each(function(i){
        ...
        ok(t.hasCl***REMOVED***('cw'), "First column is cw column");
    });
});
...
test('it prepends column to switcher thead row', function(){
    ...
    ok(target.children().first().hasCl***REMOVED***('cw'), 'cw column is prepended');
});
