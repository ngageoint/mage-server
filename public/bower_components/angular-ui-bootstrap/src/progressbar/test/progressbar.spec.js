describe('progressbar directive with no binding', function () {
  ...
  it('has a "progress" css cl***REMOVED***', function() {
    expect(element.hasCl***REMOVED***('progress')).toBe(true);
  });

  it('contains one child element with "bar" css cl***REMOVED***', function() {
    expect(element.children().length).toBe(1);
    expect(element.children().eq(0).hasCl***REMOVED***('bar')).toBe(true);
  });
...
describe('progressbar directive with data-binding', function () {
  ...
  it('can handle correctly objects value && cl***REMOVED***', function() {
    $rootScope.percent = {
      value: 45,
      type: 'warning'
    };
    ...
    expect(barElement.hasCl***REMOVED***('bar')).toBe(true);
    expect(barElement.hasCl***REMOVED***('bar-warning')).toBe(true);
  });
...
describe('stacked progressbar directive', function () {
  ...
  it('contains tree child elements with "bar" css cl***REMOVED*** each', function() {
    expect(element.children().length).toBe(3);
    expect(element.children().eq(0).hasCl***REMOVED***('bar')).toBe(true);
    expect(element.children().eq(1).hasCl***REMOVED***('bar')).toBe(true);
    expect(element.children().eq(2).hasCl***REMOVED***('bar')).toBe(true);
  });
  ...
  it('can handle correctly object changes', function() {
    ...
    expect(barElement.hasCl***REMOVED***('bar')).toBe(true);
    expect(barElement.hasCl***REMOVED***('bar-danger')).toBe(false);

    barElement = element.children().eq(1);
    expect(barElement.css('width')).toBe('29%');
    expect(barElement.hasCl***REMOVED***('bar')).toBe(true);
    expect(barElement.hasCl***REMOVED***('bar-danger')).toBe(true);
  ...
  it('can handle mixed objects with custom cl***REMOVED***es', function() {
    $rootScope.stacked = [
      { value: 15, type: 'info' },
      11, 
      { value: 9, type: 'danger' },
    ...
    expect(barElement.hasCl***REMOVED***('bar-info')).toBe(true);

    barElement = element.children().eq(1);
    expect(barElement.css('width')).toBe('11%');
    expect(barElement.hasCl***REMOVED***('bar-info')).toBe(false);
    ...
    expect(barElement.hasCl***REMOVED***('bar-warning')).toBe(false);
  });
...
describe('stacked progressbar directive with auto-types', function () {
  ...
  it('contains tree child elements with "bar" css cl***REMOVED*** each', function() {
    expect(element.children().length).toBe(5);
    for (var i = 0; i < 5; i++) {
      expect(element.children().eq(i).hasCl***REMOVED***('bar')).toBe(true);
    }
  });
  ...
  it('has elements with automatic types', function() {
    ...
    for (var i = 0; i < stackedTypes.length; i++) {
      expect(element.children().eq(i).hasCl***REMOVED***('bar-' + stackedTypes[i])).toBe(true);
    }
  });
  ...
  it('ignore automatic type if one is specified', function() {
    ...
    expect(bar.hasCl***REMOVED***('bar-' + stackedTypes[1])).toBe(false);
    expect(bar.hasCl***REMOVED***('bar-something')).toBe(true);
  });


  it('can provide automatic cl***REMOVED***es to be applied', function() {
    $rootScope.stacked[1] = {
      value: 18,
      type: 'something'
    };
  ...
  it('can byp***REMOVED*** default configuration for stacked cl***REMOVED***es from attribute', function() {
    ...
    expect(element.children().eq(0).hasCl***REMOVED***('bar-danger')).toBe(true);
    expect(element.children().eq(0).hasCl***REMOVED***('bar-' + stackedTypes[0])).toBe(false);

    expect(element.children().eq(1).hasCl***REMOVED***('bar-warning')).toBe(true);
    expect(element.children().eq(2).hasCl***REMOVED***('bar-success')).toBe(true);
  });
