describe('rating directive', function () {
  ...
  function getState(cl***REMOVED***On, cl***REMOVED***Off) {
    var stars = getStars();
    var state = [];
    for (var i = 0, n = stars.length; i < n; i++) {
      state.push( (stars.eq(i).hasCl***REMOVED***(cl***REMOVED***On || 'icon-star') && ! stars.eq(i).hasCl***REMOVED***(cl***REMOVED***Off || 'icon-star-empty')) );
    }
    return state;
  }
  ...
  describe('custom states', function() {
    beforeEach(inject(function() {
      $rootScope.cl***REMOVED***On = 'icon-ok-sign';
      $rootScope.cl***REMOVED***Off = 'icon-ok-circle';
      element = $compile('<rating value="rate" state-on="cl***REMOVED***On" state-off="cl***REMOVED***Off"></rating>')($rootScope);
      $rootScope.$digest();
    }));

    it('changes the default icons', function() {
      expect(getState($rootScope.cl***REMOVED***On, $rootScope.cl***REMOVED***Off)).toEqual([true, true, true, false, false]);
    });
  });
  ...
  describe('`rating-states`', function() {
    ...
    it('handles each icon', function() {
      ...
      for (var i = 0; i < stars.length; i++) {
        ...
        expect(star.hasCl***REMOVED***(state.stateOn)).toBe(isOn);
        expect(star.hasCl***REMOVED***(state.stateOff)).toBe(!isOn);
      }
    });
  });
