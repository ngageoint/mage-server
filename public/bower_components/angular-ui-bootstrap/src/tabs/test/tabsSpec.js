describe('tabs', function() {
  ...
  describe('basics', function() {
    ...
    beforeEach(inject(function($compile, $rootScope) {
      ...
      elm = $compile([
        '<div>',
        '  <tabset cl***REMOVED***="hello" data-pizza="pepperoni">',
    ...
    it('should p***REMOVED*** cl***REMOVED*** and other attributes on to tab template', function() {
      var tabbable = elm.find('.tabbable');
      expect(tabbable).toHaveCl***REMOVED***('hello');
    ...
    it('should bind tabs content and set first tab active', function() {
      ...
      expect(titles().eq(0)).toHaveCl***REMOVED***('active');
      expect(titles().eq(1)).not.toHaveCl***REMOVED***('active');
    ...
    it('should change active on click', function() {
      ...
      expect(contents().eq(1)).toHaveCl***REMOVED***('active');
      expect(titles().eq(0)).not.toHaveCl***REMOVED***('active');
      expect(titles().eq(1)).toHaveCl***REMOVED***('active');
    ...
    function expectTabActive(activeTab) {
      ...
      angular.forEach(scope.tabs, function(tab, i) {
        if (activeTab === tab) {
          ...
          expect(_titles.eq(i)).toHaveCl***REMOVED***('active');
          expect(contents().eq(i).text().trim()).toBe('content ' + i);
          expect(contents().eq(i)).toHaveCl***REMOVED***('active');
        } else {
          expect(tab.active).toBe(false);
          expect(_titles.eq(i)).not.toHaveCl***REMOVED***('active');
        }
      });
    }
  ...
  describe('advanced tab-heading element', function() {
    beforeEach(inject(function($compile, $rootScope) {
      ...
      elm = $compile([
        '<tabset>',
        '  <tab>',
        '    <tab-heading ng-bind-html-u***REMOVED***fe="myHtml" ng-show="value">',
        '    </tab-heading>',
        '  </tab>',
  ...
  describe('remove', function() {
    ...
    it('should remove title tabs when elements are destroyed and change selection', inject(function($controller, $compile, $rootScope) {
      ...
      expect(contents().eq(3)).toHaveCl***REMOVED***('active');
      expect(titles().eq(3)).toHaveCl***REMOVED***('active');

      scope.$apply('list = [1,2]');
      expectTitles(['1', 'tab 1', 'tab 2']);
      expectContents(['Hello', 'content 1', 'content 2']);

      expect(titles().eq(2)).toHaveCl***REMOVED***('active');
      expect(contents().eq(2)).toHaveCl***REMOVED***('active');
    }));
  });
  ...
  describe('disabled', function() {
    ...
    it('should toggle between states', function() {
      expect(titles().eq(3)).toHaveCl***REMOVED***('disabled');
      scope.$apply('tabs[3].disabled = false');
      expect(titles().eq(3)).not.toHaveCl***REMOVED***('disabled');

      expect(titles().eq(2)).not.toHaveCl***REMOVED***('disabled');
      scope.$apply('tabs[2].disabled = true');
      expect(titles().eq(2)).toHaveCl***REMOVED***('disabled');
    });
  });
  ...
  describe('vertical', function() {
    ...
    it('to stack tabs', function() {
      expect(elm.find('ul.nav-tabs')).toHaveCl***REMOVED***('nav-stacked');
    });
  });
  ...
  describe('type', function() {
    ...
    it('to show pills', function() {
      expect(elm.find('ul')).toHaveCl***REMOVED***('nav-pills');
      expect(elm.find('ul')).not.toHaveCl***REMOVED***('nav-tabs');
    });
  });

  describe('direction', function() {
    it('should not have `tab-left`, `tab-right` nor `tabs-below` cl***REMOVED***es if the direction is undefined', inject(function($compile, $rootScope) {
      scope = $rootScope.$new();
      scope.direction = undefined;

      elm = $compile('<tabset direction="direction"></tabset>')(scope);
      scope.$apply();
      expect(elm).not.toHaveCl***REMOVED***('tabs-left');
      expect(elm).not.toHaveCl***REMOVED***('tabs-right');
      expect(elm).not.toHaveCl***REMOVED***('tabs-below');
      expect(elm.find('.nav + .tab-content').length).toBe(1);
    }));

    it('should only have the `tab-left` direction cl***REMOVED*** if the direction is "left"', inject(function($compile, $rootScope) {
      scope = $rootScope.$new();
      scope.direction = 'left';

      elm = $compile('<tabset direction="direction"></tabset>')(scope);
      scope.$apply();
      expect(elm).toHaveCl***REMOVED***('tabs-left');
      expect(elm).not.toHaveCl***REMOVED***('tabs-right');
      expect(elm).not.toHaveCl***REMOVED***('tabs-below');
      expect(elm.find('.nav + .tab-content').length).toBe(1);
    }));

    it('should only have the `tab-right direction cl***REMOVED*** if the direction is "right"', inject(function($compile, $rootScope) {
      scope = $rootScope.$new();
      ...
      scope.$apply();
      expect(elm).not.toHaveCl***REMOVED***('tabs-left');
      expect(elm).toHaveCl***REMOVED***('tabs-right');
      expect(elm).not.toHaveCl***REMOVED***('tabs-below');
      expect(elm.find('.nav + .tab-content').length).toBe(1);
    }));

    it('should only have the `tab-below direction cl***REMOVED*** if the direction is "below"', inject(function($compile, $rootScope) {
      scope = $rootScope.$new();
      ...
      expect(elm).not.toHaveCl***REMOVED***('tabs-left');
      expect(elm).not.toHaveCl***REMOVED***('tabs-right');
      expect(elm).toHaveCl***REMOVED***('tabs-below');
