describe('dropdownToggle', function() {
  ...
  function dropdown() {
    return $compile('<li cl***REMOVED***="dropdown"><a dropdown-toggle></a><ul dropdown-toggle><li>Hello</li></ul></li>')($rootScope);
  }
  
  it('should toggle on `a` click', function() {
    var elm = dropdown();
    expect(elm.hasCl***REMOVED***('open')).toBe(false);
    elm.find('a').click();
    expect(elm.hasCl***REMOVED***('open')).toBe(true);
    elm.find('a').click();
    expect(elm.hasCl***REMOVED***('open')).toBe(false);
  });
  ...
  it('should only allow one dropdown to be open at once', function() {
    ...
    elm1.find('a').click();
    elm2.find('a').click();
    expect(elm1.hasCl***REMOVED***('open')).toBe(false);
    expect(elm2.hasCl***REMOVED***('open')).toBe(true);
  });
});
