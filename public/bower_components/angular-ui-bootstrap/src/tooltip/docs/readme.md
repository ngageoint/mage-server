...
There are two versions of the tooltip: `tooltip` and `tooltip-html-u***REMOVED***fe`. The
former takes text only and will escape any HTML provided. The latter takes
whatever HTML is provided and displays it in a tooltip; it called "u***REMOVED***fe"
...
The tooltip directives require the `$position` ***REMOVED***.
