...
- **New and improved features:**
  ...
  - Added print utility cl***REMOVED***es to show and hide content between `screen` and `print` via CSS.
  ...
  - Added `.text-left`, `.text-center`, and `.text-right` utility cl***REMOVED***es for easy typographic alignment.
  - Added `@ms-viewport` so IE10 can use responsive CSS when in split-screen mode.
- **Docs changes:**
  - Added [new justified navigation example](https://f.cloud.github.com/***REMOVED***ets/98681/25869/5e2f812c-4afa-11e2-9293-501cd689232d.png).
...
* New feature: alert text. We documented these new cl***REMOVED***es, like `.text-success`, at the bottom of the [Typography section](http://twitter.github.com/bootstrap/base-css.html#typography) along with the long undocumented `.muted`.
...
* Block level buttons
* State cl***REMOVED***es on table rows
* Improved disabled states on navs and dropdowns
* The navbar component is now white by default, with an optional cl***REMOVED*** to darken it
...
- Refactored forms.less to make our selectors more specific for fewer overrides and less code. Instead of a generic `input` selector and various resets, we target each type of input like `input[type="text"]`, `input[type="p***REMOVED***word"]`, etc.
...
- Overhauled the responsive utility cl***REMOVED***es to simplify required CSS, add `!important` to all declarations, and use `display: inherit` in place of `display: block` to account for different types of elements.
- Removed `>` from fluid grid column selectors, meaning every element with a `.span*` cl***REMOVED*** within a `.row-fluid` will use percentage widths instead of fixed-pixels.
...
- Generalized and simplified the open dropdown cl***REMOVED***es while adding smarter defaults. Instead of `.dropdown.open`, we now use just `.open`. On the defaults side, all dropdown menus now have rounded corners to start.
...
- Fixed issue with anchor buttons in the `.navbar-text`. Instead of a general styling on all anchors within an element with that cl***REMOVED***, we now have a new cl***REMOVED*** to specifically apply appropriate link color.
...
- Changed specificity of grid cl***REMOVED***es in responsive layouts under 767px to accurately target `input`, `select`, and `textarea` elements that use `.span*` cl***REMOVED***es.
...
- Fixed incorrect use of cl***REMOVED*** instead of ID for tabs example and added documentation for multiple ways of toggling tabs.
...
- Replaced `.badge-error` with `.badge-important`. The error option is not a valid cl***REMOVED*** and was a typo in the docs.
- Fixed mention of how to add plain text to the navbar. Previously the docs stated you only needed a `p` tag, but the required HTML is any element with cl***REMOVED*** `.navbar-text`.
- Clarified the use of `.tabbable` for tabs. The wrapping cl***REMOVED*** is only required for left and right tabs to clear their floats. Also added mention of `.fade` to fade in tabs.
- Updated forms documentation:
  - Remove unnecessary duplicate help text in first example
  - Added mention of required `input` cl***REMOVED***, `.search-query`, for the search form variation
  - Removed incorrect mention of form fields being `display: block;` to start as fields are `inline-block` to start.
- Added mention of `data-target` attribute for the dropdowns javascript plugin to show how to keep custom URLs intact on links with `.dropdown-toggle` cl***REMOVED***.
...
- Updated progress bars section to simplify how the cl***REMOVED***es stack and more clearly indicate the available optional cl***REMOVED***es and styles.
...
- Updated `.navbar .container` to use `width: auto;` to start and then reset the fixed widths via the `#gridSystem` mixin (it's a little dirty, but required to avoid adding another cl***REMOVED***).
...
- Added visible/hidden cl***REMOVED***es for devices
...
- Added `.disabled` cl***REMOVED*** support to the pager component (also added a mention of this to the docs)
- Added `.well-large` and `.well-small` cl***REMOVED***es for extending the well component
...
- Previously the docs called for use of `.control-label` in the examples, but the CSS didn't make clear use of it. This cl***REMOVED*** is required for horizontal forms and has been reflected in the CSS.
...
- Added a mini button cl***REMOVED***, `.btn-mini`.
...
- **M***REMOVED***ively updated docs** for both the main page and for the new javascript plugins
...
- **Added HTML5 form support** by generalizing the form selectors (e.g., input instead of input[type=text|p***REMOVED***word])
- **Gradients back in IE**, but removed rounded corners in IE9 to prevent background bleed on buttons and alert messages
- **Simplified the grid CSS** by removing the static .span1-16 cl***REMOVED***es in favor of CSS-style regex for column styles in the grid
