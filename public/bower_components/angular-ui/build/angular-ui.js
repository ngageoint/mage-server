  ...
  return {
    ...
    link: function (scope, element, attrs, controller) {
      ...
      renderview = function (viewvalue) {
        var num;
        num = viewvalue * 1;
        element.toggleCl***REMOVED***(opts.pos, (num > 0) );
        element.toggleCl***REMOVED***(opts.neg, (num < 0) );
        element.toggleCl***REMOVED***(opts.zero, (num === 0) );
        if (viewvalue === '') {
          element.text('');
        } else {
...
/*
 ...
 @param [ui-date] {object} Options to p***REMOVED*** to $.fn.datepicker() merged onto ui.config
 */
 ...
 * P***REMOVED*** an object with keynames for events to ui-event
 * Allows $event object and $params object to be p***REMOVED***ed
 *
...
/**
 * General-purpose jQuery wrapper. Simply p***REMOVED*** the plugin name as the expression.
 *
 * It is possible to specify a default set of parameters for each jQuery plugin.
 * Under the jq key, namespace each plugin by that which will be p***REMOVED***ed to ui-jq.
 * Unfortunately, at this time you can only pre-define the first parameter.
 * @example { jq : { datepicker : { showOn:'click' } } }
 *
 * @param ui-jq {string} The $elm.[pluginName]() to call.
 * @param [ui-options] {mixed} Expression to be evaluated and p***REMOVED***ed as options to the function
  ...
  return {
    ...
    compile: function uiJqCompilingFunction(tElm, tAttrs) {
      ...
      return function uiJqLinkingFunction(scope, elm, attrs) {
        ...
        // If ui-options are p***REMOVED***ed, merge (or override) them onto global defaults and p***REMOVED*** to the jQuery method
        if (attrs.uiOptions) {
        ...
        // Call jQuery method and p***REMOVED*** relevant options
        function callPlugin() {
          $timeout(function() {
      ...
      return {
        link: function (scope, elm, attrs) {
          ...
          if (!infoWindow) {
            ...
            model.***REMOVED***ign(scope, infoWindow);
          }
...
angular.module('ui.directives').directive('uiReset', ['ui.config', function (uiConfig) {
  ...
  return {
    ...
    link: function (scope, elm, attrs, ctrl) {
      var aElement;
      aElement = angular.element('<a cl***REMOVED***="ui-reset" />');
      elm.wrap('<span cl***REMOVED***="ui-resetwrap" />').after(aElement);
...
angular.module('ui.directives').directive('uiRoute', ['$location', '$parse', function ($location, $parse) {
  return {
    ...
    compile: function(tElement, tAttrs) {
      ...
      return function ($scope, elm, attrs) {
        var modelSetter = $parse(attrs.ngModel || attrs.routeModel || '$uiRoute').***REMOVED***ign;
...
/**
 * Adds a 'ui-scrollfix' cl***REMOVED*** to the element when the page scrolls past it's position.
  ...
  return {
    link: function (scope, elm, attrs) {
      ...
      angular.element($window).on('scroll.ui-scrollfix', function () {
        // if pageYOffset is defined use it, otherwise use other ***REMOVED*** for IE
        var offset;
        ...
        if (!elm.hasCl***REMOVED***('ui-scrollfix') && offset > attrs.uiScrollfix) {
          elm.addCl***REMOVED***('ui-scrollfix');
        } else if (elm.hasCl***REMOVED***('ui-scrollfix') && offset < attrs.uiScrollfix) {
          elm.removeCl***REMOVED***('ui-scrollfix');
        }
      });
    }
  };
 ...
 * @params [options] {object} The configuration options p***REMOVED***ed to $.fn.select2(). Refer to the documentation
 */
...
/*
 ...
 @param [ui-sortable] {object} Options to p***REMOVED*** to $.fn.sortable() merged onto ui.config
*/
 ...
 *
 * @param ui-validate {string|object literal} If strings is p***REMOVED***ed it should be a scope's function to be used as a validator.
 * If an object literal is p***REMOVED***ed a key denotes a validation error key while a value should be a validator function.
...
angular.module('ui.filters').filter('highlight', function () {
  return function (text, search, caseSensitive) {
    if (search || angular.isNumber(search)) {
      ...
      if (caseSensitive) {
        return text.split(search).join('<span cl***REMOVED***="ui-match">' + search + '</span>');
      } else {
        return text.replace(new RegExp(search, 'gi'), '<span cl***REMOVED***="ui-match">$&</span>');
      }
    } else {
      return text;
    }
