...
note: QUnit's eventual addition of an argument to stop/start is ignored in this test suite so that start and stop can be p***REMOVED***ed as callbacks without worrying about their parameters

### Test ***REMOVED***ertions ###
...
and p***REMOVED***es the iFrame's jQuery to the callback.
...
The arguments p***REMOVED***ed to the callback are the same as the
arguments p***REMOVED***ed to window.iframeCallback, whatever that may be
