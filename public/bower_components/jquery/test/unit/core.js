...
*/
	...
	ok( p***REMOVED***, "jQuery('&lt;tag&gt;') needs optional document parameter to ease cross-frame DOM wrangling, see #968" );*/
	...
	equal( jQuery([1,2,3]).get(1), 2, "Test p***REMOVED***ing an array to the factory" );

	equal( jQuery(document.body).get(0), jQuery("body").get(0), "Test p***REMOVED***ing an html node to the factory" );
	...
	equal( elem[0].cl***REMOVED***Name, "test2", "jQuery() quick setter cl***REMOVED***");
...
test( "globalEval", function() {
	...
	ok( window.globalEvalTest, "Test variable ***REMOVED***ignments are global" );
...
test( "isNumeric", function() {
	...
	// It doesn't p***REMOVED*** lint, so disabling until a solution can be found
...
test("jQuery('m***REMOVED***ive html #7990')", function() {
	expect( 3 );
...
test("each(Function)", function() {
	...
	ok( p***REMOVED***, "Execute a function, Relative" );
});
...
test("jQuery.extend(Object, Object)", function() {
	...
	/** @constructor */
	var myKl***REMOVED*** = function() {};
	var customObject = new myKl***REMOVED***();
	var optionsWithCustomObject = { "foo": { "date": customObject } };
	empty = {};
	jQuery.extend(true, empty, optionsWithCustomObject);
	ok( empty["foo"] && empty["foo"]["date"] === customObject, "Custom objects copy correctly (no methods)" );

	// Makes the cl***REMOVED*** a little more realistic
	myKl***REMOVED***.prototype = { "someMethod": function(){} };
	empty = {};
	jQuery.extend(true, empty, optionsWithCustomObject);
...
test("jQuery.makeArray", function(){
	...
	equal( jQuery.makeArray(jQuery("html>*"))[0].nodeName.toUpperCase(), "HEAD", "P***REMOVED*** makeArray a jQuery object" );

	equal( jQuery.makeArray(document.getElementsByName("PWD")).slice(0,1)[0].name, "PWD", "P***REMOVED*** makeArray a nodelist" );

	equal( (function(arg1, arg2){ return jQuery.makeArray(arguments); })(1,2).join(""), "12", "P***REMOVED*** makeArray an arguments array" );

	equal( jQuery.makeArray([1,2,3]).join(""), "123", "P***REMOVED*** makeArray a real array" );

	equal( jQuery.makeArray().length, 0, "P***REMOVED*** nothing to makeArray and expect an empty array" );

	equal( jQuery.makeArray( 0 )[0], 0 , "P***REMOVED*** makeArray a number" );

	equal( jQuery.makeArray( "foo" )[0], "foo", "P***REMOVED*** makeArray a string" );

	equal( jQuery.makeArray( true )[0].constructor, Boolean, "P***REMOVED*** makeArray a boolean" );

	equal( jQuery.makeArray( document.createElement("div") )[0].nodeName.toUpperCase(), "DIV", "P***REMOVED*** makeArray a single node" );

	equal( jQuery.makeArray( {length:2, 0:"a", 1:"b"} ).join(""), "ab", "P***REMOVED*** makeArray an array like map (with length)" );

	ok( !!jQuery.makeArray( document.documentElement.childNodes ).slice(0,1)[0].nodeName, "P***REMOVED*** makeArray a childNodes array" );

	// function, is tricky as it has length
	equal( jQuery.makeArray( function(){ return 1;} )[0](), 1, "P***REMOVED*** makeArray a function" );

	//window, also has length
	equal( jQuery.makeArray(window)[0], window, "P***REMOVED*** makeArray the window" );

	equal( jQuery.makeArray(/a/)[0].constructor, RegExp, "P***REMOVED*** makeArray a regex" );

	ok( jQuery.makeArray(document.getElementById("form")).length >= 13, "P***REMOVED*** makeArray a form (treat as elements)" );
...
test("jQuery.proxy", function(){
	...
	var test3 = function( a, b ){ equal( b, "normal", "Ensure arguments can be pre-applied and p***REMOVED***ed as usual." ); };
...
test("jQuery.parseHTML", function() {
	...
	}, "P***REMOVED***ing an element as the context raises an exception (context should be a document)");
	...
	equal( jQuery.parseHTML( script ).length, 0, "P***REMOVED***ing a script is not allowed by default" );
	raises(function() {
		jQuery(jQuery.parseHTML( script, true )).appendTo("#qunit-fixture");
	}, "P***REMOVED***ing a script is allowed if allowScripts is true");
...
test("jQuery.sub() - Static Methods", function(){
	expect(18);
	var Subcl***REMOVED*** = jQuery.sub();
	Subcl***REMOVED***.extend({
	...
	Subcl***REMOVED***.fn.extend({"subCl***REMOVED***Method": function() { return this;}});

	//Test Simple Subcl***REMOVED***
	ok(Subcl***REMOVED***["topLevelMethod"]() === false, "Subcl***REMOVED***.topLevelMethod thought debug was true");
	ok(Subcl***REMOVED***["config"]["locale"] == "en_US", Subcl***REMOVED***["config"]["locale"] + " is wrong!");
	deepEqual(Subcl***REMOVED***["config"]["test"], undefined, "Subcl***REMOVED***.config.test is set incorrectly");
	equal(jQuery.ajax, Subcl***REMOVED***.ajax, "The subcl***REMOVED*** failed to get all top level methods");

	//Create a SubSubcl***REMOVED***
	var SubSubcl***REMOVED*** = Subcl***REMOVED***.sub();

	//Make Sure the SubSubcl***REMOVED*** inherited properly
	ok(SubSubcl***REMOVED***["topLevelMethod"]() === false, "SubSubcl***REMOVED***.topLevelMethod thought debug was true");
	ok(SubSubcl***REMOVED***["config"]["locale"] == "en_US", SubSubcl***REMOVED***["config"]["locale"] + " is wrong!");
	deepEqual(SubSubcl***REMOVED***["config"]["test"], undefined, "SubSubcl***REMOVED***.config.test is set incorrectly");
	equal(jQuery.ajax, SubSubcl***REMOVED***.ajax, "The subsubcl***REMOVED*** failed to get all top level methods");

	//Modify The Subcl***REMOVED*** and test the Modifications
	SubSubcl***REMOVED***.fn.extend({"subSubCl***REMOVED***Method": function() { return this;}});
	SubSubcl***REMOVED***["setup"]({"locale": "es_MX", "test": "worked"});
	SubSubcl***REMOVED***["debug"] = true;
	SubSubcl***REMOVED***.ajax = function() {return false;};
	ok(SubSubcl***REMOVED***["topLevelMethod"](), "SubSubcl***REMOVED***.topLevelMethod thought debug was false");
	deepEqual(SubSubcl***REMOVED***(document)["subCl***REMOVED***Method"], Subcl***REMOVED***.fn["subCl***REMOVED***Method"], "Methods Differ!");
	ok(SubSubcl***REMOVED***["config"]["locale"] == "es_MX", SubSubcl***REMOVED***["config"]["locale"] + " is wrong!");
	ok(SubSubcl***REMOVED***["config"]["test"] == "worked", "SubSubcl***REMOVED***.config.test is set incorrectly");
	notEqual(jQuery.ajax, SubSubcl***REMOVED***.ajax, "The subsubcl***REMOVED*** failed to get all top level methods");

	//This shows that the modifications to the SubSubCl***REMOVED*** did not bubble back up to it's supercl***REMOVED***
	ok(Subcl***REMOVED***["topLevelMethod"]() === false, "Subcl***REMOVED***.topLevelMethod thought debug was true");
	ok(Subcl***REMOVED***["config"]["locale"] == "en_US", Subcl***REMOVED***["config"]["locale"] + " is wrong!");
	deepEqual(Subcl***REMOVED***["config"]["test"], undefined, "Subcl***REMOVED***.config.test is set incorrectly");
	deepEqual(Subcl***REMOVED***(document)["subSubCl***REMOVED***Method"], undefined, "subSubCl***REMOVED***Method set incorrectly");
	equal(jQuery.ajax, Subcl***REMOVED***.ajax, "The subcl***REMOVED*** failed to get all top level methods");
});
...
test("jQuery.sub() - .fn Methods", function(){
	...
	var Subcl***REMOVED*** = jQuery.sub(),
			Subcl***REMOVED***Subcl***REMOVED*** = Subcl***REMOVED***.sub(),
			jQueryDocument = jQuery(document),
			selectors, contexts, methods, method, arg, description;

	jQueryDocument.toString = function(){ return "jQueryDocument"; };

	Subcl***REMOVED***.fn.subcl***REMOVED***Method = function(){};
	Subcl***REMOVED***Subcl***REMOVED***.fn.subcl***REMOVED***Subcl***REMOVED***Method = function(){};
	...
	jQuery.each(selectors, function(i, selector){
		...
		jQuery.expandedEach({ // all methods that return a new jQuery instance
			...
			jQuery.each(contexts, function(i, context){
				...
				deepEqual(
					(function(var_args){ return jQuery.fn[method].apply(jQuery(selector, context), arguments).subcl***REMOVED***Method; })(arg),
					undefined, "jQuery"+description+" doesn't have Subcl***REMOVED*** methods"
				);
				deepEqual(
					(function(var_args){ return jQuery.fn[method].apply(jQuery(selector, context), arguments).subcl***REMOVED***Subcl***REMOVED***Method; })(arg),
					undefined, "jQuery"+description+" doesn't have Subcl***REMOVED***Subcl***REMOVED*** methods"
				);
				deepEqual(
					Subcl***REMOVED***(selector, context)[method](arg).subcl***REMOVED***Method, Subcl***REMOVED***.fn.subcl***REMOVED***Method,
					"Subcl***REMOVED***"+description+" has Subcl***REMOVED*** methods"
				);
				deepEqual(
					Subcl***REMOVED***(selector, context)[method](arg).subcl***REMOVED***Subcl***REMOVED***Method, undefined,
					"Subcl***REMOVED***"+description+" doesn't have Subcl***REMOVED***Subcl***REMOVED*** methods"
				);
				deepEqual(
					Subcl***REMOVED***Subcl***REMOVED***(selector, context)[method](arg).subcl***REMOVED***Method, Subcl***REMOVED***.fn.subcl***REMOVED***Method,
					"Subcl***REMOVED***Subcl***REMOVED***"+description+" has Subcl***REMOVED*** methods"
				);
				deepEqual(
					Subcl***REMOVED***Subcl***REMOVED***(selector, context)[method](arg).subcl***REMOVED***Subcl***REMOVED***Method, Subcl***REMOVED***Subcl***REMOVED***.fn.subcl***REMOVED***Subcl***REMOVED***Method,
					"Subcl***REMOVED***Subcl***REMOVED***"+description+" has Subcl***REMOVED***Subcl***REMOVED*** methods"
				);
