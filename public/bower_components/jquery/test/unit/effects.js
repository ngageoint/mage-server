...
test("show()", 27, function () {
	...
	jQuery.each(speeds, function(name, speed) {
		...
		ok( p***REMOVED***, "Show with " + name + " does not call animate callback" );
	});
...
test("Overflow and Display", function() {
	...
	var
		testCl***REMOVED*** = jQuery.makeTest("Overflow and Display")
			.addCl***REMOVED***("overflow inline"),
	...
	testCl***REMOVED***.add( testStyle )
		.addCl***REMOVED***("widewidth")
...
jQuery.each({
	"CSS Auto": function( elem, prop ) {
		jQuery( elem ).addCl***REMOVED***( "auto" + prop )
	...
	"CSS 100": function( elem, prop ) {
		jQuery( elem ).addCl***REMOVED***( "large" + prop );
		return "";
	},
	...
	"CSS 50": function( elem, prop ) {
		jQuery( elem ).addCl***REMOVED***( "med" + prop );
		return "";
	},
	...
	"CSS 0": function( elem, prop ) {
		jQuery( elem ).addCl***REMOVED***( "no" + prop );
		return "";
	},
	...
	jQuery.each({
		"show": function( elem, prop ) {
			jQuery( elem ).hide( ).addCl***REMOVED***( "wide" + prop );
			return "show";
		},
		"hide": function( elem, prop ) {
			jQuery( elem ).addCl***REMOVED***( "wide" + prop );
			return "hide";
		},
		...
		"0": function( elem, prop ) {
			jQuery( elem ).addCl***REMOVED***( "noback" );
			return 0;
		}
	}, function( tn, t ) {
...
asyncTest("Effects chaining", function() {
	...
	// We need to p***REMOVED*** jQuery.support.shrinkWrapBlocks for all methods that
	// set overflow hidden (slide* and show/hide with speed)
	setup( ".fadeOut().fadeIn()", "#fadein div" ).fadeOut("fast").fadeIn( "fast", ***REMOVED***ert );
	setup( ".fadeIn().fadeOut()", "#fadeout div" ).fadeIn("fast").fadeOut( "fast", ***REMOVED***ert );
	setup( ".hide().show()", "#show div",  shrinkwrap ).hide("fast").show( "fast", ***REMOVED***ert );
	setup( ".show().hide()", "#hide div",  shrinkwrap ).show("fast").hide( "fast", ***REMOVED***ert );
	setup( ".show().hide(easing)", "#easehide div", shrinkwrap ).show("fast").hide( "fast", "linear", ***REMOVED***ert );
	setup( ".toggle().toggle() - in", "#togglein div", shrinkwrap ).toggle("fast").toggle( "fast", ***REMOVED***ert );
	setup( ".toggle().toggle() - out", "#toggleout div", shrinkwrap ).toggle("fast").toggle( "fast", ***REMOVED***ert );
	setup( ".toggle().toggle(easing) - out", "#easetoggleout div", shrinkwrap ).toggle("fast").toggle( "fast", "linear", ***REMOVED***ert );
	setup( ".slideDown().slideUp()", "#slidedown div", shrinkwrap ).slideDown("fast").slideUp( "fast", ***REMOVED***ert );
	setup( ".slideUp().slideDown()", "#slideup div", shrinkwrap ).slideUp("fast").slideDown( "fast", ***REMOVED***ert );
	setup( ".slideUp().slideDown(easing)", "#easeslideup div", shrinkwrap ).slideUp("fast").slideDown( "fast", "linear", ***REMOVED***ert );
	setup( ".slideToggle().slideToggle() - in", "#slidetogglein div", shrinkwrap ).slideToggle("fast").slideToggle( "fast", ***REMOVED***ert );
	setup( ".slideToggle().slideToggle() - out", "#slidetoggleout div", shrinkwrap ).slideToggle("fast").slideToggle( "fast", ***REMOVED***ert );
	setup( ".fadeToggle().fadeToggle() - in", "#fadetogglein div" ).fadeToggle("fast").fadeToggle( "fast", ***REMOVED***ert );
	setup( ".fadeToggle().fadeToggle() - out", "#fadetoggleout div" ).fadeToggle("fast").fadeToggle( "fast", ***REMOVED***ert );
	setup( ".fadeTo(0.5).fadeTo(1.0, easing)", "#fadeto div" ).fadeTo( "fast", 0.5 ).fadeTo( "fast", 1.0, "linear", ***REMOVED***ert );
});
...
test("callbacks should fire in correct order (#9100)", function() {
	...
	var a = 1,
		...
		$lis = jQuery("<p data-operation='*2'></p><p data-operation='^2'></p>").appendTo("#qunit-fixture")
			// The test will always p***REMOVED*** if no properties are animated or if the duration is 0
