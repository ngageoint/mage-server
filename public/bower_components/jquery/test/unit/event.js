...
test("null or undefined handler", function() {
	...
	try {
		...
		ok(true, "P***REMOVED***ing a null handler will not throw an exception");
	} catch (e) {}

	try {
		jQuery("#firstp").click(undefined);
		ok(true, "P***REMOVED***ing an undefined handler will not throw an exception");
	} catch (e) {}
});
...
test("bind(),live(),delegate() with non-null,defined data", function() {
	...
	var handler = function( event, data ) {
		equal( data, 0, "non-null, defined data (zero) is correctly p***REMOVED***ed" );
	};
...
test("Handler changes and .trigger() order", function() {
	...
	var markup = jQuery(
		"<div><div><p><span><b cl***REMOVED***=\"a\">b</b></span></p></div></div>"
	),
	path = "";
...
test("bind(), with data", function() {
	...
	var handler = function(event) {
		ok( event.data, "bind() with data, check p***REMOVED***ed data exists" );
		equal( event.data["foo"], "bar", "bind() with data, Check value of p***REMOVED***ed data" );
	};
	...
	var handler2 = function(event) {
		equal( event.data, test, "bind() with function data, Check value of p***REMOVED***ed data" );
	};
...
test("bind(), with data, trigger with data", function() {
	expect(4);
	var handler = function(event, data) {
		ok( event.data, "check p***REMOVED***ed data exists" );
		equal( event.data.foo, "bar", "Check value of p***REMOVED***ed data" );
...
test("bind(), namespace with special add", function() {
	...
	jQuery.event.special["test"] = {
		_default: function(e, data) {
			...
			ok( data !== undefined , "And that trigger data was p***REMOVED***ed." );
		},
		setup: function(){},
		teardown: function(){
	...
	div.bind("test.a", {"x": 1}, function(e) {
		ok( !!e.xyz, "Make sure that the data is getting p***REMOVED***ed through." );
...
test("bind/delegate bubbling, isDefaultPrevented", function() {
	...
	$main.delegate("#foo", "click", function(e) {
		...
		if ( typeof(orig.defaultPrevented) === "boolean" || typeof(orig.returnValue) === "boolean" || orig["getPreventDefault"] ) {
			equal( e.isDefaultPrevented(), true, "isDefaultPrevented true p***REMOVED***ed to bubbled event" );

		} else {
			// Opera < 11 doesn't implement any interface we can use, so give it a p***REMOVED***
	...
	$main.delegate("#foo", "click", function(e) {
		equal( e.isDefaultPrevented(), false, "isDefaultPrevented false p***REMOVED***ed to bubbled event" );
	});
	fakeClick( $anchor2 );
	$anchor2.unbind( "click" );
	$main.undelegate( "click" );
...
test("bind(), namespaced events, cloned events", 18, function() {
	...
	jQuery("<a href='#fail' cl***REMOVED***='test'>test</a>").click(function(){ return false; }).appendTo("#qunit-fixture");
...
test("unbind(type)", function() {
	...
	message = "unbind p***REMOVED***ing function";
...
test("trigger(type, [data], [fn])", function() {
	...
	var handler = function(event, a, b, c) {
		equal( event.type, "click", "check p***REMOVED***ed data" );
		equal( a, 1, "check p***REMOVED***ed data" );
		equal( b, "2", "check p***REMOVED***ed data" );
		equal( c, "abc", "check p***REMOVED***ed data" );
		return "test";
	};
	...
	ok( p***REMOVED***, "Trigger focus on hidden element" );
	...
	ok( p***REMOVED***, "Trigger on a table with a colon in the even type, see #3533" );
...
test("trigger(eventObject, [data], [fn])", function() {
	...
	$parent.bind("foo",function(e){
		// Tries bubbling
		equal( e.type, "foo", "Verify event type when p***REMOVED***ed p***REMOVED***ing an event object" );
		equal( e.target.id, "child", "Verify event.target when p***REMOVED***ed p***REMOVED***ing an event object" );
		equal( e.currentTarget.id, "par", "Verify event.currentTarget when p***REMOVED***ed p***REMOVED***ing an event object" );
		equal( e.***REMOVED***, "boo!", "Verify event object's custom attribute when p***REMOVED***ed p***REMOVED***ing an event object" );
	});
	...
	$child.trigger({"type": "foo", "***REMOVED***": "boo!"});

	$parent.unbind();

	function error(){
		ok( false, "This ***REMOVED***ertion shouldn't be reached");
	}
...
test("Delegated events in SVG (#10791)", function() {
	...
	var svg = jQuery(
			...
			'<rect cl***REMOVED***="svg-by-cl***REMOVED***" x="10" y="20" width="100" height="60" r="10" rx="10" ry="10"></rect>'+
	...
	jQuery( "body" )
		...
		.on( "click", "[cl***REMOVED***~='svg-by-cl***REMOVED***']", function() {
			ok( true, "delegated cl***REMOVED*** selector" );
		})
		.find( "#svg-by-id, [cl***REMOVED***~='svg-by-cl***REMOVED***']" )
			.trigger( "click" )
		.end()
		.off( "click" );
...
test("Delegated events in forms (#10844; #11145; #8165; #11382, #11764)", function() {
	...
	var form = jQuery(
			'<form id="myform">'+
				'<input type="text" name="id" value="***REMOVED*** agent man" />'+
			'</form>'
		)
...
test("toggle(Function, Function, ...)", function() {
	...
	jQuery("#firstp").toggle(function () {
		equal(arguments.length, 4, "toggle correctly p***REMOVED***es through additional triggered arguments, see #1701" );
	...
	jQuery("#simon1").one("click", function() {
		...
		jQuery(this).toggle(function() {
			equal( first++, 0, "toggle(Function,Function) ***REMOVED***igned from within one('xxx'), see #1054" );
		}, function() {
			equal( first, 1, "toggle(Function,Function) ***REMOVED***igned from within one('xxx'), see #1054" );
		});
		return false;
	}).click().click().click();
...
test("live with change", function(){
	...
	// test blur/focus on p***REMOVED***word
	var p***REMOVED***word = jQuery("#name"), p***REMOVED***wordChange = 0, oldP***REMOVED***wordVal = p***REMOVED***word.val();
	p***REMOVED***word.live("change", function() {
		p***REMOVED***wordChange++;
	});

	p***REMOVED***word.val(oldP***REMOVED***wordVal + "foo");
	p***REMOVED***word.trigger("change");
	equal( p***REMOVED***wordChange, 1, "Change on p***REMOVED***word input." );

	p***REMOVED***word.val(oldP***REMOVED***wordVal);
	p***REMOVED***word.die("change");
...
test( "delegated event with delegateTarget-relative selector", function() {
	expect(3);
	var markup = jQuery("<ul><li><a id=\"a0\"></a><ul id=\"ul0\"><li cl***REMOVED***=test><a id=\"a0_0\"></a></li><li><a id=\"a0_1\"></a></li></ul></li></ul>").appendTo("#qunit-fixture");
...
(function(){
	...
	function makeHandler( testId ) {
		...
		// the correct arg is being p***REMOVED***ed into the event handler.
		return function( arg ) {
			order.push(testId);
			args[testId] = arg;
		};
	...
	// This ***REMOVED***umes that QUnit tests are run on DOM ready!
	test("jQuery ready", function() {
		...
		equal(args["a"], jQuery, "Argument p***REMOVED***ed to fn in jQuery( fn ) should be jQuery");
		equal(args["b"], jQuery, "Argument p***REMOVED***ed to fn in jQuery(document).ready( fn ) should be jQuery");
		ok(args["c"] instanceof jQuery.Event, "Argument p***REMOVED***ed to fn in jQuery(document).bind( 'ready', fn ) should be an event object");
		...
		equal(args["g"], jQuery, "Argument p***REMOVED***ed to fn in jQuery( fn ) should be jQuery");

		jQuery(document).ready(makeHandler("h"));
		equal(order.pop(), "h", "Event handler should execute immediately");
		equal(args["h"], jQuery, "Argument p***REMOVED***ed to fn in jQuery(document).ready( fn ) should be jQuery");

		jQuery(document).bind("ready.readytest", makeHandler("never"));
		equal(order.length, 0, "Event handler should never execute since DOM ready has already p***REMOVED***ed");
