...
//----------- jQuery.ajax()
	...
	asyncTest( "jQuery.ajax(), jQuery.get[Script|JSON](), jQuery.post(), p***REMOVED***-through request object", 8, function() {
		...
		jQuery( document ).on( "ajaxError.p***REMOVED***thru", function( e, xml, s, ex ) {
			errorCount++;
			errorEx += ": " + xml.status;
		});
		jQuery( document ).one( "ajaxStop", function() {
			...
			jQuery( document ).off("ajaxError.p***REMOVED***thru");
			start();
		});
		Globals.register("testBar");
	...
	ajaxTest( "#13292 - jQuery.ajax() - converter is byp***REMOVED***ed for 204 requests", 3, {
...
//----------- jQuery.ajaxSetup()
	...
	asyncTest( "jQuery.ajaxSetup({ timeout: Number }) - with global timeout", 2, function() {
		var p***REMOVED***ed = 0,
			p***REMOVED*** = function() {
				ok( p***REMOVED***ed++ < 2, "Error callback executed" );
				if ( p***REMOVED***ed == 2 ) {
		...
		jQuery( document ).on( "ajaxError.setupTest", p***REMOVED*** );
...
//----------- jQuery.fn.load()
	...
	asyncTest( "jQuery.fn.load( String, Object, Function )", 2, function() {
		jQuery("<div />").load( url("data/params_html.php"), {
			...
			strictEqual( $post.find("#foo").text(), "3", "Check if a hash of data is p***REMOVED***ed correctly" );
			strictEqual( $post.find("#bar").text(), "ok", "Check if a hash of data is p***REMOVED***ed correctly" );
			start();
		});
	});
	...
	asyncTest( "jQuery.fn.load( String, String, Function )", 2, function() {
		jQuery("<div />").load( url("data/params_html.php"), "foo=3&bar=ok", function() {
			...
			strictEqual( $get.find("#foo").text(), "3", "Check if a string of data is p***REMOVED***ed correctly" );
			strictEqual( $get.find("#bar").text(), "ok", "Check if a   of data is p***REMOVED***ed correctly" );
			start();
		});
	});
