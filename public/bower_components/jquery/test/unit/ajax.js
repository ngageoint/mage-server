...
test("p***REMOVED***-through request object", function() {
	expect(8);
	stop();
...
/*
 * Test disabled.
 * The ***REMOVED***ertions expect that the p***REMOVED***ed-in object will be modified,
...
test("load(String, Object, Function)", function() {
	...
	jQuery("<div />").load(url("data/params_html.php"), { "foo": 3, "bar": "ok" }, function() {
		...
		equal( $post.find("#foo").text(), "3", "Check if a hash of data is p***REMOVED***ed correctly");
		equal( $post.find("#bar").text(), "ok", "Check if a hash of data is p***REMOVED***ed correctly");
		start();
	});
});
...
test("load(String, String, Function)", function() {
	...
	jQuery("<div />").load(url("data/params_html.php"), "foo=3&bar=ok", function() {
		...
		equal( $get.find("#foo").text(), "3", "Check if a string of data is p***REMOVED***ed correctly");
		equal( $get.find("#bar").text(), "ok", "Check if a	 of data is p***REMOVED***ed correctly");
		start();
	});
});
...
test("jQuery.ajaxSetup({timeout: Number}) - with global timeout", function() {
	...
	jQuery("#qunit-fixture").ajaxError(p***REMOVED***);
