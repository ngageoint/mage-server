  ...
  jQuery(function() {
    for(i = 0; i < jQuery.benchmarker.tests.length; i++) {
      jQuery("tbody").append("<tr><td cl***REMOVED***='test'>" + jQuery.benchmarker.tests[i] + "</td></tr>");
    }
    ...
    jQuery("#addTest").bind("click", function() {
      ...
      jQuery("tbody tr:last button").bind("click", function() {
        ...
        td.html("<button>-</button>" + jQuery(this).prev().val()).addCl***REMOVED***("test");
    ...
    var wlfooters = "";
    for(i = 0; i < jQuery.benchmarker.libraries.length; i++)
      wlfooters += "<td><span cl***REMOVED***='wins'>W</span> / <span cl***REMOVED***='fails'>F</span></th>"
   ...
   benchmark = function(list, times, libraries) {
     if(list[0]) {
       ...
       var code = jQuery(el).text().replace(/^-/, "");
         ...
         try {
           ...
           if(((jqRes.length == 0) && (libRes.length != 0)) ||
             ...
             jQuery(el).parent().append("<td cl***REMOVED***='fail'>FAIL</td>");
           }
         } catch(e) {
           jQuery(el).parent().append("<td cl***REMOVED***='fail'>FAIL</td>");
         }
       ...
       jQuery("tbody tr").each(function() {
         ...
         if(winners.length == 1) winners.addCl***REMOVED***("winner");
         else winners.addCl***REMOVED***("tie");
       });
 ...
 function count() {
   for(i = 3; i <= jQuery.benchmarker.libraries.length + 2 ; i++) {
     ...
     jQuery("tfoot tr:first th:eq(" + (i - 1) + ")")
      .html("<span cl***REMOVED***='wins'>" + wins + "</span> / <span cl***REMOVED***='fails'>" + fails + "</span>");
   }
 }
