 ...
 * Added **generic toolbar cl***REMOVED***es** for reuse by plugin developers (used by zoom control).
 ...
 * Fixed a bug where static properties of a child cl***REMOVED*** would not override the parent ones.
 ...
 * Added ability to p***REMOVED*** Geolocation options to map `locate` and `locateAndSetView` methods (by [@JasonSanford](https://github.com/JasonSanford)).
