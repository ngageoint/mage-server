...
If you have a new cl***REMOVED***, put it directly in the `L` namespace (`L.MyPlugin`).<br>
If you inherit one of the existing cl***REMOVED***es, make it a sub-property (`L.TileLayer.Banana`).<br>
If you want to add new methods to existing Leaflet cl***REMOVED***es, you can do it like this: `L.Marker.include({myPlugin: …})`.

Function, method and property names should be in `camelCase`.<br>
Cl***REMOVED*** names should be in `CapitalizedCamelCase`.
