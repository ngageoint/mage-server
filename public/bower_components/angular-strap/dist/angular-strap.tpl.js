...
angular.module('mgcrea.ngStrap.alert').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('alert/alert.tpl.html', '<div cl***REMOVED***="alert" ng-cl***REMOVED***="[type ? \'alert-\' + type : null]"><button type="button" cl***REMOVED***="close" ng-if="dismissable" ng-click="$hide()">&times;</button> <strong ng-bind="title"></strong>&nbsp;<span ng-bind-html="content"></span></div>');
...
angular.module('mgcrea.ngStrap.dropdown').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('dropdown/dropdown.tpl.html', '<ul tabindex="-1" cl***REMOVED***="dropdown-menu" role="menu"><li role="presentation" ng-cl***REMOVED***="{divider: item.divider}" ng-repeat="item in content"><a role="menuitem" tabindex="-1" ng-href="{{item.href}}" ng-if="!item.divider && item.href" target="{{item.target || \'\'}}" ng-bind="item.text"></a> <a role="menuitem" tabindex="-1" href="javascript:void(0)" ng-if="!item.divider && item.click" ng-click="$eval(item.click);$hide()" ng-bind="item.text"></a></li></ul>');
...
angular.module('mgcrea.ngStrap.timepicker').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('timepicker/timepicker.tpl.html', '<div cl***REMOVED***="dropdown-menu timepicker" style="min-width: 0px;width: auto"><table height="100%"><thead><tr cl***REMOVED***="text-center"><th><button tabindex="-1" type="button" cl***REMOVED***="btn btn-default pull-left" ng-click="$arrowAction(-1, 0)"><i cl***REMOVED***="{{ $iconUp }}"></i></button></th><th>&nbsp;</th><th><button tabindex="-1" type="button" cl***REMOVED***="btn btn-default pull-left" ng-click="$arrowAction(-1, 1)"><i cl***REMOVED***="{{ $iconUp }}"></i></button></th></tr></thead><tbody><tr ng-repeat="(i, row) in rows"><td cl***REMOVED***="text-center"><button tabindex="-1" style="width: 100%" type="button" cl***REMOVED***="btn btn-default" ng-cl***REMOVED***="{\'btn-primary\': row[0].selected}" ng-click="$select(row[0].date, 0)" ng-disabled="row[0].disabled"><span ng-cl***REMOVED***="{\'text-muted\': row[0].muted}" ng-bind="row[0].label"></span></button></td><td><span ng-bind="i == midIndex ? timeSeparator : \' \'"></span></td><td cl***REMOVED***="text-center"><button tabindex="-1" ng-if="row[1].date" style="width: 100%" type="button" cl***REMOVED***="btn btn-default" ng-cl***REMOVED***="{\'btn-primary\': row[1].selected}" ng-click="$select(row[1].date, 1)" ng-disabled="row[1].disabled"><span ng-cl***REMOVED***="{\'text-muted\': row[1].muted}" ng-bind="row[1].label"></span></button></td><td ng-if="showAM">&nbsp;</td><td ng-if="showAM"><button tabindex="-1" ng-show="i == midIndex - !isAM * 1" style="width: 100%" type="button" ng-cl***REMOVED***="{\'btn-primary\': !!isAM}" cl***REMOVED***="btn btn-default" ng-click="$switchMeridian()" ng-disabled="el.disabled">AM</button> <button tabindex="-1" ng-show="i == midIndex + 1 - !isAM * 1" style="width: 100%" type="button" ng-cl***REMOVED***="{\'btn-primary\': !isAM}" cl***REMOVED***="btn btn-default" ng-click="$switchMeridian()" ng-disabled="el.disabled">PM</button></td></tr></tbody><tfoot><tr cl***REMOVED***="text-center"><th><button tabindex="-1" type="button" cl***REMOVED***="btn btn-default pull-left" ng-click="$arrowAction(1, 0)"><i cl***REMOVED***="{{ $iconDown }}"></i></button></th><th>&nbsp;</th><th><button tabindex="-1" type="button" cl***REMOVED***="btn btn-default pull-left" ng-click="$arrowAction(1, 1)"><i cl***REMOVED***="{{ $iconDown }}"></i></button></th></tr></tfoot></table></div>');
...
angular.module('mgcrea.ngStrap.typeahead').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('typeahead/typeahead.tpl.html', '<ul tabindex="-1" cl***REMOVED***="typeahead dropdown-menu" ng-show="$isVisible()" role="select"><li role="presentation" ng-repeat="match in $matches" ng-cl***REMOVED***="{active: $index == $activeIndex}"><a role="menuitem" tabindex="-1" ng-click="$select($index, $event)" ng-bind="match.label"></a></li></ul>');
