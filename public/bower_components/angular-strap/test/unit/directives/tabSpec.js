...
describe('tab', function () {
  ...
  var templates = {
    'default': {
      element: '<div bs-tabs><div cl***REMOVED***="active" data-tab="\'Home\'"><p>A</p></div><div data-tab="\'Profile\'"><p>B</p></div><div data-tab="tab.title"><p>{{tab.content}}</p></div></div>',
    ...
    'repeat': {
      element: '<div bs-tabs><div ng-repeat="(i, tab) in tabs" ng-cl***REMOVED***="{active:!i}" data-tab="tab.title"><p>{{tab.content}}</p></div></div>',
      scope: {tabs: [
    ...
    // },
    'ngModel': {
      element: '<div ng-model="tab.active" bs-tabs><div cl***REMOVED***="active" data-tab="\'Home\'"><p>A</p></div><div data-tab="\'Profile\'"><p>B</p></div><div data-tab="\'About\'"><p>C</p></div></div>',
      scope: {tab: {active: 1}}
    }
  };
  ...
  it('should correctly switch tabs', function() {
    var elm = compileDirective();
    expect(elm.find('ul.nav-tabs li:first').hasCl***REMOVED***('active')).toBe(true);
    expect(elm.find('div.tab-content div.active').text()).toBe('A');
    elm.find('ul.nav-tabs li:nth-child(2) a').trigger('click');
    expect(elm.find('ul.nav-tabs li:nth-child(2)').hasCl***REMOVED***('active')).toBe(true);
    expect(elm.find('div.tab-content div.active').text()).toBe('B');
  });

  it('should correctly switch tabs with fade cl***REMOVED***', function() {
