...
describe('alert', function () {
  ...
  var templates = {
    'default': {
      ...
      element: '<div cl***REMOVED***="alert fade in" bs-alert="alert"></div>'
    },
    'alertStack': {
      scope: {alerts:[{type:'error', title: 'Holy guacamole!', content: 'Hello Alert, <pre>2 + 3 = {{ 2 + 3 }}</pre>'}]},
      element: '<div cl***REMOVED***="alerts"><div cl***REMOVED***="alert" ng-repeat="alert in alerts" bs-alert="alert"></div></div>'
    },
    'deepAlertStack': {
      scope: {deep:{alerts:[{type:'error', title: 'Holy guacamole!', content: 'Hello Alert, <pre>2 + 3 = {{ 2 + 3 }}</pre>'}]}},
      element: '<div cl***REMOVED***="alert"><div cl***REMOVED***="alert" ng-repeat="alert in deep.alerts" bs-alert="alert"></div></div>'
    }
  };
  ...
  it('should correctly build the alert', function () {
    ...
    var expected = '<button type="button" cl***REMOVED***="close" data-dismiss="alert">×</button>' +
    '<strong cl***REMOVED***="ng-scope">Holy guacamole!</strong>' +
    '<span cl***REMOVED***="ng-scope">&nbsp;Hello Alert, </span><pre cl***REMOVED***="ng-scope ng-binding">2 + 3 = 5</pre>';
    expect(elm.html()).toBe(expected);
  });

  it('should correctly update the alert cl***REMOVED***', function () {
    var elm = compileDirective();
    scope.alert.type = 'info';
    scope.$digest();
    expect(elm.hasCl***REMOVED***('error')).toBe(false);
    expect(elm.hasCl***REMOVED***('info')).toBe(false);
  });
