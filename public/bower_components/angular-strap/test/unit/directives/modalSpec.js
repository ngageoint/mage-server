...
describe('modal', function () {
  ...
  beforeEach(inject(function ($injector, $rootScope, _$compile_, _$timeout_, _$httpBackend_) {
    ...
    scope.modal = 'Hello <span ng-bind-html-u***REMOVED***fe="content"></span>';
  ...
  var templates = {
    'default': '<a cl***REMOVED***="btn" bs-modal="\'partials/modal.html\'"></a>',
    'extra': '<a cl***REMOVED***="btn" bs-modal="\'partials/modal.html\'" data-backdrop="0" data-keyboard="0" data-modal-cl***REMOVED***="modal-wide"></a>',
    'cached': '<script type="text/ng-template" id="cached-modal">' + 'Hello <span ng-bind-html-u***REMOVED***fe="content"></span>' + '</script><a cl***REMOVED***="btn" bs-modal="\'cached-modal\'" data-unique="1" data-title="aTitleBis" data-placement="left"></a>'
  };
  ...
  it('should fetch the partial and build the modal', function () {
    ...
    expect($modal.hasCl***REMOVED***('modal')).toBe(true);
  ...
  it('should handle extra attributes', function () {
    ...
    expect($modal.hasCl***REMOVED***('modal-wide')).toBe(true);
  });
  ...
  it('should show the modal on click', function(/*done*/) {
    var elm = compileDirective();
    var $modal = $(elm.attr('data-target'));
    expect($modal.hasCl***REMOVED***('hide')).toBe(true);
    elm.trigger('click');
    /*setTimeout(function() {
      dump($('body').html());
      expect($modal.hasCl***REMOVED***('in')).toBe(true); //@fixme
      expect($modal.hasCl***REMOVED***('hide')).toBe(false); //@fixme
      done();
    }, 100);*/
  });
