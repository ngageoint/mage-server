...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    wrapper.cl***REMOVED***Name = "CodeMirror" + (options.lineWrapping ? " CodeMirror-wrap" : "");
    // This mess creates the base DOM structure for the editor.
    wrapper.innerHTML =
      '<div style="overflow: hidden; position: relative; width: 3px; height: 0px;">' + // Wraps and hides input textarea
        '<textarea style="position: absolute; padding: 0; width: 1px; height: 1em" wrap="off" ' +
          'autocorrect="off" autocapitalize="off"></textarea></div>' +
      '<div cl***REMOVED***="CodeMirror-scrollbar">' + // The vertical scrollbar. Horizontal scrolling is handled by the scroller itself.
        '<div cl***REMOVED***="CodeMirror-scrollbar-inner">' + // The empty scrollbar content, used solely for managing the scrollbar thumb.
      '</div></div>' + // This must be before the scroll area because it's float-right.
      '<div cl***REMOVED***="CodeMirror-scroll" tabindex="-1">' +
        '<div style="position: relative">' + // Set to the height of the text, causes scrolling
          '<div style="position: relative">' + // Moved around its parent to cover visible view
            '<div cl***REMOVED***="CodeMirror-gutter"><div cl***REMOVED***="CodeMirror-gutter-text"></div></div>' +
            // Provides positioning relative to (visible) text origin
            '<div cl***REMOVED***="CodeMirror-lines"><div style="position: relative; z-index: 0">' +
              // Used to measure text size
              '<div style="position: absolute; width: 100%; height: 0; overflow: hidden; visibility: hidden;"></div>' +
              '<pre cl***REMOVED***="CodeMirror-cursor">&#160;</pre>' + // Absolutely positioned blinky cursor
              '<pre cl***REMOVED***="CodeMirror-cursor" style="visibility: hidden">&#160;</pre>' + // Used to force a width
    ...
    if (mac_geLion) {
      scrollbar.cl***REMOVED***Name += (overlapScrollbars() ? " cm-sb-overlap" : " cm-sb-nonoverlap");
    } else if (ie_lt8) {
      // Need to set a minimum width to see the scrollbar on IE7 (but must not set it on IE8).
      scrollbar.cl***REMOVED***Name += " cm-sb-ie7";
    }
    ...
    var instance = wrapper.CodeMirror = {
      ...
      setLineCl***REMOVED***: operation(setLineCl***REMOVED***),
      ...
      toggleOverwrite: function() {
        if(overwrite){
          overwrite = false;
          cursor.cl***REMOVED***Name = cursor.cl***REMOVED***Name.replace(" CodeMirror-overwrite", "");
        } else {
          overwrite = true;
          cursor.cl***REMOVED***Name += " CodeMirror-overwrite";
        }
      },
    ...
    function onFocus() {
      ...
      if (!focused) {
        ...
        focused = true;
        if (scroller.cl***REMOVED***Name.search(/\bCodeMirror-focused\b/) == -1)
          scroller.cl***REMOVED***Name += " CodeMirror-focused";
    ...
    function onBlur() {
      if (focused) {
        ...
        scroller.cl***REMOVED***Name = scroller.cl***REMOVED***Name.replace(" CodeMirror-focused", "");
      }
    ...
    function overlapScrollbars() {
      ...
      tmpSb.cl***REMOVED***Name = "CodeMirror-scrollbar";
      tmpSb.style.cssText = "position: absolute; left: -9999px; height: 100px;";
      tmpSbInner.cl***REMOVED***Name = "CodeMirror-scrollbar-inner";
    ...
    function updateDisplay(changes, suppressCallback, scrollTop) {
      ...
      // only ***REMOVED***ertion in the whole file.
    ...
    function patchDisplay(from, to, intact) {
      // The first p***REMOVED*** removes the DOM nodes that aren't intact.
      ...
      // This p***REMOVED*** fills in the lines that actually changed.
      ...
      doc.iter(from, to, function(line) {
        ...
        if (!nextIntact || nextIntact.from > j) {
          if (line.hidden) var html = scratch.innerHTML = "<pre></pre>";
            ...
            var html = '<pre' + (line.cl***REMOVED***Name ? ' cl***REMOVED***="' + line.cl***REMOVED***Name + '"' : '') + '>'
              + line.getHTML(makeTab) + '</pre>';
            // Kludge to make sure the styled element lies behind the selection (by z-index)
            if (line.bgCl***REMOVED***Name)
              html = '<div style="position: relative"><pre cl***REMOVED***="' + line.bgCl***REMOVED***Name +
      ...
      doc.iter(showingFrom, Math.max(showingTo, showingFrom + 1), function(line) {
        if (line.hidden) {
          ...
          if (marker && marker.text)
            ...
            text = "\u00a0";
          html.push((marker && marker.style ? '<pre cl***REMOVED***="' + marker.style + '">' : "<pre>"), text);
      ...
      if (collapsed) {
        ...
        function add(left, top, right, height) {
          var rstyle = quirksMode ? "width: " + (!right ? clientWidth : clientWidth - right - left) + "px"
                                  : "right: " + right + "px";
          html += '<div cl***REMOVED***="CodeMirror-selected" style="position: absolute; left: ' + left +
    ...
    function indentLine(n, how) {
      ...
      if (how == "smart") {
        ...
        if (indentation == P***REMOVED***) how = "prev";
      }
      if (how == "prev") {
    ...
    function wrappingChanged(from, to) {
      if (options.lineWrapping) {
        wrapper.cl***REMOVED***Name += " CodeMirror-wrap";
        ...
        wrapper.cl***REMOVED***Name = wrapper.cl***REMOVED***Name.replace(" CodeMirror-wrap", "");
        maxLine = ""; maxLineChanged = true;
        doc.iter(0, doc.size, function(line) {
    ...
    function makeTab(col) {
      ...
      for (var str = '<span cl***REMOVED***="cm-tab">', i = 0; i < w; ++i) str += " ";
      return (tabCache[w] = {html: str + "</span>", width: w});
    }
    function themeChanged() {
      scroller.cl***REMOVED***Name = scroller.cl***REMOVED***Name.replace(/\s*cm-s-\S+/g, "") +
        options.theme.replace(/(^|\s)\s*/g, " cm-s-");
    }
    function keyMapChanged() {
      var style = keyMap[options.keyMap].style;
      wrapper.cl***REMOVED***Name = wrapper.cl***REMOVED***Name.replace(/\s*cm-keymap-\S+/g, "") +
    ...
    function markText(from, to, cl***REMOVED***Name) {
      from = clipPos(from); to = clipPos(to);
      var tm = new TextMarker();
      if (!posLess(from, to)) return tm;
      function add(line, from, to, cl***REMOVED***Name) {
        getLine(line).addMark(new MarkedText(from, to, cl***REMOVED***Name, tm));
      }
      if (from.line == to.line) add(from.line, from.ch, to.ch, cl***REMOVED***Name);
      else {
        add(from.line, from.ch, null, cl***REMOVED***Name);
        for (var i = from.line + 1, e = to.line; i < e; ++i)
          add(i, null, null, cl***REMOVED***Name);
        add(to.line, null, to.ch, cl***REMOVED***Name);
      }
    ...
    function addGutterMarker(line, text, cl***REMOVED***Name) {
      if (typeof line == "number") line = getLine(clipLine(line));
      line.gutterMarker = {text: text, style: cl***REMOVED***Name};
      gutterDirty = true;
      return line;
    }
    ...
    function setLineCl***REMOVED***(handle, cl***REMOVED***Name, bgCl***REMOVED***Name) {
      return changeLine(handle, function(line) {
        if (line.cl***REMOVED***Name != cl***REMOVED***Name || line.bgCl***REMOVED***Name != bgCl***REMOVED***Name) {
          line.cl***REMOVED***Name = cl***REMOVED***Name;
          line.bgCl***REMOVED***Name = bgCl***REMOVED***Name;
          return true;
        }
      });
    }
    ...
    function lineInfo(line) {
      ...
      return {line: n, handle: line, text: line.text, markerText: marker && marker.text,
              markerCl***REMOVED***: marker && marker.style, lineCl***REMOVED***: line.cl***REMOVED***Name, bgCl***REMOVED***: line.bgCl***REMOVED***Name};
    }
  ...
  function MarkedText(from, to, cl***REMOVED***Name, marker) {
    this.from = from; this.to = to; this.style = cl***REMOVED***Name; this.marker = marker;
  }
  ...
  function Line(text, styles) {
    ...
    this.text = text;
    this.height = 1;
    this.marked = this.gutterMarker = this.cl***REMOVED***Name = this.bgCl***REMOVED***Name = this.handlers = null;
  ...
  Line.prototype = {
    ...
    getHTML: function(makeTab, wrapAt, wrapId, wrapWBR) {
      ...
      function span_(text, style) {
        ...
        if (style) html.push('<span cl***REMOVED***="', style, '">', escaped, "</span>");
      ...
      function styleToCl***REMOVED***(style) {
      ...
      if (!allText && wrapAt == null) {
        ...
        for (var i = 0, ch = 0; ch < len; i+=2) {
          ...
          ch += l;
          span(str, styleToCl***REMOVED***(style));
        }
      } else {
        ...
        while (pos < len) {
          ...
          while (true) {
            ...
            text = st[i++]; style = styleToCl***REMOVED***(st[i++]);
          }
        }
      }
      return html.join("");
  ...
  var P***REMOVED*** = CodeMirror.P***REMOVED*** = {toString: function(){return "CodeMirror.P***REMOVED***";}};
