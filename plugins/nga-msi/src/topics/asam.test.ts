...
type DateCtorOnly = new (value?: string | number | Date) => Date
...
describe('asam topic module', function () {
  ...
  describe('creating requests', function () {
    ...
    let dateSpy: jest.SpiedClass<DateConstructor>
    type DateMockFn = (value?: string | number | Date) => Date
    ...
    it('creates a request', function () {
      ...
      dateSpy.mockImplementation(
        ((value?: string | number | Date) => {
          if (value === undefined) return dateNow
          return new RealDate(value as any)
        }) as unknown as DateMockFn
      )      
      ...
      expect(dateSpy).toHaveBeenCalledWith(
        dateNow.getTime() - params.newerThanDays! * 24 * 60 * 60 * 1000
      )
    })
      ...
      asam: [
        {
          ...
          victim: null,
          description:
            'On 10 June at 0030LT, a duty crew member spotted two robbers armed with a knife and pipe on the aft deck of an anchored bulk carrier at Macapa Anchorage at 00:20N - 050:58W. When the alarm was raised, the robbers fled with some ship�??s property. The crew reported the incident to both the port authority and local agent.'
        },
        {
          reference: '2020-223',
          date: '2020-07-01',
          ...
          victim: null,
          description:
            "FOUR ROBBERS ARMED WITH KNIVES BOARDED A REEFER VESSEL. DUTY CREW SPOTTED THE ROBBERS AND RAISED THE ALARM. ALL CREW MUSTERED. SEEING THE CREW ALERTNESS, THE ROBBERS ESCAPED WITH SHIP'S SPARES. INCIDENT REPORTED TO VTIS."
        },
        {
          reference: '2020-232',
          date: '2020-06-29',
        ...
        },
        {
          ...
          description:
            'On 24 June at 1530LT, armed pirates boarded and kidnapped six crew (5 South Korean and 1 Ghanaian) from the  Ghana-flagged purse seiner PANOFI FRONTIER at position 05:21 N - 002:37E, 61 NM south of Cotonou. The pirates fled in an easterly direction toward Nigerian waters.'
        }
      ]
    }

    it('transforms the asam response', function () {
      ...
      expect(content.items).toEqual(expected)
    })
  })
})
