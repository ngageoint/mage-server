...
function next() {
  ...
  dicer.on('error', function(err) {
    ...
    ***REMOVED***ert(finishes++ === 0, makeMsg(v.what, 'finish emitted multiple times'));
    ...
    if (v.dicerError)
      ***REMOVED***ert(error !== undefined, makeMsg(v.what, 'Expected error'));
    else
      ***REMOVED***ert(error === undefined, makeMsg(v.what, 'Unexpected error'));
    ...
    if (v.events && v.events.indexOf('preamble') > -1) {
      ...
      ***REMOVED***ert.deepEqual(state.preamble,
                       preamble,
                       makeMsg(v.what,
                               'Preamble mismatch:\nActual:'
    ...
    if (v.events && v.events.indexOf('part') > -1) {
      ***REMOVED***ert.equal(state.parts.length,
                   v.nparts,
                   makeMsg(v.what,
                           'Part count mismatch:\nActual: '
                           + state.parts.length
      ...
      ***REMOVED***ert.equal(partErrors,
      ...
      for (var i = 0, header, body; i < v.nparts; ++i) {
        if (fs.existsSync(fixtureBase + '/part' + (i+1))) {
          ...
          body = undefined;
        ***REMOVED***ert.deepEqual(state.parts[i].body,
                         body,
                         makeMsg(v.what,
        ...
        if (fs.existsSync(fixtureBase + '/part' + (i+1) + '.header')) {
          ...
          header = undefined;
        ***REMOVED***ert.deepEqual(state.parts[i].header,
                         header,
                         makeMsg(v.what,
                                 'Part #' + (i+1)
