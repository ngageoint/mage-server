...
wows.describe('XML Serializer').addBatch({
  'text node containing "]]>"': function() {
    ...
    console.***REMOVED***ert(doc.documentElement.firstChild.toString() == 'hello ]]> there',doc.documentElement.firstChild.toString());
  },
  '<script> element with no children': function() {
    var doc = new DOMParser().parseFromString('<html><script></script></html>', 'text/html');
    console.***REMOVED***ert(doc.documentElement.firstChild.toString() == '<script></script>');
  },
}).run();
