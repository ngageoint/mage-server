...
wows.describe('XML Node Parse').addBatch({
    "big file parse":function(){
		...
		var domjsTime = t3-t2;
		console.***REMOVED***ert(domjsTime>xmldomTime,'xmldom performance must more height!!')
		...
		//***REMOVED***ert.equal(xmldomresult,domjsresult);
		...
		if(xmldomresult !== domjsresult){
			for(var i=0;i<xmldomresult.length;i++){
				if(xmldomresult.charAt(i)!=domjsresult.charAt(i)){
					...
					console.log('pos'+i,'\n',xmldomresult,'\n\n\n\n',domjsresult)
					console.***REMOVED***ert(xmldomresult == domjsresult)
					break;
				}
			} 
			
		}
		//console.***REMOVED***ert(xmldomresult == domjsresult,xmldomresult.length,i)
    }
}).run(); // Run it
