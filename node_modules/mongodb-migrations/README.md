...
File name can be p***REMOVED***ed through the means of `--config` parameter.
...
* `p***REMOVED***word` _[optional]_ — MongoDB p***REMOVED***word when authentication is required
...
It will also print any custom messages you p***REMOVED*** to
`this.log` inside of the `up` / `down` function.

To suppress this logging p***REMOVED*** `customLogFn = null` to the
`Migrator` constructor (`undefined` won't do the trick).

If you want to handle the logging on your own (save it to file, or
whatever else) you can p***REMOVED*** your custom function having this signature:
...
The `results` object is always p***REMOVED***ed (even in case of error).
...
you can roll back all recently ran tra***REMOVED***ctions. Currently this
...
p***REMOVED***ed the error object in case of error,
...
responsibility to ***REMOVED***ure it's unique.
