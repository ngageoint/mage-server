...
var authenticate = function(db, username, p***REMOVED***word, options, callback) {
  ...
  // Create payload
  var payload = new Binary(format("\x00%s\x00%s", username, p***REMOVED***word));
    ...
    db._executeQueryCommand(DbCommand.createDbCommand(db, command, {}, '$external'), {connection:connection}, function(err, result) {
      ...
      if(numberOfConnections <= 0 && typeof callback == 'function') {
        ...
        if(errorObject == null && result.documents[0].ok == 1) {
          ...
          db.serverConfig.auth.add('PLAIN', db.databaseName, username, p***REMOVED***word);
          // Return callback
