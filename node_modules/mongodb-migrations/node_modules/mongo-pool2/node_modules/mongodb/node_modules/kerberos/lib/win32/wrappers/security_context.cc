...
Handle<Value> SecurityContext::InitializeContextSync(const Arguments &args) {
  HandleScope scope;  
  char ****REMOVED***_principal_name_str = NULL, *input_str = NULL, *decoded_input_str = NULL;
  ...
  if(args.Length() != 3)
    return VException("Initialize must be called with either [credential:SecurityCredential, ***REMOVED***PrincipalName:string, input:string]");
  ...
  Local<String> ***REMOVED***_principal_name = args[1]->ToString();
  ***REMOVED***_principal_name_str = (char *)calloc(***REMOVED***_principal_name->Utf8Length() + 1, sizeof(char));
  ***REMOVED***_principal_name->WriteUtf8(***REMOVED***_principal_name_str);
  ...
  //
  // Prepare the data we are p***REMOVED***ing to the SSPI method
  if(input->Utf8Length() > 0) {
  ...
  status = _sspi_initializeSecurityContext(
    ...
    , NULL
    , const_cast<TCHAR*>(***REMOVED***_principal_name_str)
    , 0x02  // MUTUAL
    , 0
    , 0     // Network
...
typedef struct SecurityContextStaticInitializeCall {
  char ****REMOVED***_principal_name_str;
...
static void _initializeContext(Worker *worker) {
  ...
  status = _sspi_initializeSecurityContext(
    ...
    , NULL
    , const_cast<TCHAR*>(call->***REMOVED***_principal_name_str)
    , 0x02  // MUTUAL
    , 0
    , 0     // Network
  ...
  if(call->***REMOVED***_principal_name_str != NULL) free(call->***REMOVED***_principal_name_str);
}
...
Handle<Value> SecurityContext::InitializeContext(const Arguments &args) {
  ...
  // We need 3 parameters
  if(args.Length() != 4)
    return VException("Initialize must be called with [credential:SecurityCredential, ***REMOVED***PrincipalName:string, input:string, callback:function]");
  ...
  ***REMOVED***_principal_name->WriteUtf8(***REMOVED***_principal_name_str);
  ...
  call->***REMOVED***_principal_name_str = ***REMOVED***_principal_name_str;
...
Handle<Value> SecurityContext::InitalizeStep(const Arguments &args) {
  ...
  // We need 3 parameters
  if(args.Length() != 3)
    return VException("Initialize must be called with [***REMOVED***PrincipalName:string, input:string, callback:function]");
  ...
  // Let's unpack the values
  Local<String> ***REMOVED***_principal_name = args[0]->ToString();
  ***REMOVED***_principal_name_str = (char *)calloc(***REMOVED***_principal_name->Utf8Length() + 1, sizeof(char));
  ***REMOVED***_principal_name->WriteUtf8(***REMOVED***_principal_name_str);
...
Handle<Value> SecurityContext::InitalizeStepSync(const Arguments &args) {
  ...
  // We need 3 parameters
  if(args.Length() != 2)
    return VException("Initialize must be called with [***REMOVED***PrincipalName:string, input:string]");
...
void SecurityContext::Initialize(Handle<Object> target) {
  ...
  constructor_template->SetCl***REMOVED***Name(String::NewSymbol("SecurityContext"));
