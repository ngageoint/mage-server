...
void Kerberos::Initialize(v8::Handle<v8::Object> target) {
  ...
  constructor_template->SetCl***REMOVED***Name(String::NewSymbol("Kerberos"));
...
Handle<Value> Kerberos::AuthGSSClientInit(const Arguments &args) {
  ...
  // Ensure valid call
  if(args.Length() != 3) return VException("Requires a ***REMOVED*** string uri, integer flags and a callback function");
  if(args.Length() == 3 && !args[0]->IsString() && !args[1]->IsInt32() && !args[2]->IsFunction()) 
      return VException("Requires a ***REMOVED*** string uri, integer flags and a callback function");    

  Local<String> ***REMOVED*** = args[0]->ToString();
  // Convert uri string to c-string
  char ****REMOVED***_str = (char *)calloc(***REMOVED***->Utf8Length() + 1, sizeof(char));
  // Write v8 string to c-string
  ***REMOVED***->WriteUtf8(***REMOVED***_str);

  // Allocate a structure
  AuthGSSClientCall *call = (AuthGSSClientCall *)calloc(1, sizeof(AuthGSSClientCall));
  call->flags =args[1]->ToInt32()->Uint32Value();
  call->uri = ***REMOVED***_str;
