...
exports['file.expand*'] = {
  ...
  'options.matchBase': function(test) {
    ...
    test.deepEqual(file.expand(opts, '*.js'), ['js/bar.js', 'js/foo.js'], 'options should be p***REMOVED***ed through to minimatch.');
...
exports['file'] = {
  ...
  'read': function(test) {
    test.expect(5);
    helpers.***REMOVED***ertTextEqual(file.read('tests/fixtures/utf8.txt'), this.string, test, 'file should be read as utf8 by default.');
    helpers.***REMOVED***ertTextEqual(file.read('tests/fixtures/iso-8859-1.txt', {encoding: 'iso-8859-1'}), this.string,test, 'file should be read using the specified encoding.');
    ...
    helpers.***REMOVED***ertTextEqual(file.read('tests/fixtures/iso-8859-1.txt'), this.string, test, 'changing the default encoding should work.');
    test.done();
  },
  ...
  'copy and process': function(test) {
    ...
    file.copy('tests/fixtures/utf8.txt', tmpfile.path, {
      process: function(src, filepath) {
        test.equal(filepath.replace(/\\/g, '/'), 'tests/fixtures/utf8.txt', 'filepath should be p***REMOVED***ed in, as-specified.');
    ...
    helpers.***REMOVED***ertTextEqual(file.read(tmpfile.path), 'føø' + this.string + 'bår', test, 'file should be saved as properly encoded processed string.');
    tmpfile.unlinkSync();
    ...
    helpers.***REMOVED***ertTextEqual(file.read(tmpfile.path, {encoding: 'iso-8859-1'}), 'føø' + this.string + 'bår', test, 'file should be saved as properly encoded processed string.');
    tmpfile.unlinkSync();
    ...
    helpers.***REMOVED***ertTextEqual(file.read(tmpfile.path), 'føø' + this.string + 'bår', test, 'file should be saved as the buffer returned by process.');
    tmpfile.unlinkSync();
  ...
  'copy and process, noprocess': function(test) {
    ...
    helpers.***REMOVED***ertTextEqual(file.read(tmpfile.path), this.string, test, 'file should not have been processed.');
    tmpfile.unlinkSync();
    ...
    ['process', 'noprocess', 'othernoprocess'].forEach(function(filename) {
      ...
      if (filename === 'process') {
        helpers.***REMOVED***ertTextEqual(file.read(tmpfile.path), 'føø' + this.string + 'bår', test, 'file should have been processed.');
      } else {
        helpers.***REMOVED***ertTextEqual(file.read(tmpfile.path), this.string, test, 'file should not have been processed.');
      }
      tmpfile.unlinkSync();
