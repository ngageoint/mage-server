...
encodings.forEach(function(encoding) {
    ...
    var tests = {
        "Convert to empty buffer": function() {
            ***REMOVED***ert.strictEqual(iconv.toEncoding("", enc).toString('binary'), new Buffer('').toString('binary'));
        },
        "Convert from empty buffer": function() {
            ***REMOVED***ert.strictEqual(iconv.fromEncoding(new Buffer(''), enc), "");
        },
        "Convert from buffer": function() {
            for (var key in encoding.encodedStrings)
                ***REMOVED***ert.strictEqual(iconv.fromEncoding(encoding.encodedStrings[key], enc),
                    encoding.strings[key]);
        },
        "Convert to buffer": function() {
            for (var key in encoding.encodedStrings)
                ***REMOVED***ert.strictEqual(iconv.toEncoding(encoding.strings[key], enc).toString('binary'),
        ...
        "Try different variations of encoding": function() {
            encoding.variations.forEach(function(enc) {
                ***REMOVED***ert.strictEqual(iconv.fromEncoding(encoding.encodedStrings[key], enc), encoding.strings[key]);
                ***REMOVED***ert.strictEqual(iconv.toEncoding(encoding.strings[key], enc).toString('binary'), encoding.encodedStrings[key].toString('binary'));
            });
        },
        "Untranslatable chars are converted to defaultCharSingleByte": function() {
            ...
            ***REMOVED***ert.strictEqual(iconv.toEncoding(encoding.strings.untranslatable, enc).toString('binary'), expected); // Only '?' characters.
        }
    };
