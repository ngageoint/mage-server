...
Creates an Archiver instance based on the format (zip, tar, etc) p***REMOVED***ed. Parameters can be p***REMOVED***ed directly to `Archiver` constructor for convenience.
...
Inherits [Transform Stream](http://nodejs.org/api/stream.html#stream_cl***REMOVED***_stream_transform) methods.
...
Appends multiple entries from p***REMOVED***ed array of src-dest mappings. A lazystream wrapper is used to prevent issues with open file limits.
...
Inherits [Transform Stream](http://nodejs.org/api/stream.html#stream_cl***REMOVED***_stream_transform) events.

#### entry

Fired when the input has been received, processed, and emitted. P***REMOVED***es entry data as first argument.
