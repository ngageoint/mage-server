...
require('mongodb').connect(uri, function (err, db) {
  ...
  // p***REMOVED*** it to the constructor
  mquery(collection).find({..}, callback);

  // or p***REMOVED*** it to the collection method
...
`mquery` requires a collection object to work with. In the example above we just p***REMOVED*** the collection object created using the official [MongoDB driver](https://github.com/mongodb/node-mongodb-native).
...
Declares this query a _find_ query. Optionally p***REMOVED*** a match clause and / or callback. If a callback is p***REMOVED***ed the query is executed.
...
mquery().find(match, function (err, docs) {
  ***REMOVED***ert(Array.isArray(docs));
})
```

###findOne()

Declares this query a _findOne_ query. Optionally p***REMOVED*** a match clause and / or callback. If a callback is p***REMOVED***ed the query is executed.
...
Declares this query a _count_ query. Optionally p***REMOVED*** a match clause and / or callback. If a callback is p***REMOVED***ed the query is executed.
...
Declares this query a _remove_ query. Optionally p***REMOVED*** a match clause and / or callback. If a callback is p***REMOVED***ed the query is executed.
...
Declares this query an _update_ query. Optionally p***REMOVED*** an update document, match clause, options or callback. If a callback is p***REMOVED***ed, the query is executed. To force execution without p***REMOVED***ing a callback, run `update(true)`.
...
mquery().update(true) // executes (u***REMOVED***fe write)
```

#####the update document

All paths p***REMOVED***ed that are not `$atomic` operations will become `$set` ops. For example:
...
Options are p***REMOVED***ed to the `setOptions()` method.

- overwrite

P***REMOVED***ing an empty object `{ }` as the update document will result in a no-op unless the `overwrite` option is p***REMOVED***ed. Without the `overwrite` option, the update operation will be ignored and the callback executed without sending the command to MongoDB to prevent accidently overwritting documents in the collection.
...
Declares this query a _findAndModify_ with update query. Optionally p***REMOVED*** a match clause, update document, options, or callback. If a callback is p***REMOVED***ed, the query is executed.

When executed, the first matching document (if found) is modified according to the update document and p***REMOVED***ed back to the callback.
...
Declares this query a _findAndModify_ with remove query. Optionally p***REMOVED*** a match clause, options, or callback. If a callback is p***REMOVED***ed, the query is executed.

When executed, the first matching document (if found) is modified according to the update document, removed from the collection and p***REMOVED***ed to the callback.
...
Declares this query a _distinct_ query. Optionally p***REMOVED*** the distinct field, a match clause or callback. If a callback is p***REMOVED***ed the query is executed.
...
// For a $nearSphere condition, p***REMOVED*** the `spherical` option.
...
When p***REMOVED***ing a string, prefixing a path with `-` will flag that path as excluded. When a path does not have the `-` prefix, it is included.
...
// p***REMOVED***ing query conditions is permitted too
...
If an object is p***REMOVED***ed, key values allowed are `asc`, `desc`, `ascending`, `descending`, `1`, and `-1`.

If a string is p***REMOVED***ed, it must be a space delimited list of path names. The sort order of each path is ascending unless the path name is prefixed with `-` which will be treated as descending.
...
If you need to specify tags, p***REMOVED*** any non-string argument as the first argument.
`mquery` will p***REMOVED*** this argument untouched to your collections methods later.
For example:
...
Merges other mquery or match condition objects into this one. When an muery instance is p***REMOVED***ed, its match conditions, field selection and options are merged.
...
- [hint](#hint) *
- [slaveOk](#slaveOk) *
- [safe](http://docs.mongodb.org/manual/reference/write-concern/): Boolean - p***REMOVED***ed through to the collection. Setting to `true` is equivalent to `{ w: 1 }`
...
Often times we want custom base queries that encapsulate predefined criteria. With `mquery` this is easy. First create the query you want to reuse and call its `toConstructor()` method which returns a new subcl***REMOVED*** of `mquery` that retains all options and criteria of the original.
...
`mquery` clones query arguments before p***REMOVED***ing them to a `collection` method for execution.
This prevents accidental side-affects to the objects you p***REMOVED***.
To clone `ObjectIds` we need to make some ***REMOVED***umptions.
...
back to calling `new obj.constructor(obj.id)`. We ***REMOVED***ume, for compatibility with the
Node.js driver, that the `ObjectId` instance has a public `id` property and that
when creating an `ObjectId` instance we can p***REMOVED*** that `id` as an argument.

#### Read Preferences

`mquery` supports specifying [Read Preferences]() to control from which MongoDB node your query will read.
The Read Preferences spec also support specifying tags. To p***REMOVED*** tags, some
drivers (Node.js driver) require p***REMOVED***ing a special constructor that handles both the read preference and its tags.
If you need to specify tags, p***REMOVED*** an instance of your drivers ReadPreference constructor or roll your own. `mquery` will store whatever you provide and p***REMOVED*** later to your collection during execution.
