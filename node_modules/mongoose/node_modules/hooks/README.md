...
We can use `hooks` to add validation and background jobs in the following way:
    ...
    Document.pre('save', function validate (next) {
      ...
      if (this.isValid()) next();      // next() p***REMOVED***es control to the next middleware
...
## Error Handling
You can define a default error handler by p***REMOVED***ing a 2nd function as the 3rd argument to `hook`:
...
Then, we can p***REMOVED*** errors to this handler from a pre or post middleware function:
...
doc.save( function (err, saved) {
  // We can p***REMOVED*** err via `next` in any of our pre or post middleware functions
...
they augment. This is useful if you want to mutate the arguments before p***REMOVED***ing
them along to the next middleware and eventually p***REMOVED*** a mutated arguments list to
...
that runs before `set`, alters the arguments by namespacing the `key` argument, and p***REMOVED***es them onto `set`:
...
As you can see above, we p***REMOVED*** arguments via `next`.

If you are not mutating the arguments, then you can p***REMOVED*** zero arguments
...
to the arguments.
    ...
    Document.pre('set', function (next, key, val) {
      ...
      next(); // We don't need to p***REMOVED*** anything to next
    });
    Document.pre('set', function (next, key, val) {
...
asynchronous ***REMOVED***s at once to respond.

For instance, you may only want to save a Document only after you have checked
that the Document is valid according to two different remote ***REMOVED***s.
...
- `next` p***REMOVED***es control to the next middleware in the chain
- `done` keeps track of how many parallel middleware have invoked `done` and p***REMOVED***es
   control to the target method when ALL parallel middleware have invoked `done`. If
   you p***REMOVED*** an `Error` to `done`, then the error is handled, and the main method that is
   wrapped by pres and posts will not get invoked.

We declare pre middleware that is parallel by p***REMOVED***ing a 3rd boolean argument to our `pre`
definition method.
...
We illustrate via the parallel validation example mentioned above:
    ...
    Document.pre('save', true, function preOne (next, doneOne, callback) {
      ...
      next(); // P***REMOVED*** control to the next middleware
    });
    
    // We will suppose that we need 2 different remote ***REMOVED***s to validate our document
...
1. You call `doc.save(...)`
2. First, your preOne middleware gets executed. It makes a remote call to the validation ***REMOVED*** and `next()`s to the preTwo middleware.
3. Now, your preTwo middleware gets executed. It makes a remote call to another validation ***REMOVED*** and `next()`s to the preThree middleware.
4. Your preThree middleware gets executed. It immediately `next()`s. But nothing else gets executing until both `doneOne` and `doneTwo` are invoked inside the callbacks handling the response from the two valiation ***REMOVED***s.
...
You can remove a particular pre ***REMOVED***ociated with a hook:

    Document.pre('set', someFn);
    Document.removePre('set', someFn);

And you can also remove all pres ***REMOVED***ociated with a hook:
