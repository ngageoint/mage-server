...
Handle<Value> SecurityCredentials::AquireSync(const Arguments &args) {
  HandleScope scope;  
  char *package_str = NULL, *username_str = NULL, *p***REMOVED***word_str = NULL, *domain_str = NULL;
  // Status of operation
  SECURITY_STATUS status;

  // Unpack the variables
  if(args.Length() != 2 && args.Length() != 3 && args.Length() != 4)
    return VException("Aquire must be called with either [package:string, username:string, [p***REMOVED***word:string, domain:string]]");
  ...
  // If we have a p***REMOVED***word
  if(args.Length() == 3 || args.Length() == 4) {
    Local<String> p***REMOVED***word = args[2]->ToString();
    p***REMOVED***word_str = (char *)calloc(p***REMOVED***word->Utf8Length() + 1, sizeof(char));
    p***REMOVED***word->WriteUtf8(p***REMOVED***word_str);    
  }
  ...
  // If we have a p***REMOVED***word string
  if(p***REMOVED***word_str != NULL) {
    // Set up the p***REMOVED***word
    security_credentials->m_Identity.P***REMOVED***word = USTR(_tcsdup(p***REMOVED***word_str));
    security_credentials->m_Identity.P***REMOVED***wordLength = (unsigned long)_tcslen(p***REMOVED***word_str);    
  }
  ...
  status = _sspi_AcquireCredentialsHandle(
    ...
    NULL, 
    p***REMOVED***word_str != NULL ? &security_credentials->m_Identity : NULL,
    NULL, NULL,
...
static void _authSSPIAquire(Worker *worker) {
  ...
  // If we have a p***REMOVED***word string
  if(call->p***REMOVED***word_str != NULL) {
    // Set up the p***REMOVED***word
    security_credentials->m_Identity.P***REMOVED***word = USTR(_tcsdup(call->p***REMOVED***word_str));
    security_credentials->m_Identity.P***REMOVED***wordLength = (unsigned long)_tcslen(call->p***REMOVED***word_str);    
  }
  ...
  status = _sspi_AcquireCredentialsHandle(
    ...
    SECPKG_CRED_OUTBOUND,
    NULL, 
    call->p***REMOVED***word_str != NULL ? &security_credentials->m_Identity : NULL,
    NULL, NULL,
    &security_credentials->m_Credentials,
    &security_credentials->Expiration
  );
  ...
  if(call->p***REMOVED***word_str != NULL) free(call->p***REMOVED***word_str);
...
Handle<Value> SecurityCredentials::Aquire(const Arguments &args) {
  ...
  if(args.Length() != 2 && args.Length() != 3 && args.Length() != 4 && args.Length() != 5)
    return VException("Aquire must be called with either [package:string, username:string, [p***REMOVED***word:string, domain:string], callback:function]");
  ...
  call->p***REMOVED***word_str = p***REMOVED***word_str;
...
void SecurityCredentials::Initialize(Handle<Object> target) {
  ...
  constructor_template->SetCl***REMOVED***Name(String::NewSymbol("SecurityCredentials"));

  // Cl***REMOVED*** methods
  NODE_SET_METHOD(constructor_template, "aquireSync", AquireSync);
  NODE_SET_METHOD(constructor_template, "aquire", Aquire);

  // Set the cl***REMOVED*** on the target module
