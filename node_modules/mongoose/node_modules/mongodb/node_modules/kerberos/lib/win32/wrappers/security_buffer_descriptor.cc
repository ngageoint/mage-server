...
Handle<Value> SecurityBufferDescriptor::New(const Arguments &args) {
  ...
  if(args.Length() != 1)
    return VException("There must be 1 argument p***REMOVED***ed in where the first argument is a [int32 or an Array of SecurityBuffers]");
...
void SecurityBufferDescriptor::Initialize(Handle<Object> target) {
  ...
  constructor_template->SetCl***REMOVED***Name(String::NewSymbol("SecurityBufferDescriptor"));
