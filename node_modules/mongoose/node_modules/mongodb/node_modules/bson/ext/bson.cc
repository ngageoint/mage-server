...
void BSON::Initialize(v8::Handle<v8::Object> target)
{
	...
	constructor_template->SetCl***REMOVED***Name(String::NewSymbol("BSON"));
...
// Create a new instance of BSON and p***REMOVED***ing it the existing context
Handle<Value> BSON::New(const Arguments &args)
{
	...
	if(args.Length() == 1 && args[0]->IsArray())
	{
		...
		if(array->Length() > 0)
		{
			...
			uint32_t foundCl***REMOVED***esMask = 0;
		...
		{
			return VException("No types p***REMOVED***ed in");
		}
	}
	else
	{
		return VException("Argument p***REMOVED***ed in must be an array of types");
	}
}
...
Handle<Value> BSON::BSONDeserialize(const Arguments &args)
{
	...
	// If we p***REMOVED***ed in a buffer, let's unpack it, otherwise let's unpack the string
