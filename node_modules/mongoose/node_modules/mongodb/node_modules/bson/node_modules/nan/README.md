...
We have **removed support for Node 0.11 versions prior to 0.11.13**, (although our tests are still p***REMOVED***ing for 0.11.10). As usual, our tests are run against (and p***REMOVED***) the last 5 versions of Node 0.8 and Node 0.10. We also include Node 0.11.13 obviously.
...
A helper for setting optional argument pointers. If the pointer is `NULL`, the function simply returns `false`.  Otherwise, the value is ***REMOVED***igned to the variable the pointer points to.
