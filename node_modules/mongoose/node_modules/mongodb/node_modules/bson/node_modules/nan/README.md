...
Two new functions have been introduced to replace the functionality that's been provided by `NanFromV8String` until now. NanCString has sensible defaults so it's super easy to fetch a null-terminated c-style string out of a `v8::String`. `NanFromV8String` is still around and has defaults that allow you to p***REMOVED*** a single handle to fetch a `char*` while `NanRawString` requires a little more attention to arguments.
...
Note that there is no embedded version sniffing going on here and also the async work is made much simpler, see below for details on the `NanAsyncWorker` cl***REMOVED***.
...
cl***REMOVED*** PiWorker : public NanAsyncWorker {
 public:
 ...
 * <a href="#api_nan_***REMOVED***ign_persistent"><b><code>NanAssignPersistent</code></b></a>
...
```c++
// .h:
cl***REMOVED*** Foo : public node::ObjectWrap {
  ...
...
A helper for setting optional argument pointers. If the pointer is `NULL`, the function simply return `false`.  Otherwise, the value is ***REMOVED***igned to the variable the pointer points to.
...
When you want to convert a V8 `String` to a `char*` buffer, use `NanRawString`. You have to supply an encoding as well as a pointer to a variable that will be ***REMOVED***igned the number of bytes in the returned string. It is also possible to supply a buffer and its length to the function in order not to have a new buffer allocated. The final argument allows setting `String::WriteOptions`.
...
When you want to convert a V8 `String` to a null-terminated C `char*` use `NanCString`. The resulting `char*` will be UTF-8-encoded, and you need to supply a pointer to a variable that will be ***REMOVED***igned the number of bytes in the returned string. It is also possible to supply a buffer and its length to the function in order not to have a new buffer allocated. The final argument allows optionally setting `String::WriteOptions`, which default to `v8::String::NO_OPTIONS`.
Just remember that you'll end up with an object that you'll need to `delete[]` at some point unless you supply your own buffer:
...
While it was possible to get around this, it required a shim by p***REMOVED***ing a
callback. So the new API `Buffer::Use(char*, uint32_t)` was introduced to remove
needing to use this shim.

`NanBufferUse` uses the `char*` p***REMOVED***ed as the backing data, and will free the
...
Can be used to check the type of an object to determine it is of a particular cl***REMOVED*** you have already defined and have a `Persistent<FunctionTemplate>` handle for.

<a name="api_nan_persistent_to_local"></a>
### Local&lt;Type&gt; NanPersistentToLocal(Persistent&lt;Type&gt;&)

Aside from `FunctionCallbackInfo`, the biggest and most painful change to V8 in Node 0.11 is the many restrictions now placed on `Persistent` handles. They are difficult to ***REMOVED***ign and difficult to fetch the original value out of.
...
<a name="api_nan_***REMOVED***ign_persistent"></a>
### NanAssignPersistent(type, handle, object)

Use `NanAssignPersistent` to ***REMOVED***ign a non-`Persistent` handle to a `Persistent` one. You can no longer just declare a `Persistent` handle and ***REMOVED***ign directly to it later, you have to `Reset` it in Node 0.11, so this makes it easier.

In general it is now better to place anything you want to protect from V8's garbage collector as properties of a generic `Object` and then ***REMOVED***ign that to a `Persistent`. This works in older versions of Node also if you use `NanAssignPersistent`:
...
User `NanInitPersistent` to declare and initialize a new `Persistent` with the supplied object. The ***REMOVED***ignment operator for `Persistent` is no longer public in Node 0.11, so this macro makes it easier to declare and initializing a new `Persistent`. See <a href="#api_nan_***REMOVED***ign_persistent"><b><code>NanAssignPersistent</code></b></a> for more information.
...
// p***REMOVED*** `callback` around and it's safe from GC until you:
delete callback;
```
...
`NanAsyncWorker` is an abstract cl***REMOVED*** that you can subcl***REMOVED*** to have much of the annoying async queuing and handling taken care of for you. It can even store arbitrary V8 objects for you and have them persist while the async work is in progress.

See a rough outline of the implementation:

```c++
cl***REMOVED*** NanAsyncWorker {
public:
  ...
  // be p***REMOVED***ed that string in an Error object
