...
Db.prototype.open = function(callback) {
  ...
  if(self.serverConfig instanceof Server || self.serverConfig instanceof ReplSet || self.serverConfig instanceof Mongos) {
    // Ensure we have the original options p***REMOVED***ed in for the server config
    var connect_options = {};
...
Db.prototype.eval = function(code, parameters, options, callback) {
  ...
  // Check if the nolock parameter is p***REMOVED***ed in
  if(options['nolock']) {
 ...
 * @param {String} p***REMOVED***word p***REMOVED***word.
 * @param {Object} [options] the options
 ...
 * @api public
 */
Db.prototype.authenticate = function(username, p***REMOVED***word, options, callback) {
  var self = this;
  ...
  // If cl***REMOVED***ic auth delegate to auth command
  if(options.authMechanism == 'MONGODB-CR') {
    mongodb_cr_authenticate(self, username, p***REMOVED***word, authdb, options, _callback);
  } else if(options.authMechanism == 'PLAIN') {
    mongodb_plain_authenticate(self, username, p***REMOVED***word, options, _callback);
  } else if(options.authMechanism == 'MONGODB-X509') {
    mongodb_x509_authenticate(self, username, p***REMOVED***word, options, _callback);
  } else if(options.authMechanism == 'GSSAPI') {
    //
    ...
    if(process.platform == 'win32') {
      mongodb_sspi_authenticate(self, username, p***REMOVED***word, authdb, options, _callback);
    } else {
      // We have the kerberos library, execute auth process
      mongodb_gssapi_authenticate(self, username, p***REMOVED***word, authdb, options, _callback);      
    }
  }
};
 ...
 *  - **fsync**, (Boolean, default:false) write waits for fsync before returning, from MongoDB 2.6 on, fsync cannot be combined with journal
 *  - **j**, (Boolean, default:false) write waits for journal sync before returning
 *  - **customData**, (Object, default:{}) custom data ***REMOVED***ociated with the user (only Mongodb 2.6 or higher)
 *  - **roles**, (Array, default:[]) roles ***REMOVED***ociated with the created user (only Mongodb 2.6 or higher)
 *
 * @param {String} username username.
 * @param {String} p***REMOVED***word p***REMOVED***word.
 * @param {Object} [options] additional options during update.
 ...
 * @api public
 */
Db.prototype.addUser = function(username, p***REMOVED***word, options, callback) {
  ...
  if(connection != null 
    ...
    && connection.serverCapabilities.hasAuthCommands) {
      return _executeAuthCreateUserCommand(this, username, p***REMOVED***word, options, callback);
  }
  ...
  md5.update(username + ":mongo:" + p***REMOVED***word);
  var userP***REMOVED***word = md5.digest('hex');
  ...
  collection.count({}, function(err, count) {
    ...
    collection.find({user: username}, {dbName: options['dbName']}).toArray(function(err, documents) {
      ...
      // We have a user, let's update the p***REMOVED***word or upsert if not
      collection.update({user: username},{$set: {user: username, pwd: userP***REMOVED***word}}, commandOptions, function(err, results, full) {
        if(count == 0 && err) {
          callback(null, [{user:username, pwd:userP***REMOVED***word}]);
        } else if(err) {
          callback(err, null)
        } else {
          callback(null, [{user:username, pwd:userP***REMOVED***word}]);
 ...
 * @ignore
 */
var _executeAuthCreateUserCommand = function(self, username, p***REMOVED***word, options, callback) {
  // Special case where there is no p***REMOVED***word ($external users)
  if(typeof username == 'string' 
    && p***REMOVED***word != null && typeof p***REMOVED***word == 'object') {
    callback = options;
    options = p***REMOVED***word;
    p***REMOVED***word = null;
  }
  ...
  // Error out if we digestP***REMOVED***word set
  if(options.digestP***REMOVED***word != null) {
    throw utils.toError("The digestP***REMOVED***word option is not supported via add_user. Please use db.command('createUser', ...) instead for this option.");
  }
  ...
  // No p***REMOVED***word
  if(typeof p***REMOVED***word == 'string') {
    command.pwd = userP***REMOVED***word;
  }
...
Db.prototype.removeUser = function(username, options, callback) {
  ...
  // Override with options p***REMOVED***ed in if applicable
...
Db.prototype.command = function(selector, options, callback) {
  ...
  // Add a write concern if we have p***REMOVED***ed in any
...
var __executeQueryCommand = function(self, db_command, options, callback) {
  ...
  // If we have a read preference set and we are a mongos p***REMOVED*** the read preference on to the mongos instance,
  ...
  if(typeof callback === 'function' && !onAll) {
    ...
    // Override connection if we p***REMOVED***ed in a specific connection
 ...
 * Default URL
 *
 * @cl***REMOVED***constant DEFAULT_URL
 **/
 ...
 *
 * Options
 *  - **uri_decode_auth** {Boolean, default:false} uri decode the user name and p***REMOVED***word for authentication
