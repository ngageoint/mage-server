...
Db.prototype.authenticate = function(username, p***REMOVED***word, options, callback) {
  ...
  if(options['connection'] != null) {
    //if a connection was explicitly p***REMOVED***ed on options, then we have only one...
  ...
  this._executeQueryCommand(DbCommand.createGetNonceCommand(self), options, function(err, result, connection) {
    ...
    if(err == null) {
      ...
      // Execute command
      self._executeQueryCommand(DbCommand.createAuthenticationCommand(self, username, p***REMOVED***word, nonce, authdb), {connection:connection}, function(err, result) {
        // Count down
        numberOfConnections = numberOfConnections - 1;
        ...
        if(numberOfConnections <= 0 && typeof callback == 'function') {
          ...
          if(errorObject == null && result.documents[0].ok == 1) {
            ...
            self.auths = [{'username':username, 'p***REMOVED***word':p***REMOVED***word, 'authdb': authdb}];
            // Return callback
...
Db.prototype.addUser = function(username, p***REMOVED***word, options, callback) {
  ...
  collection.count({}, function(err, count) {
    ...
    collection.find({user: username}, {dbName: options['dbName']}).toArray(function(err, documents) {
      ...
      collection.update({user: username},{$set: {user: username, pwd: userP***REMOVED***word}}, commandOptions, function(err, results) {
        if(count == 0 && err) {
    ...
    _self.serverConfig.connect(_self, {}, function(err, result, _serverConfig) {
      ...
      if(err != null && _numberOfRetriesDone > 0) {
        ...
        for(var i = 0; i < _self.auths.length; i++) {
          _self.authenticate(_self.auths[i].username, _self.auths[i].p***REMOVED***word, {'authdb':_self.auths[i].authdb}, function(err, authenticated) {
...
Db.connect = function(url, options, callback) {
  ...
  for(var i = 0; i < object.servers.length; i++) {
    ...
    if(object.rs_options.ssl) {
      ...
      _server_options.sslP***REMOVED*** = object.rs_options.sslP***REMOVED***;
      ...
      _server_options.sslP***REMOVED*** = object.server_options.sslP***REMOVED***;
    }
...
var _finishConnecting = function(serverConfig, object, options, callback) {
  ...
  db.open(function(err, db){
    if(err == null && object.auth){
      ...
      // Authenticate
      authentication_db.authenticate(object.auth.user, object.auth.p***REMOVED***word, function(err, success){
        if(success){
          callback(null, db);
        } else {
          if(db) db.close();
