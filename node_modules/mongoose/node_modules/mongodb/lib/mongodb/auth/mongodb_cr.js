...
var authenticate = function(db, username, p***REMOVED***word, authdb, options, callback) {
  ...
  db._executeQueryCommand(DbCommand.createGetNonceCommand(db), options, function(err, result, connection) {
    ...
    if(err == null) {
      ...
      // Execute command
      db._executeQueryCommand(DbCommand.createAuthenticationCommand(db, username, p***REMOVED***word, nonce, authdb), {connection:connection}, function(err, result) {
        // Count down
        ...
        if(numberOfConnections <= 0 && typeof callback == 'function') {
          ...
          if(errorObject == null && result.documents[0].ok == 1) {
            ...
            db.serverConfig.auth.add('MONGODB-CR', db.databaseName, username, p***REMOVED***word, authdb);
            // Return callback
