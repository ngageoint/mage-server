...
/**
 * Internal cl***REMOVED*** for callback storage
 * @ignore
 */
var CallbackStore = function() {
  // Make cl***REMOVED*** an event emitter
...
/**
 * Internal cl***REMOVED*** for holding non-executed commands
 * @ignore
 */
var NonExecutedOperationStore = function(config) {  
  ...
  this.validateBufferLimit = function(numberToFailOn) {
    ...
    // Error p***REMOVED***ed back
    var error = utils.toError("No connection operations buffering limit of " + numberToFailOn + " reached");

    // If we have p***REMOVED***ed the number of items to buffer we need to fail
...
/**
 * Internal cl***REMOVED*** for authentication storage
 * @ignore
 */
var AuthStore = function() {
  var _auths = [];

  this.add = function(authMechanism, dbName, username, p***REMOVED***word, authdbName, gssapiServiceName) {
    ...
    if(!this.contains(dbName)) {
      ...
      // Add auth source if p***REMOVED***ed in
...
/**
 * Internal cl***REMOVED*** for storing db references
 * @ignore
 */
...
var _apply_auths_serially = function(self, db, auths, callback) {
  ...
  if(auth.authMechanism == 'GSSAPI') {
    ...
    if(process.platform == 'win32') {
      mongodb_sspi_authenticate(db, auth.username, auth.p***REMOVED***word, auth.authdb, options, callback);
    } else {
      mongodb_gssapi_authenticate(db, auth.username, auth.p***REMOVED***word, auth.authdb, options, callback);
    }
  } else if(auth.authMechanism == 'MONGODB-CR') {
    mongodb_cr_authenticate(db, auth.username, auth.p***REMOVED***word, auth.authdb, options, callback);
  } else if(auth.authMechanism == 'PLAIN') {
    mongodb_plain_authenticate(db, auth.username, auth.p***REMOVED***word, auth.authdb, options, callback);
  } else if(auth.authMechanism == 'MONGODB-X509') {
    mongodb_x509_authenticate(db, auth.username, auth.p***REMOVED***word, auth.authdb, options, callback);
  }
}
