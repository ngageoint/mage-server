 ...
 *  - **sslP***REMOVED***** {Buffer/String, default:null}, String or buffer containing the certificate p***REMOVED***word (needs to have a mongod server with ssl support, 2.4 or higher)
 *
 * @cl***REMOVED*** Represents a Replicaset Configuration
...
var ReplSet = exports.ReplSet = function(servers, options) {
  ...
  // P***REMOVED***word to unlock private key
  this.sslP***REMOVED*** = this.options.sslP***REMOVED***;
  ...
  if(!Array.isArray(servers) || servers.length == 0) {
    ...
    servers.forEach(function(server) {
      ...
      server.sslP***REMOVED*** = self.sslP***REMOVED***;
...
ReplSet.prototype.allServerInstances = function() {
  ...
  // P***REMOVED***ive keys
  var keys = Object.keys(self._state.p***REMOVED***ives);
  // Add all arbiters
  for(var i = 0; i < keys.length; i++) {
    allServers.push(self._state.p***REMOVED***ives[keys[i]]);
  }
    ...
    if(self.dbInstances.length > 0) {
      ...
      if(self._haServer == null) {
        ...
        self._haServer = new Server(selectedServer.host, selectedServer.port, {
          ...
          sslKey: self.sslKey,
          sslP***REMOVED***: self.sslP***REMOVED***
        });
  ...
  server.connect(replset.db, options, function(err, result) {
    ...
    for(var x = 0; x <connections.length; x++) {
      ...
      for(var i = 0; i < auths.length; i++) {
        ...
        var p***REMOVED***word = auth.p***REMOVED***word;
        replset.db.authenticate(username, p***REMOVED***word, options, function() {
          --pending;
          if(0 === pending) {
            authDone = true;
            --pendingAuthConn;
...
function createServer(host, replset) {
  ...
  var serverOptions = {
    ...
    sslP***REMOVED***: replset.sslP***REMOVED***
  }
...
var _handler = function(event, self) {
  return function(err, server) {   
    ...
    delete self._state.p***REMOVED***ives[server.name];
...
var _connectHandler = function(self, candidateServers, instanceServer) {
  return function(err, result) {
    ...
    if(err == null && result.documents[0].hosts != null) {
      ...
      var p***REMOVED***ive = document.p***REMOVED***ive;
      ...
      var p***REMOVED***ives = Array.isArray(document.p***REMOVED***ives) ? document.p***REMOVED***ives : [];
      ...
      if(secondary == true && (p***REMOVED***ive == false || p***REMOVED***ive == null)) {
        self._state.secondaries[me] = instanceServer;
      } else if(arbiterOnly == true) {
        self._state.arbiters[me] = instanceServer;
      } else if(secondary == true && p***REMOVED***ive == true) {
        self._state.p***REMOVED***ives[me] = instanceServer;
      ...
      possibleHosts = Array.isArray(p***REMOVED***ives) ? possibleHosts.concat(p***REMOVED***ives) : possibleHosts;
      ...
      if(Array.isArray(candidateServers)) {
        ...
        for(var j = 0; j < possibleHosts.length; j++) {
          if(self._state.addresses[possibleHosts[j]] == null && possibleHosts[j] != null) {
            ...
            candidateServer.sslP***REMOVED*** = self.sslP***REMOVED***;
...
ReplSet.prototype.connect = function(parent, options, callback) {
  ...
  for(var i = 0; i < keys.length; i++) {
    ...
    server.***REMOVED***ignReplicaSet(this);
...
ReplSet.prototype.checkoutReader = function(readPreference, tags) {
  ...
  // from it, otherwise just p***REMOVED*** the primary connection
...
/**
 * Get list of all secondaries including p***REMOVED***ives
 * @ignore
 */
Object.defineProperty(ReplSet.prototype, "allSecondaries", {enumerable: true
  , get: function() {
      return this.secondaries.concat(this.p***REMOVED***ives);
    }
});
...
/**
 * Get list of p***REMOVED***ives
 * @ignore
 */
Object.defineProperty(ReplSet.prototype, "p***REMOVED***ives", {enumerable: true
  , get: function() {
      var keys = Object.keys(this._state.p***REMOVED***ives);
      var array = new Array(keys.length);
      // Convert arbiters to array
      for(var i = 0; i < keys.length; i++) {
        array[i] = this._state.p***REMOVED***ives[keys[i]];
      }
      return array;
    }
});
