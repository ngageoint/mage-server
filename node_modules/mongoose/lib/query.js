...
function Query(conditions, options, model, collection) {
  ...
  if (!this._mongooseOptions) {
    ...
    // options p***REMOVED***ed in, and if we did, merge them in
 ...
 *
 *     // Adventure is now a subcl***REMOVED*** of mongoose.Query and works the same way but with the
 ...
 * New in 3.7.3
 *
 * @return {Query} subcl***REMOVED***-of-Query
 * @api public
 */
...
/**
 * Specifies a javascript function or expression to p***REMOVED*** to MongoDBs query system.
 *
 * ####Example
 *
 ...
 *
 *     // p***REMOVED***ing query conditions is permitted
 ...
 *
 * When called with one argument, the most recent path p***REMOVED***ed to `where()` is used.
 *
 * ####Example
 *
 ...
 *
 * When a Query is p***REMOVED***ed, conditions, field selection and options are merged.
 *
 * New in 3.7.0
 *
 * @method merge
 ...
 * Finds documents.
 *
 * When no `callback` is p***REMOVED***ed, the query is not executed.
 *
 * ####Example
 *
...
/**
 * Declares the query a findOne operation. When executed, the first found document is p***REMOVED***ed to the callback.
 *
 * P***REMOVED***ing a `callback` executes the query.
 *
 * ####Example
 *
...
Query.prototype.findOne = function (conditions, fields, options, callback) {
  ...
  // don't p***REMOVED*** in the conditions because we already merged them in
 ...
 * Sets the sort order
 *
 * If an object is p***REMOVED***ed, values allowed are `asc`, `desc`, `ascending`, `descending`, `1`, and `-1`.
 *
 * If a string is p***REMOVED***ed, it must be a space delimited list of path names. The
 ...
 * ####Note
 *
 * The operation is only executed when a callback is p***REMOVED***ed. To force execution without a callback (which would be an u***REMOVED***fe write), we must first call remove() and then execute it by using the `exec()` method.
 *
 *     // not executed
 ...
 *
 *     // executed without a callback (u***REMOVED***fe write)
 *     query.exec()
 *
 *     // summary
 ...
 *
 * Finds a matching document, updates it according to the `update` arg, p***REMOVED***ing any `options`, and returns the found document (if any) to the callback. The query executes immediately if `callback` is p***REMOVED***ed.
 *
 * ####Available options
 *
 ...
 *
 * Finds a matching document, removes it, p***REMOVED***ing the found document (if any) to the callback. Executes immediately if `callback` is p***REMOVED***ed.
 *
 * ####Available options
 *
 * - `sort`: if multiple docs are found by the conditions, sets the sort order to choose which doc to update
 ...
 *
 * _All paths p***REMOVED***ed that are not $atomic operations will become $set ops._
 *
 * ####Example
 *
 ...
 * ####Note
 *
 * P***REMOVED***ing an empty object `{}` as the doc will result in a no-op unless the `overwrite` option is p***REMOVED***ed. Without the `overwrite` option set, the update operation will be ignored and the callback executed without sending the command to MongoDB so as to prevent accidently overwritting documents in the collection.
 *
 * ####Note
 *
 * The operation is only executed when a callback is p***REMOVED***ed. To force execution without a callback (which would be an u***REMOVED***fe write), we must first call update() and then execute it by using the `exec()` method.
 *
 *     var q = Model.where({ _id: id });
 *     q.update({ $set: { name: 'bob' }}).update(); // not executed
 *
 *     q.update({ $set: { name: 'bob' }}).exec(); // executed as u***REMOVED***fe
 *
 ...
 *     update(doc)
 *     update(cb) // executes
 *     update(true) // executes (u***REMOVED***fe write)
 *     update()
 *
...
Query.prototype._castUpdate = function _castUpdate (obj, overwrite) {
  ...
  // if we get p***REMOVED***ed {} for the update, we still need to respect that when it
  ...
  while (i--) {
    ...
    if ('Object' === val.constructor.name && !overwrite) {
      ...
      // user p***REMOVED***es {} and wants to clobber the whole document
 ...
 *     })
 *
 * Paths are populated after the query executes and a response is received. A separate query is then executed for each path specified for population. After a response for each query has also been returned, the results are p***REMOVED***ed to the callback.
 *
...
Query.prototype._castFields = function _castFields (fields) {
  ...
  if (selected) {
    // they p***REMOVED***ed $elemMatch, cast em
    try {
 ...
 *
 * The argument is ***REMOVED***igned to the most recent path p***REMOVED***ed to `where()`.
 *
 * ####NOTE:
 *
