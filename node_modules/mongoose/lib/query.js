...
/**
 * Executes the query as a findOne() operation, p***REMOVED***ing the first found document to the callback.
 *
 * ####Example
 *
 ...
 *
 * _All paths p***REMOVED***ed that are not $atomic operations will become $set ops so we retain backwards compatibility._
 *
 * ####Example
 *
 ...
 * ####Note
 *
 * P***REMOVED***ing an empty object `{}` as the doc will result in a no-op. The update operation will be ignored and the callback executed without sending the command to MongoDB so as to prevent accidently overwritting the collection.
 *
 ...
 * ####Example
 *
 *     C***REMOVED***ette.where('artist').equals('Anne Murray').remove(callback)
 *
 * @param {Function} callback
 * @api public
 ...
 *
 * Finds a matching document, removes it, p***REMOVED***ing the found document (if any) to the callback. Executes immediately if `callback` is p***REMOVED***ed else a Query object is returned.
 *
 * ####Available options
 *
 * - `sort`: if multiple docs are found by the conditions, sets the sort order to choose which doc to update
