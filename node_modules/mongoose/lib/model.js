 ...
 * @inherits Document
 * @event `error`: If listening to this event, it is emitted when a document was saved without p***REMOVED***ing a callback and an `error` occurred. If not listening, the event bubbles to the connection used to create this Model.
 * @event `index`: Emitted after `Model#ensureIndexes` completes. If an error occurred it is p***REMOVED***ed with the event.
 * @api public
 */
  ...
  return tick(function handleSave (err, result) {
    ...
    if (self.$__.version && !self.$__.inserting) {
      ...
      if (numAffected > 0) {
        ...
        // the update failed. p***REMOVED*** an error back
 ...
 *
 * The `fn` callback is optional. If no `fn` is p***REMOVED***ed and validation fails, the validation error will be emitted on the connection used to create this model.
 *
 ...
 *     var BossSchema = new BaseSchema({ ***REMOVED***: String });
 *
...
Model.discriminator = function discriminator (name, schema) {
  if (!(schema instanceof Schema)) {
    throw new Error("You must p***REMOVED*** a valid discriminator Schema");
  }
 ...
 *     });
 *
 * After completion, an `index` event is emitted on this `Model` p***REMOVED***ing an error if one occurred.
 *
 * ####Example:
 *
 ...
 * ####Note:
 *
 * To remove documents without waiting for a response from MongoDB, do not p***REMOVED*** a `callback`, then call `exec` on the returned [Query](#query-js):
 *
 ...
 *
 *     // executes immediately, p***REMOVED***ing results to callback
 *     MyModel.find({ name: 'john', age: { $gte: 18 }}, function (err, docs) {});
 *
 *     // name LIKE john and only selecting the "name" and "friends" fields, executing immediately
 *     MyModel.find({ name: /john/i }, 'name friends', function (err, docs) { })
 *
 *     // p***REMOVED***ing options
 *     MyModel.find({ name: /john/i }, null, { skip: 10 })
 *
 *     // p***REMOVED***ing options and executing immediately
 ...
 *
 *     // p***REMOVED***ing options (in this case return the raw js objects, not mongoose documents by p***REMOVED***ing `lean`
 ...
 *
 * P***REMOVED***ing a `callback` immediately executes the query.
 *
 * ####Example
 *
 ...
 *       if (err) return handleError(err);
 *
 *       ***REMOVED***ert(Array.isArray(result));
...
/**
 * Creates a Query, applies the p***REMOVED***ed conditions, and returns the Query.
 *
 ...
 *
 * Since the Query cl***REMOVED*** also supports `where` you can continue chaining
 *
 *     User
 ...
 *
 * Finds a matching document, updates it according to the `update` arg, p***REMOVED***ing any `options`, and returns the found document (if any) to the callback. The query executes immediately if `callback` is p***REMOVED***ed else a Query object is returned.
 *
 * ####Options:
 *
...
Model.findByIdAndUpdate = function (id, update, options, callback) {
  ...
  // if a model is p***REMOVED***ed in instead of an id
  if (id && id._id) {
    id = id._id;
  }
  if (id) {
 ...
 *
 * Finds a matching document, removes it, p***REMOVED***ing the found document (if any) to the callback.
 *
 * Executes immediately if `callback` is p***REMOVED***ed else a Query object is returned.
 *
 * ####Options:
 *
 * - `sort`: if multiple docs are found by the conditions, sets the sort order to choose which doc to update
 ...
 * ####Note:
 *
 * To update documents without waiting for a response from MongoDB, do not p***REMOVED*** a `callback`, then call `exec` on the returned [Query](#query-js):
 *
...
Model.geoNear = function (near, options, callback) {
  ...
  var promise = new Promise(callback);
  if (!near) {
    promise.error(new Error("Must p***REMOVED*** a near option to geoNear"));
    return promise;
  }
  ...
  if (Array.isArray(near)) {
    ...
    if (near.type != "Point" || !Array.isArray(near.coordinates)) {
      promise.error(new Error("Must p***REMOVED*** either a legacy coordinate array or GeoJSON Point to geoNear"));
      return promise;
    }
 ...
 *
 * If a `callback` is p***REMOVED***ed, the `aggregate` is executed and a `Promise` is returned. If a callback is not p***REMOVED***ed, the `aggregate` itself is returned.
 *
 * ####Example:
 *
...
Model.geoSearch = function (conditions, options, callback) {
  ...
  if (conditions == undefined || !utils.isObject(conditions)) {
    return promise.error(new Error("Must p***REMOVED*** conditions to geoSearch"));
  }
...
function populate (model, docs, options, cb) {
  ...
  // normalize single / multiple docs p***REMOVED***ed
  ...
  for (i = 0; i < len; i++) {
    ...
    if (ret) {
      ...
      // previously we always ***REMOVED***igned this even if the document had no _id
  ...
  var ***REMOVED***ignmentOpts = {};
  ***REMOVED***ignmentOpts.sort = options.options && options.options.sort || undefined;
  ***REMOVED***ignmentOpts.excludeId = /\s?-_id\s?/.test(select) || (select && 0 === select._id);

  if (***REMOVED***ignmentOpts.excludeId) {
    // override the exclusion from the query so we can use the _id
    // for document matching during ***REMOVED***ignment. we'll delete the
  ...
  // if a limit option is p***REMOVED***ed, we should have the limit apply to *each*
  // document, not apply in the aggregate
  if (options.options && options.options.limit) {
    ***REMOVED***ignmentOpts.originalLimit = options.options.limit;
...
function ***REMOVED***ignVals (o) {
  // replace the original ids in our intermediate _ids structure
  // with the documents found by query

  ***REMOVED***ignRawDocsToIdStructure(o.rawIds, o.rawDocs, o.rawOrder, o.options);
...
function valueFilter (val, ***REMOVED***ignmentOpts) {
  if (Array.isArray(val)) {
    // find logic
    var ret = [];
    for (var i = 0; i < val.length; ++i) {
      ...
      maybeRemoveId(subdoc, ***REMOVED***ignmentOpts);
      ret.push(subdoc);
      if (***REMOVED***ignmentOpts.originalLimit &&
          i + 1 >= ***REMOVED***ignmentOpts.originalLimit) {
        break;
      }
    }
    return ret;
  ...
  // findOne
  if (isDoc(val)) {
    maybeRemoveId(val, ***REMOVED***ignmentOpts);
    return val;
  }
...
function maybeRemoveId (subdoc, ***REMOVED***ignmentOpts) {
  if (***REMOVED***ignmentOpts.excludeId) {
 ...
 *   for findOne the results are ***REMOVED***igned directly to doc path (including null results).
 *   for find, if user specified sort order, results are ***REMOVED***igned directly
...
function ***REMOVED***ignRawDocsToIdStructure (rawIds, resultDocs, resultOrder, options, recursed) {
  ...
  for (var i = 0; i < rawIds.length; ++i) {
    ...
    if (Array.isArray(id)) {
      // handle [ [id0, id2], [id3] ]
      ***REMOVED***ignRawDocsToIdStructure(id, resultDocs, resultOrder, options, true);
      newOrder.push(id);
      continue;
    }
    ...
    if (recursed) {
      ...
      // ***REMOVED***ign matching documents in original order unless sorting
      doc = resultDocs[sid];
      if (doc) {
        if (sorting) {
      ...
      // apply findOne behavior - if document in results, ***REMOVED***ign, else ***REMOVED***ign null
  ...
  rawIds.length = 0;
  if (newOrder.length) {
    // re***REMOVED***ign the documents based on corrected order
...
/*!
 * Subcl***REMOVED*** this model with `conn`, `schema`, and `collection` settings.
 *
...
Model.__subcl***REMOVED*** = function subcl***REMOVED*** (conn, schema, collection) {
  // subcl***REMOVED*** model using this connection and collection name
  var model = this;
