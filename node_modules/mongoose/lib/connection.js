 ...
 * - 3 = disconnecting
 *
 * Each state change emits its ***REMOVED***ociated event name.
 *
 * ####Example
 *
...
/**
 * A hash of the collections ***REMOVED***ociated with this connection
 *
 ...
 *
 *     db      - p***REMOVED***ed to the connection db instance
 *     server  - p***REMOVED***ed to the connection server instance(s)
 *     replset - p***REMOVED***ed to the connection ReplSet instance
 *     user    - username for authentication
 *     p***REMOVED***    - p***REMOVED***word for authentication
...
Connection.prototype.open = function (host, database, port, options, callback) {
  ...
  // authentication
  if (options && options.user && options.p***REMOVED***) {
    this.user = options.user;
    this.p***REMOVED*** = options.p***REMOVED***;

  } else if (parsed && parsed.auth) {
    this.user = parsed.auth.user;
    this.p***REMOVED*** = parsed.auth.p***REMOVED***;

  // Check hostname for user/p***REMOVED***
  } else if (/@/.test(host) && /:/.test(host.split('@')[0])) {
    host = host.split('@');
    var auth = host.shift().split(':');
    host = host.pop();
    this.user = auth[0];
    this.p***REMOVED*** = auth[1];

  } else {
    this.user = this.p***REMOVED*** = undefined;
  }
 ...
 * The `options` is a hash which is p***REMOVED***ed to the internal driver connection object.
 *
 * Valid `options`
 *
 *     db      - p***REMOVED***ed to the connection db instance
 *     server  - p***REMOVED***ed to the connection server instance(s)
 *     replset - p***REMOVED***ed to the connection ReplSetServer instance
 *     user    - username for authentication
 *     p***REMOVED***    - p***REMOVED***word for authentication
 *     auth    - options for authentication (see http://mongodb.github.com/node-mongodb-native/api-generated/db.html#authenticate)
 *
 ...
 * @param {Object} [options] p***REMOVED***ed to the internal driver
 * @param {Function} [callback]
 * @see node-mongodb-native https://github.com/mongodb/node-mongodb-native
 * @see http://mongodb.github.com/node-mongodb-native/api-generated/db.html#authenticate
 * @api public
 ...
 * error
 *
 * Graceful error handling, p***REMOVED***es error to callback
...
Connection.prototype.onOpen = function (callback) {
  ...
  // re-authenticate
  if (self.user && self.p***REMOVED***) {
    self.db.authenticate(self.user, self.p***REMOVED***, self.options.auth, open);
  }
  else
    open();
};
...
Connection.prototype.model = function (name, schema, collection) {
  ...
  if (this.models[name] && !collection) {
    // model exists but we are not subcl***REMOVED***ing with custom collection
  ...
  if (this.models[name] && collection) {
    // subcl***REMOVED***ing current model with alternate collection
    model = this.models[name];
    schema = model.prototype.schema;
    var sub = model.__subcl***REMOVED***(this, schema, collection);
  ...
  return this.models[name] = model.__subcl***REMOVED***(this, schema, collection);
}
