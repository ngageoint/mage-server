...
describe('ncp', function () {
  describe('regular files and directories', function () {
      ...
      it('files are copied correctly', function (cb) {
        readDirFiles(src, 'utf8', function (srcErr, srcFiles) {
          readDirFiles(out, 'utf8', function (outErr, outFiles) {
            ***REMOVED***ert.ifError(srcErr);
            ***REMOVED***ert.deepEqual(srcFiles, outFiles);
            cb();
          });
        });
      });
    ...
    describe('when using transform', function () {
      it('file descriptors are p***REMOVED***ed correctly', function (cb) {
        ncp(src, out, {
           transform: function(read,write,file) {
              ***REMOVED***ert.notEqual(file.name, undefined);
              ***REMOVED***ert.strictEqual(typeof file.mode,'number');
              read.pipe(write);
           }
        }, cb);
      });
      ...
      it('output files are correctly redirected', function(cb) {
        ncp(src, out, {
          ...
          readDirFiles(src, 'utf8', function (srcErr, srcFiles) {
            readDirFiles(out, 'utf8', function (outErr, outFiles) {
              ***REMOVED***ert.ifError(srcErr);
              ***REMOVED***ert.deepEqual(srcFiles.a, outFiles.z);
              cb();
            });
          });
        });
  ...
  describe('symlink handling', function () {
    ...
    it('copies symlinks by default', function (cb) {
      ncp(src, out, function (err) {
        if (err) return cb(err);
        ***REMOVED***ert.equal(fs.readlinkSync(path.join(out, 'file-symlink')), 'foo');
        ***REMOVED***ert.equal(fs.readlinkSync(path.join(out, 'dir-symlink')), 'dir');
        cb();
      })
    });
    ...
    it('copies file contents when dereference=true', function (cb) {
      ncp(src, out, { dereference: true }, function (err) {
        ...
        ***REMOVED***ert.ok(fs.lstatSync(fileSymlinkPath).isFile());
        ***REMOVED***ert.equal(fs.readFileSync(fileSymlinkPath), 'foo contents');

        var dirSymlinkPath = path.join(out, 'dir-symlink');
        ***REMOVED***ert.ok(fs.lstatSync(dirSymlinkPath).isDirectory());
        ***REMOVED***ert.deepEqual(fs.readdirSync(dirSymlinkPath), ['bar']);
  ...
  describe('broken symlink handling', function () {
    ...
    it('copies broken symlinks by default', function (cb) {
      ncp(src, out, function (err) {
        if (err) return cb(err);
        ***REMOVED***ert.equal(fs.readlinkSync(path.join(out, 'broken-symlink')), 'does-not-exist');
        cb();
      })
    });
    ...
    it('returns an error when dereference=true', function (cb) {
      ncp(src, out, {dereference: true}, function (err) {
        ***REMOVED***ert.equal(err.length, 1);
        ***REMOVED***ert.equal(err[0].code, 'ENOENT');
        cb();
      });
    });
  });
