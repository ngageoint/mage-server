...
module.exports = function (transport, options) {
  ...
  var out = {
    'topic': logger,
    'when p***REMOVED***ed valid options': {
      'should have the proper methods defined': function () {
        switch (transport.name) {
          case 'console':
            helpers.***REMOVED***ertConsole(transport);
            break;
          case 'file':
            helpers.***REMOVED***ertFile(transport);
            break;
          case 'webhook':
            helpers.***REMOVED***ertWebhook(transport);
            break;
          case 'couchdb':
            helpers.***REMOVED***ertCouchdb(transport);
            break;
        }
    ...
    'the stream() method': {
        ...
        'should stream logs': function (err, results) {
          ...
          ***REMOVED***ert.isNull(err);
          results.forEach(function (log) {
            ***REMOVED***ert.ok(log.message.indexOf('hello world') === 0
      ...
      'using the `start` option': {
        ...
        'should stream logs': function (err, log) {
          if (!transport.stream) return;
          ***REMOVED***ert.isNull(err);
          ***REMOVED***ert.isNotNull(log.message);
          log.stream.destroy();
        }
      }
    },
      ...
      'the query() method': {
        ...
        'using the `rows` option': {
          ...
          'should return one result': function (err, results) {
            ...
            ***REMOVED***ert.equal(results.length, 1);
          }
        },
        'using `fields` and `order` option': {
          ...
          'should return matching results': function (err, results) {
            ...
            ***REMOVED***ert.equal(Object.keys(results[0]).length, 1);
            ***REMOVED***ert.ok(new Date(results.shift().timestamp)
        ...
        'using the `from` and `until` option': {
          ...
          'should return matching results': function (err, results) {
            ...
            ***REMOVED***ert.ok(results.length >= 1);
          }
        },
        'using a bad `from` and `until` option': {
          ...
          'should return no results': function (err, results) {
            ...
            ***REMOVED***ert.isUndefined(results[0]);
          }
        }
      }
    }
