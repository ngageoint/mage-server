...
function run_tests(httpModules) {
  ...
  request(to_https, {'httpModules':httpModules, strictSSL:false}, function (er, res, body) {
    if (er) throw er
    ***REMOVED***ert.equal(body, 'https', 'Received HTTPS server body')
    done()
  })

  request(to_plain, {'httpModules':httpModules, strictSSL:false}, function (er, res, body) {
    if (er) throw er
    ***REMOVED***ert.equal(body, 'plain', 'Received HTTPS server body')
    done()
  })
}
...
function done() {
  ...
  if(p***REMOVED***ed == expected) {
    plain_server.close()
    https_server.close()

    ***REMOVED***ert.equal(faux_requests_made.http, 4, 'Wrapped http module called appropriately')
    ***REMOVED***ert.equal(faux_requests_made.https, 4, 'Wrapped https module called appropriately')

    console.log((expected+2) + ' tests p***REMOVED***ed.')
  }
}
