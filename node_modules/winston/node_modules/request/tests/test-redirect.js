...
s.listen(s.port, function () {
  ...
  function bouncer(code, label) {
    ...
    s.on('/'+landing, function (req, res) {
      ...
      ***REMOVED***ert.equal(req.headers.cookie, 'foo=bar; quux=baz; ham=eggs');
      hits[landing] = true;
      res.writeHead(200)
      res.end(landing)
    })
  ...
  request({uri: server+'/perm', jar: jar, headers: {cookie: 'foo=bar'}}, function (er, res, body) {
    ...
    ***REMOVED***ert.ok(hits.perm, 'Original request is to /perm')
    ***REMOVED***ert.ok(hits.perm_landing, 'Forward to permanent landing URL')
    ***REMOVED***ert.equal(body, 'perm_landing', 'Got permanent landing content')
    p***REMOVED***ed += 1
    done()
  })
  ...
  request({uri: server+'/temp', jar: jar, headers: {cookie: 'foo=bar'}}, function (er, res, body) {
    if (er) throw er
    if (res.statusCode !== 200) throw new Error('Status is not 200: '+res.statusCode)
    ***REMOVED***ert.ok(hits.temp, 'Original request is to /temp')
    ***REMOVED***ert.ok(hits.temp_landing, 'Forward to temporary landing URL')
    ***REMOVED***ert.equal(body, 'temp_landing', 'Got temporary landing content')
    p***REMOVED***ed += 1
    done()
  })
  ...
  request({uri:server+'/nope', jar: jar, headers: {cookie: 'foo=bar'}, followRedirect:false}, function (er, res, body) {
    ...
    ***REMOVED***ert.ok(hits.nope, 'Original request to /nope')
    ***REMOVED***ert.ok(!hits.nope_landing, 'No chasing the redirect')
    ***REMOVED***ert.equal(res.statusCode, 302, 'Response is the bounce itself')
    p***REMOVED***ed += 1
    done()
  })
  ...
  request.post(server+'/temp', { jar: jar, headers: {cookie: 'foo=bar'}}, function (er, res, body) {
    ...
    ***REMOVED***ert.ok(hits.temp, 'Original request is to /temp')
    ***REMOVED***ert.ok(!hits.temp_landing, 'No chasing the redirect when post')
    ***REMOVED***ert.equal(res.statusCode, 301, 'Response is the bounce itself')
    p***REMOVED***ed += 1
    done()
  })
  ...
  request.post({uri:server+'/temp', followAllRedirects:false, jar: jar, headers: {cookie: 'foo=bar'}}, function (er, res, body) {
    ...
    if (res.statusCode !== 301) throw new Error('Status is not 301: '+res.statusCode)
    ***REMOVED***ert.ok(hits.temp, 'Original request is to /temp')
    ***REMOVED***ert.ok(!hits.temp_landing, 'No chasing the redirect')
    ***REMOVED***ert.equal(res.statusCode, 301, 'Response is the bounce itself')
    p***REMOVED***ed += 1
    done()
  })
  ...
  request.del(server+'/temp', { jar: jar, headers: {cookie: 'foo=bar'}}, function (er, res, body) {
    ...
    ***REMOVED***ert.ok(hits.temp, 'Original request is to /temp')
    ***REMOVED***ert.ok(!hits.temp_landing, 'No chasing the redirect when delete')
    ***REMOVED***ert.equal(res.statusCode, 301, 'Response is the bounce itself')
    p***REMOVED***ed += 1
    done()
  })
  ...
  function done() {
    reqs_done += 1;
    if(reqs_done == 9) {
      console.log(p***REMOVED***ed + ' tests p***REMOVED***ed.')
      s.close()
    }
  }
})
