...
s.listen(s.port, function () {
  ...
  function createTest(requestObj, serverAssertFn) {
    ...
    request(requestObj, function (err, res, body) {
      ***REMOVED***ert.ok(!err)
      ***REMOVED***ert.equal(res.statusCode, 200)
      numOutstandingTests -= 1
      if (numOutstandingTests === 0) {
        console.log(numTests + ' tests p***REMOVED***ed.')
        s.close()
      }
    })
  }
  ...
  createTest({headers: {cookie: 'foo=bar'}}, function (req, res) {
    ***REMOVED***ert.ok(req.headers.cookie)
    ***REMOVED***ert.equal(req.headers.cookie, 'foo=bar')
  })
  ...
  createTest({jar: jar, headers: {cookie: 'foo=bar'}}, function (req, res) {
    ***REMOVED***ert.ok(req.headers.cookie)
    ***REMOVED***ert.equal(req.headers.cookie, 'foo=bar; quux=baz')
  })
