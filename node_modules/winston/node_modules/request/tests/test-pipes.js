...
function ValidationStream(str) {
  ...
  this.on('end', function () {
    ***REMOVED***ert.equal(this.str, this.buf)
  })
  this.writable = true
}
...
s.listen(s.port, function () {
  ...
  mypulldata.end = function () {
    ***REMOVED***ert.equal(d, 'mypulldata');
    check();
  };
  ...
  s.on('/cat', function (req, resp) {
    if (req.method === "GET") {
      ...
      ***REMOVED***ert.equal(req.headers['content-type'], 'text/plain-test');
      ***REMOVED***ert.equal(req.headers['content-length'], 4)
      var validate = '';
      ...
      req.on('end', function () {
        resp.writeHead(201);
        resp.end();
        ***REMOVED***ert.equal(validate, 'asdf');
        check();
      })
    }
  })
  s.on('/pushjs', function (req, resp) {
    if (req.method === "PUT") {
      ***REMOVED***ert.equal(req.headers['content-type'], 'application/javascript');
      check();
    }
  })
  ...
  request.get('http://localhost:3453/catresp', function (e, resp, body) {
    ***REMOVED***ert.equal(resp.headers['content-type'], 'text/plain-test');
    ***REMOVED***ert.equal(resp.headers['content-length'], 4)
    check();
  })
  ...
  doodleWrite.on('close', function () {
    ***REMOVED***ert.deepEqual(fs.readFileSync(path.join(__dirname, 'googledoodle.jpg')), fs.readFileSync(path.join(__dirname, 'test.jpg')))
    check()
  })
  ...
  request.get({uri:'http://localhost:3453/onelineproxy', headers:{'x-oneline-proxy':'nope'}}, function (err, resp, body) {
    ***REMOVED***ert.equal(resp.headers['x-oneline-proxy'], 'yup')
    check()
  })
