...
Mocha.prototype.growl = function(runner, reporter) {
  ...
  runner.on('end', function(){
    ...
    if (stats.failures) {
      ...
      notify(stats.p***REMOVED***es + ' tests p***REMOVED***ed in ' + stats.duration + 'ms', {
          title: 'P***REMOVED***ed'
        , image: image('p***REMOVED***')
      });
    }
  });
};
...
/**
 * Check if both stdio streams are ***REMOVED***ociated with a tty.
 */
 ...
 * of tests p***REMOVED***ed / failed etc.
 *
 * @param {Runner} runner
 * @api public
 */
...
function Base(runner) {
  var self = this
    , stats = this.stats = { suites: 0, tests: 0, p***REMOVED***es: 0, pending: 0, failures: 0 }
  ...
  runner.on('p***REMOVED***', function(test){
    stats.p***REMOVED***es = stats.p***REMOVED***es || 0;
...
function Doc(runner) {
  ...
  runner.on('suite', function(suite){
    ...
    ++indents;
    console.log('%s<section cl***REMOVED***="suite">', indent());
    ++indents;
...
function HTMLCov(runner) {
  ...
  runner.on('end', function(){
    process.stdout.write(fn({
        cov: self.cov
      , coverageCl***REMOVED***: coverageCl***REMOVED***
    }));
  });
}

function coverageCl***REMOVED***(n) {
  if (n >= 75) return 'high';
...
var statsTemplate = '<ul id="stats">'
  + '<li cl***REMOVED***="progress"><canvas width="40" height="40"></canvas></li>'
  + '<li cl***REMOVED***="p***REMOVED***es">p***REMOVED***es: <em>0</em></li>'
  + '<li cl***REMOVED***="failures">failures: <em>0</em></li>'
  + '<li cl***REMOVED***="duration">duration: <em>0</em>s</li>'
  + '</ul>';
...
function HTML(runner) {
  ...
  var self = this
    ...
    , p***REMOVED***es = items[1].getElementsByTagName('em')[0]
  ...
  runner.on('suite', function(suite){
    ...
    // suite
    var el = fragment('<div cl***REMOVED***="suite"><h1>%s</h1></div>', suite.title);
  ...
  runner.on('test end', function(test){
    ...
    text(p***REMOVED***es, stats.p***REMOVED***es);
    text(failures, stats.failures);
    text(duration, (ms / 1000).toFixed(2));

    // test
    if ('p***REMOVED***ed' == test.state) {
      var el = fragment('<div cl***REMOVED***="test p***REMOVED*** %e"><h2>%e<span cl***REMOVED***="duration">%ems</span></h2></div>', test.speed, test.title, test.duration);
    } else if (test.pending) {
      var el = fragment('<div cl***REMOVED***="test p***REMOVED*** pending"><h2>%e</h2></div>', test.title);
    } else {
      var el = fragment('<div cl***REMOVED***="test fail"><h2>%e</h2></div>', test.title);
      ...
      el.appendChild(fragment('<pre cl***REMOVED***="error">%e</pre>', str));
    }
...
function JSONCov(runner, output) {
  ...
  runner.on('end', function(){
    ...
    result.p***REMOVED***es = p***REMOVED***es.map(clean);
    if (!output) return;
...
function List(runner) {
  ...
  runner.on('p***REMOVED***', function(test){
    console.log(JSON.stringify(['p***REMOVED***', clean(test)]));
  });
...
function JSONReporter(runner) {
  ...
  runner.on('end', function(){
    var obj = {
      ...
      , p***REMOVED***es: p***REMOVED***es.map(clean)
    };
...
function List(runner) {
  ...
  runner.on('test', function(test){
    process.stdout.write(color('p***REMOVED***', '    ' + test.fullTitle() + ': '));
  });
...
function Spec(runner) {
  ...
  runner.on('test', function(test){
    process.stdout.write(indent() + color('p***REMOVED***', '  ◦ ' + test.title + ': '));
  });
...
function XUnit(runner) {
  ...
  runner.on('end', function(){
    console.log(tag('testsuite', {
      ...
      , skip: stats.tests - stats.failures - stats.p***REMOVED***es
...
function test(test) {
  var attrs = {
      cl***REMOVED***name: test.parent.fullTitle()
    , name: test.title
 ...
 *   - `p***REMOVED***`  (test) test p***REMOVED***ed
  ...
  function next(err) {
    ...
    self.hookDown('beforeEach', function(){
      ...
      self.runTest(function(err){
        ...
        test.state = 'p***REMOVED***ed';
        self.emit('p***REMOVED***', test);
        self.emit('test end', test);
...
;(function(){
  ...
  function highlight(js) {
    return js
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
      .replace(/\/\/(.*)/gm, '<span cl***REMOVED***="comment">//$1</span>')
      .replace(/('.*?')/gm, '<span cl***REMOVED***="string">$1</span>')
      .replace(/(\d+\.\d+)/gm, '<span cl***REMOVED***="number">$1</span>')
      .replace(/(\d+)/gm, '<span cl***REMOVED***="number">$1</span>')
      .replace(/\bnew *(\w+)/gm, '<span cl***REMOVED***="keyword">new</span> <span cl***REMOVED***="init">$1</span>')
      .replace(/\b(function|new|throw|return|var|if|else)\b/gm, '<span cl***REMOVED***="keyword">$1</span>')
  }
