...
``` js
  //
  // You can add a separate exception logger by p***REMOVED***ing it to `.handleExceptions`
  //
...
When working with custom logger instances, you can p***REMOVED*** in separate transports to the `exceptionHandlers` property or set `.handleExceptions` on any transport.
...
Setting the level for your logging message can be accomplished in one of two ways. You can p***REMOVED*** a string representing the logging level to the log() method or use the level specified methods defined on every winston Logger.
...
Although there is slight repetition in this data structure, it enables simple encapsulation if you not to have colors. If you do wish to have colors, in addition to p***REMOVED***ing the levels to the Logger itself, you must make winston aware of them:
...
``` js
  ...
  // 2. By p***REMOVED***ing `transports` into the constructor function of winston.Container
  // 3. By p***REMOVED***ing `transports` into the `.get()` or `.add()` methods
  //
...
* __***REMOVED***AccessKey__:* your AWS Secret Access Key
...
* __p***REMOVED***word__ P***REMOVED***word for server auth
...
* __aws_***REMOVED***:__ Your Amazon Web Services Secret. *[required]*
...
* __graylogHostname:__ The hostname ***REMOVED***ociated with graylog2 messages. (default: require('os').hostname())
...
* __hostname:__ The hostname ***REMOVED***ociated with messages. (default: require('os').hostname())
...
### C***REMOVED***andra Transport

[winston-c***REMOVED***andra][24] is a C***REMOVED***andra transport:

``` js
  var C***REMOVED***andra = require('winston-c***REMOVED***andra').C***REMOVED***andra;
  winston.add(C***REMOVED***andra, options);
```

The C***REMOVED***andra transport connects to a cluster using the native protocol with the following options:

* __level:__ Level of messages that this transport should log (default: `'info'`).
* __table:__ The name of the C***REMOVED***andra column family you want to store log messages in (default: `'logs'`).
...
In addition to the options accepted by the [Node.js C***REMOVED***andra driver](https://github.com/jorgebay/node-c***REMOVED***andra-cql) Client.
...
[14]: http://nodejs.org/api/stream.html#stream_cl***REMOVED***_stream_writable
...
[24]: https://github.com/jorgebay/winston-c***REMOVED***andra
