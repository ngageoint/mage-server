...
The `Http` transport is a generic way to log, query, and stream logs from an arbitrary Http endpoint, preferably [winstond][1]. It takes options that are p***REMOVED***ed to the node.js `http` or `https` request:
...
* __auth:__ (Default: **None**) An object representing the `username` and `p***REMOVED***word` for HTTP Basic Auth
...
* __auth:__ (Default **None**) P***REMOVED***word set on the Redis server
...
[winston-c***REMOVED***andra][20] is a C***REMOVED***andra transport:
...
``` bash
  ...
  winston-c***REMOVED***andra    A C***REMOVED***andra transport for winston                             =jorgebay    
...
[20]: https://github.com/jorgebay/winston-c***REMOVED***andra
