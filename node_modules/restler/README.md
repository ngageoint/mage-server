...
* Deals with basic auth for you, just provide username and p***REMOVED***word options
* Simple ***REMOVED*** wrapper that allows you to easily put together REST API libraries
...
* `complete: function(result, response)` - emitted when the request has finished whether it was successful or not. Gets p***REMOVED***ed the response result and the response object as arguments. If some error has occurred, `result` is always instance of `Error`, otherwise it contains response data.
* `success: function(data, response)` - emitted when the request was successful. Gets p***REMOVED***ed the response data and the response object as arguments.
* `fail: function(data, response)` - emitted when the request was successful, but 4xx status code returned. Gets p***REMOVED***ed the response data and the response object as arguments.
* `error: function(err, response)` - emitted when some errors have occurred (eg. connection aborted, parse, encoding, decoding failed or some other unhandled errors). Gets p***REMOVED***ed the `Error` object and the response object (when available) as arguments.
...
* `abort([error])` Cancels request. `abort` event is emitted. `request.aborted` is set to `true`. If non-falsy `error` is p***REMOVED***ed, then `error` will be additionaly emitted (with `error` p***REMOVED***ed as a param and `error.type` is set to `"abort"`). Otherwise only `complete` event will raise.
...
* `p***REMOVED***word` Basic auth p***REMOVED***word. Defaults to empty.
* `multipart` If set the data p***REMOVED***ed will be formated as `multipart/form-encoded`. See multipart example below. Defaults to `false`.
...
rest.post('http://user:p***REMOVED***@***REMOVED***.com/action', {
  data: { id: 334 },
}).on('complete', function(data, response) {
...
rest.post('https://twaud.io/api/v1/upload.json', {
  multipart: true,
  username: 'danwrong',
  p***REMOVED***word: 'wouldntyouliketoknow',
  data: {
...
// create a ***REMOVED*** constructor for very easy API wrappers a la HTTParty...
Twitter = rest.***REMOVED***(function(u, p) {
  this.defaults.username = u;
  this.defaults.p***REMOVED***word = p;
}, {
...
var client = new Twitter('danwrong', 'p***REMOVED***word');
client.update('Tweeting using a Restler ***REMOVED*** thingy').on('complete', function(data) {
  console.log(data);
});
