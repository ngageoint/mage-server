 ...
 * will be p***REMOVED***ed as the first arg, followed by
...
Command.prototype.option = function(flags, description, fn, defaultValue){
  ...
  // pre***REMOVED***ign default value only for --no-*, [optional], or <required>
  if (false == option.bool || option.optional || option.required) {
    // when --no-* we make sure default is true
    if (false == option.bool) defaultValue = true;
    // pre***REMOVED***ign only if we have a default
  ...
  // when it's p***REMOVED***ed ***REMOVED***ign the value
  // and conditionally invoke the callback
  this.on(oname, function(val){
    // coercion
    if (null != val && fn) val = fn(val);

    // un***REMOVED***igned or bool
...
Command.prototype.parseOptions = function(argv){
  ...
  for (var i = 0; i < len; ++i) {
    ...
    if (arg.length > 1 && '-' == arg[0]) {
      ...
      // an argument for this option, we p***REMOVED*** it on.
 ...
 * which will print the version number when p***REMOVED***ed.
 *
 * @param {String} str
 * @param {String} flags
 * @return {Command} for chaining
...
/**
 * Prompt for p***REMOVED***word with `str`, `mask` char and callback `fn(val)`.
 *
 ...
 * Examples:
 *
 *     program.p***REMOVED***word('P***REMOVED***word: ', function(p***REMOVED***){
 *       console.log('got "%s"', p***REMOVED***);
...
Command.prototype.p***REMOVED***word = function(str, mask, fn){
  var self = this
    , buf = '';
  ...
  process.stdin.on('keypress', function(c, key){
    if (key && 'enter' == key.name) {
      ...
      if (!buf.trim().length) return self.p***REMOVED***word(str, mask, fn);
      fn(buf);
      return;
    }
