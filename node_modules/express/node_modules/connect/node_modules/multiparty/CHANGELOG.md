 ...
 * fix ***REMOVED***ertion crash when max field count is exceeded
 * fix ***REMOVED***ertion crash when client aborts an invalid request
 * (>=v0.10 only) unpipe the request when an error occurs to save resources.
 * update readable-stream to ~1.1.9
 * fix ***REMOVED***ertion crash when EMFILE occurrs
 * (no more ***REMOVED***ertions - only 'error' events)
