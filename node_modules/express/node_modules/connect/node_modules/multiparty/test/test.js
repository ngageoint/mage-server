  ...
  return function(done) {
    uploadFixture(name, function(err, parts) {
      ...
      fixture.forEach(function(expectedPart, i) {
        ...
        ***REMOVED***ert.equal(parsedPart.type, expectedPart.type);
        ***REMOVED***ert.equal(parsedPart.name, expectedPart.name);

        if (parsedPart.type === 'file') {
          var file = parsedPart.value;
          ***REMOVED***ert.equal(file.originalFilename, expectedPart.filename);
          if(expectedPart.sha1) ***REMOVED***ert.strictEqual(file.hash, expectedPart.sha1);
          if(expectedPart.size) ***REMOVED***ert.strictEqual(file.size, expectedPart.size);
        }
      });
      done();
    });
