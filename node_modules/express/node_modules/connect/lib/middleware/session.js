 ...
 *       .use(connect.session({ ***REMOVED***: 'keyboard cat', key: 'sid', cookie: { secure: true }}))
 *
 * Options:
 *
 *   - `key` cookie name defaulting to `connect.sid`
 *   - `store` session store instance
 *   - `***REMOVED***` session cookie is signed with this ***REMOVED*** to prevent tampering
 ...
 *         .use(connect.favicon())
 *         .use(connect.cookieParser())
 *         .use(connect.session({ ***REMOVED***: 'keyboard cat', cookie: { maxAge: 60000 }}))
 ...
 *  remaining in milliseconds, which we may also re-***REMOVED***ign a new value
...
function session(options){
  ...
  return function session(req, res, next) {
    ...
    // req.***REMOVED*** is p***REMOVED***ed from the cookie parser middleware
    var ***REMOVED*** = options.***REMOVED*** || req.***REMOVED***;

    // ensure ***REMOVED*** is available or bail
    if (!***REMOVED***) throw new Error('`***REMOVED***` option required for sessions');
