...
client credentials which include an identifier (e.g. username) and key (e.g. p***REMOVED***word). Likewise, just as with the Digest scheme,
...
Also unlike Digest, this scheme is not intended to protect the key itself (the p***REMOVED***word in Digest) because
the client and server must both have access to the key material in the clear.

The primary design goals of this scheme are to:
* simplify and improve HTTP authentication for ***REMOVED***s that are unwilling or unable to deploy TLS for all resources,
...
Implementers are strongly encouraged to ***REMOVED***ess how this module addresses their security requirements. This section includes
...
incorrect responses. Service providers should consider such attacks when developing ***REMOVED***s using this protocol, and should
...
The **Hawk** key functions the same way p***REMOVED***words do in traditional authentication systems. In order to compute the request MAC,
...
***REMOVED***ign keys which are long enough, and random enough, to resist such attacks for at least the length of time that the **Hawk**
...
`ext` feature to p***REMOVED*** application-specific information via the `Authorization` header which is protected by the request MAC.
...
requests only and therefore cannot be used to perform tra***REMOVED***ctions or change server state, it can still be used to expose private
...
a protocol handshake to the server. This pattern has become unacceptable for most web ***REMOVED***s, especially mobile
...
to authenticate simple client-server tra***REMOVED***ctions (i.e. two-legged). As you can guess, the OAuth working group is still hard
