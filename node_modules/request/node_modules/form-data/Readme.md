...
For edge cases, like POST request to URL with query string or to p***REMOVED*** HTTP auth credentials, object can be p***REMOVED***ed to `form.submit()` as first parameter:

``` javascript
form.submit({
  host: 'example.com',
  path: '/probably.php?extra=params',
  auth: 'username:p***REMOVED***word'
}, function(err, res) {
  console.log(res.statusCode);
});
```
