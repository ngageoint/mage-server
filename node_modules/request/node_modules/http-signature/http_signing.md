...
PKI prevent web ***REMOVED*** operators from deploying that methodoloy, and so many
...
of web ***REMOVED*** requests, but is not (always) ideal for machine to machine
communication, as the key acquisition steps (generally) imply a fixed
infrastructure that may not make sense to a ***REMOVED*** provider (e.g., symmetric
keys).

Several web ***REMOVED*** providers have invented their own schemes for signing
...
signing algorithm.  However, it does explicitly ***REMOVED***ume that clients can send an
HTTP `Date` header.
...
fingerprint, an LDAP DN, etc.  Management of keys and ***REMOVED***ignment of `keyId` is
...
then signed with the key ***REMOVED***ociated with `keyId` and the algorithm
...
If sending the request over plain HTTP, ***REMOVED*** providers SHOULD require clients
to sign ALL HTTP headers, and the `request-line`.  Additionally, ***REMOVED***
providers SHOULD require `Content-MD5` calculations to be performed to ensure
against any tampering from clients.

## Nonces

Nonces are out of scope for this document simply because many ***REMOVED*** providers
...
a ***REMOVED*** provider is fully enabled to add nonce semantics into this scheme by
using something like an `x-request-nonce` header, and ensuring it is signed
with the `Date` header.

## Clock Skew

As the default scheme is to sign the `Date` header, ***REMOVED*** providers SHOULD
...
It is out of scope for this document to dictate what headers a ***REMOVED*** provider
will want to enforce, but ***REMOVED*** providers SHOULD at minimum include the
`Date` header.
