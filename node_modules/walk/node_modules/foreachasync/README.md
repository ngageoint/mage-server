...
Analogous to `[].forEach`, but handles items asynchronously with a final callback p***REMOVED***ed to `then`.
...
```javascript
  ...
  function getPics(animal, cb) {
    var flickerAPI = "http://api.flickr.com/***REMOVED***s/feeds/photos_public.gne?jsoncallback=?";
    $.getJSON(
      flickerAPI
    , { tags: thing
      , tagmode: "any"
