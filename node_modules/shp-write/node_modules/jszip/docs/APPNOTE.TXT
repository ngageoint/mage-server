...
that the information contained in the ***REMOVED***ociated materials relating 
to the subject programs and/or the format of the files created or
accessed by the subject programs and/or the algorithms used by
the subject programs, or any other matter, is current, correct or
accurate as delivered.  Any risk of damage due to any possible
inaccurate information is ***REMOVED***umed by the user of the information.
...
-------       ------------------                       ----------
5.2           -Single P***REMOVED***word Symmetric Encryption    06/02/2003
               storage
...
--------------------------------
  ...
  C.  Data descriptor:
      ...
      Although not originally ***REMOVED***igned a signature, the value 
  ...
  F.  Central directory structure:
      ...
      Although not required, it is ***REMOVED***umed when encrypting the
  ...
  J.  Explanation of fields:
      ...
      extra field: (Variable)
          ...
          next header block, p***REMOVED***ing over any data blocks that are
          not of interest.
          ...
          value being correct, before ***REMOVED***uming that it is a block of
          known type.
...
(Rec-CTL) 0x0018    2 bytes  Tag for this "extra" block type
 ...
 (CStore) 0x0019    2 bytes  Tag for this "extra" block type
  ...
  K.  Splitting and Spanning ZIP files
          ...
          record.  Conflict in this alternate ***REMOVED***ignment can be
...
loop until done
    ...
    otherwise if the follower set S(Last-Character) is non-empty then
        ...
        otherwise if this bit is zero then
            ...
            stream, and ***REMOVED***ign this value to I.
            Copy the value of S(Last-Character)[I] to the
            output stream.

    ***REMOVED***ign the last value placed on the output stream to
    Last-Character.
end loop
...
loop until done
    ...
    case State of
        ...
        3:  move backwards D(V,C) bytes in the output stream
            ...
            stream, then ***REMOVED***ume that all the data before the
...
loop until done
    ...
    otherwise              (encoded data is sliding dictionary match)
        ...
        stream, then ***REMOVED***ume that all the data before the start of
...
except codes are ***REMOVED***igned starting at the shortest bit length, i.e. the
...
12 bits are p***REMOVED***ed as the sample size to the WavPack routines, the 4 
...
initialized using the supplied encryption p***REMOVED***word.  After each byte
...
1) Initialize the three 32-bit keys with the p***REMOVED***word.
...
loop for i <- 0 to length(p***REMOVED***word)-1
    update_keys(p***REMOVED***word(i))
end loop
...
used on versions after 2.0.  This can be used to test if the p***REMOVED***word
...
a p***REMOVED***word or an X.509v3 digital certificate to encrypt each file. 
This format specification supports either p***REMOVED***word or certificate 
...
P***REMOVED***word based encryption is the most common form of encryption 
people are familiar with.  However, inherent weaknesses with 
p***REMOVED***words (e.g. susceptibility to dictionary/brute force attack) 
as well as p***REMOVED***word management and support issues make certificate 
...
p***REMOVED***word based encryption. 
...
requires the appropriate p***REMOVED***word or private key for decryption prior to 
...
Directory and may ***REMOVED***ume the ZIP file is corrupt.  Programs that 
...
scalable, cross-platform encryption needs ranging from simple p***REMOVED***word
...
Single P***REMOVED***word Symmetric Encryption Method:
-------------------------------------------

The Single P***REMOVED***word Symmetric Encryption Method using strong 
...
2. Extra Field 0x0017 in central header only.
     ...
     Flags - Processing flags needed for decryption
         ...
         0x0001 - P***REMOVED***word is required to decrypt
         0x0002 - Certificates only
         0x0003 - P***REMOVED***word or certificate required to decrypt
...
3. Decryption header record preceding compressed file data.
         ...
         -Decryption Header:
          ...
          VSize     2 bytes  Size of p***REMOVED***word validation data
          VData     VSize-4  P***REMOVED***word validation data
          VCRC32    4 bytes  Standard ZIP CRC32 of p***REMOVED***word validation data
     ...
     ErdData - Encrypted random data is used to store random data that
               ...
               session key generated from the user-supplied p***REMOVED***word.
     ...
     VData - Random data for p***REMOVED***word validation.  This data is VSize
...
P***REMOVED***word = GetUserP***REMOVED***word()
MasterSessionKey = DeriveKey(SHA1(P***REMOVED***word)) 
...
Single P***REMOVED***word - Central Directory Encryption:
-----------------------------------------------
...
The Base 16 value ***REMOVED***igned as a masked file name is simply a sequentially
...
Compression is not required, but for storage efficiency it is ***REMOVED***umed
...
feature will ***REMOVED***ume the encryption method applied to files matches the 
...
operate in a manner similar to that of Single P***REMOVED***word Central
...
or signing the files within a ZIP file.  When only p***REMOVED***word 
...
Single P***REMOVED***word Symmetric Encryption Method as follows.  Instead
of using a user-defined p***REMOVED***word to generate a master session key,
...
This specification currently ***REMOVED***umes digital certificates will follow
...
only, and p***REMOVED***word encryption functions should not be affected by 
this value.  This means values of 61 may be found on files encrypted
with certificates only, or on files encrypted with both p***REMOVED***word
encryption and certificate encryption.  Files encrypted with both
methods can safely be decrypted using the p***REMOVED***word methods documented.
...
---------------
    ...
    Ziv, J. and Lempel, A., "Compression of individual sequences via
       variable-rate coding", IEEE Tra***REMOVED***ctions on Information Theory,
...
Field Code  Description
   ...
   000D     DF/SMS Management Cl***REMOVED***             8 bytes EBCDIC Text Value
   000E     DF/SMS Storage Cl***REMOVED***                8 bytes EBCDIC Text Value
   000F     DF/SMS Data Cl***REMOVED***                   8 bytes EBCDIC Text Value
...
on source or target encoding that may further ***REMOVED***ist applications with file 
...
p***REMOVED***word.  Values defining character encoding for file content or 
p***REMOVED***word must be stored within the 0x0008 Extended Language Encoding 
Extra Field.
