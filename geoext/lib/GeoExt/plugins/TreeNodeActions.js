 ...
 *    the name of the ``<img>`` cl***REMOVED***. The ``img`` tag being placed in a
 *    div whose cl***REMOVED*** is "gx-tree-layer-actions" a CSS selector for the
 ...
 *
 *      var uiCl***REMOVED*** = GeoExt.examples.LayerNodeUI = Ext.extend(
...
GeoExt.plugins.TreeNodeActions = Ext.extend(Ext.util.Observable, { 
    ...
    constructor: function(config) {
        ...
        GeoExt.plugins.TreeNodeActions.supercl***REMOVED***.constructor.apply(this, arguments);
    },
    ...
    onRenderNode: function(node) {
        ...
        if(!rendered) {
            ...
            if(actions && actions.length > 0) {
                var html = ['<div cl***REMOVED***="', this.actionsCls, '">'];
                for(var i=0,len=actions.length; i<len; i++) {
                    ...
                    html = html.concat([
                        ...
                        '" cl***REMOVED***="'+this.actionCls+' '+a.action+'" />'
                    ]);
                }
                html.concat(['</div>']);
    ...
    onRawClickNode: function(node, e) {
        if(e.getTarget('.' + this.actionCls, 1)) {
            ...
            var action = t.cl***REMOVED***Name.replace(this.actionCls + ' ', '');
